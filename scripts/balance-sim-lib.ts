...
// doesn't also register (and run) another file's describe/it blocks as a side effect.

let realSetTimeout: typeof setTimeout
export function installFastTimeouts() {
  realSetTimeout = global.setTimeout
  ;(global as any).setTimeout = (fn: any) => { fn(); return 0 }
}
export function restoreTimeouts() { global.setTimeout = realSetTimeout }
...
interface Counters {
  ...
  milestones: Record<number, { battles: number; heroActions: number; hits: number; totalDamage: number; atk: number; def: number; hp: number } | undefined>
}
...
function recordMilestones(counters: Counters) {
  ...
  for (const m of MILESTONES) {
    if (lvl >= m && !counters.milestones[m]) {
      // Captura ataque/defesa/vida efetivos (attackValue/defenseValue/maxHp -- mesmas funções que
      // o combate real usa) no momento exato em que o nível é cruzado, com o equipamento/atributos
      // reais que a simulação solo conquistou até aqui. Item 8: reaproveitado pelo balance-sim-coop
      // (scripts/balance-sim-coop.test.ts) como pool de "membros de grupo" realistas, em vez de
      // estimar stats de coop com uma heurística desconectada do progresso solo de verdade.
      counters.milestones[m] = { battles: counters.battles, heroActions: counters.heroActions, hits: counters.hits, totalDamage: counters.totalDamage, atk: attackValue(s), def: defenseValue(s), hp: maxHp(s) }
    }
  }
}
...
export function runHero(heroId: string, includeSteelmere = false) {
