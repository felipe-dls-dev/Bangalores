"""Recorta as folhas de sprite do Arcanista (Bases/*.png) em quadros PNG para a batalha.

Uso:
    python scripts/extract_mage_bases.py                 # regera os quadros
    python scripts/extract_mage_bases.py --measure        # só mede a extensão necessária do canvas
    python scripts/extract_mage_bases.py --preview DIR    # + tiras/GIFs de conferência em DIR

Requer: pip install pillow numpy scipy

Reaproveita a segmentação por "núcleo + propagação geodésica" do guerreiro
(scripts/extract_warrior_bases.py), no mesmo molde da sacerdotisa
(scripts/extract_priestess_bases.py). Este lote do Codex trouxe 5 folhas, sem Prompt_*.txt nem
Plano_Sprites.md (mapeamento de estado inferido pelo nome do arquivo):
  * `Descanso.png` / `Ataque.png`: idle/attack, mesmos nomes e grades da sacerdotisa/guardião.
  * `Ascensao_Arcana.png`: nome de invocação/buff (a habilidade ativa do Arcanista é um buff de
    grupo, sem nome próprio em src/data/herois.json) -> estado `skill`. Grade 5x2.
  * `Supremo_Arcano.png`: "Supremo" = padrão de golpe supremo das outras classes -> estado
    `ultimate`. Grade 4x3 (362px de célula, como o golpe supremo da sacerdotisa).
  * `Mestre_Sol_Negro.png` (2x2, 4 quadros): NÃO tem nome de estado óbvio e nenhum outro artefato
    desta leva (recorte "cru" que outro processo já tinha deixado em cima dos quadros antigos)
    tocou em heavy_*/hit_*/dodge_*/potion_*, então não foi mapeada para nenhum estado -- fica sem
    uso por enquanto, mesmo tratamento dos "leftovers" documentados no lote do druida
    (Bases/Defesa_Suave*.png, Bases/Ultimate_frames/ em docs/BATTLE_SPRITE_PROMPTS.md).
"""
import argparse
import io
import os
...
HERO_DIR = os.path.join(W.PROJECT_ROOT, 'public', 'assets', 'battle', 'sprites', 'heroes', 'arcanista')
BASES_DIR = os.path.join(HERO_DIR, 'Bases')

BODY_HEIGHT = 200
LEFT, RIGHT, UP, DOWN = 220, 210, 495, 125
...
SHEETS = {
    ...
    'Ascensao_Arcana': dict(state='skill', layout=[5, 5], min_core=13000, stand=[0, 9]),
    'Supremo_Arcano': dict(state='ultimate', layout=[4, 4, 4], min_core=16000, stand=[0, 11]),
}
