...
Integration (Antigravity & Claude Code):
- **Core Engine**: `src/battleSprites.ts` implements the full 13-state configuration table (`BATTLE_ANIMATION_CONFIG`), canonical path resolver (`getBattleSpriteFramePath`), enemy name normalization (`normalizeEnemySpriteId`), and real-time state resolution (`resolveFighterAnimationState`).
- **Component**: `src/components/BattleSpriteActor.tsx` renders animated frames, manages frame-stepping and FPS, loops or holds the last frame (victory/defeat), provides a CSS contact shadow, and gracefully falls back to `CardFrame` on any missing frame.
- **Combat Screen & Flip Card**: `src/main.tsx` (`CombatScreen` and `Fighter`) wires `battleViewMode` (`'cards'` | `'sprites'`), synchronized 3D flip animation with `perspective: 1200px` and `backface-visibility: hidden`, full accessibility overrides under `prefers-reduced-motion` (smooth cross-fade), interactive card-click flipping, and a dedicated view mode toggle in the action bar.
- **Asset Generation**: Pilot assets generated via `scripts/generate-battle-sprites.cjs` with over 400 transparent PNG frames covering heroes (`guerreiro`, `cacadora`, `monge`), enemies (`sentinela-runas`, `grumnak`), and FX (`impact-slash`, `block-spark`, `heal-glow`, `status-fire`) at `public/assets/battle/`.
- **Validation**: 100% automated test coverage in `src/battleSprites.test.ts` (191 tests passing project-wide), zero TypeScript compile errors (`tsc -b`), clean ESLint pass, and production bundle verified (`npm run build`).
...
Status: DELIVERED, NOT INTEGRATED (premise turned out to be wrong, see "Integration (Claude Code)" at the end of this entry)
...
Delivered (base 253x189 WebP; HD 1086x1448 WebP):
- `public/assets/art/monsters/lobo_dos_campos.webp` and `public/assets/art/hd/monsters/lobo-dos-campos-hd.webp`
- `public/assets/art/monsters/lobo_de_abdendriel.webp` and `public/assets/art/hd/monsters/lobo-de-abdendriel-hd.webp`
- `public/assets/art/monsters/lobo_do_gelo.webp` and `public/assets/art/hd/monsters/lobo-do-gelo-hd.webp`
- `public/assets/art/monsters/viuva_de_abdendriel.webp` and `public/assets/art/hd/monsters/viuva-de-abdendriel-hd.webp`
- `public/assets/art/monsters/tecela_gigante.webp` and `public/assets/art/hd/monsters/tecela-gigante-hd.webp`
- `public/assets/art/monsters/salamandra_de_ignaris.webp` and `public/assets/art/hd/monsters/salamandra-de-ignaris-hd.webp`
- `public/assets/art/monsters/goblin_bombeiro.webp` and `public/assets/art/hd/monsters/goblin-bombeiro-hd.webp`
- `public/assets/art/monsters/filhote_de_dragao_vermelho.webp` and `public/assets/art/hd/monsters/filhote-de-dragao-vermelho-hd.webp`
