...
| `Defesa.png` | `defend` e `hit` (`hit` não tem arquivos: `SPRITE_STATE_FRAME_ALIAS` aponta para `defend_*.png`) | 8 | 4+4 (1774x887) |
...
- **Chão por quadro na Defesa** (`own_ground`): a folha foi gerada com o quadro em pé mais baixo que os agachados (a primeira versão em 3+2 tinha 14 px de diferença); nas outras folhas o chão é a mediana da linha, na Defesa cada quadro usa as próprias botas (o druida nunca sai do chão).
- **Escala por folha** (`stand` em `SHEETS`): medida pela altura do corpo nos quadros em pé de cada folha (Descanso 0.575, Ataque 0.854, Crítico 1.07, Ultimate 1.196, Defesa 0.92), para o druida ter o mesmo tamanho em todas as animações.
