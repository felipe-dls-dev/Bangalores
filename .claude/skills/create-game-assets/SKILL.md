...
Bangalores has no in-house image generation tool and no 3D pipeline — it uses 2D raster art
only (WebP, character/monster/boss portraits, item icons), always sourced by the user
externally and handed over as local file paths for conversion/placement (see the project's
established workflow: user drops files in a Windows folder → this session reads/converts them
with Python PIL to WebP → places them under `public/assets/...` → wires the path into the
relevant TS/JSON data file). The most directly useful parts of this skill for this project are:
the **art-direction brief** structure (formalizing what's so far been ad hoc "match the existing
style" judgment calls), the **asset manifest** pattern (tracking status/provenance per art
asset — this project currently has none), and the **raster QA scripts**
(`asset_report.py`/`build_preview_sheet.py`) for catching wrong dimensions or missing alpha
before an asset gets wired in. The 3D and tile/parallax sections do not apply.
