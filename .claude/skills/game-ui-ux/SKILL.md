...
## Note for this project (Bangalores)

Bangalores is a React/TypeScript web app (no engine, no gamepad support today; it targets
desktop + mobile browser with mouse/touch). Map the concepts here directly onto the web stack:

- **Anchors + containers** → CSS flexbox/grid (already the project's pattern) instead of
  absolute positioning; this is the single biggest win from this skill and directly applies.
- **Scaling / safe area** → responsive `rem`/`%`/`clamp()` units and `env(safe-area-inset-*)`
  for notched mobile devices, rather than a fixed reference resolution.
- **Focus navigation** → standard web tab order / `:focus-visible` styling matters for
  accessibility even without gamepad support; worth auditing if a keyboard-only pass is asked
  for.
- **Event-driven HUD** → already how the project works: React re-renders off the Zustand store
  (`useGame`), which *is* the event-driven model this skill recommends over polling.
- **Screen stack** → the project's `screen` field in the Zustand store already acts as a simple
  state machine for screen flow; this skill's push/pop framing is useful if overlay-on-top-of-
  screen (e.g. a modal over combat) ever needs more structure than it has now.
