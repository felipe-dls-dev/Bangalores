...
Bangalores is a React/TypeScript, DOM-rendered, turn-based card-and-menu RPG — there is no
game engine, no per-frame render loop, and no camera object. Translate the *concepts* here, not
the code:

- **Screen shake** → CSS `transform` (translate/rotate) driven by a decaying trauma value in a
  React state/ref, animated via `requestAnimationFrame` or Framer Motion, applied to a wrapper
  element (e.g. the enemy card), never to layout-affecting properties.
- **Hit-stop** → a short `await` (~60-120ms) before resolving the next combat step, so the hit
  frame is visible before damage/text updates.
- **Squash & stretch / pop** → Framer Motion `animate`/`transition` with an overshoot
  (`type: 'spring'` or a `backOut`-style ease) instead of linear CSS transitions.
- **Tiers** → reuse the small/medium/large trauma table as a guide for how strong the shake/sfx
  combo should be for a normal hit vs. a critical vs. a boss/death event.
