...
import { Copy, Heart, Link2, LogOut, Map, ShieldHalf, Sword, Users, Wifi, WifiOff } from 'lucide-react'
import { attackValue, buildCoopEnemy, buildCoopRegionBoss, buildCoopSubregionBoss, defenseValue, HEROES, hasCraftedEffect, heroResistances, heroWeaponAnimationType, levelInfo, maxHp, regionListSort, SUBREGIONS, TERRITORIES, useGame } from '../store/game'
...
type MemberVitals={hp:number;maxHp:number;level?:number;attack?:number;defense?:number;shield?:number}
...
export default function PersistentCoopScreen(){
 ...
 const g=useGame(),coop=useCoop(),[name,setName]=React.useState(()=>{try{return localStorage.getItem('bangalores-coop-name')??''}catch{return ''}}),[code,setCode]=React.useState(codeFromLink),[linkCopied,setLinkCopied]=React.useState(false),startedBattle=React.useRef(''),publishedProgress=React.useRef('')
...
function CoopHostMap({regionId}:{regionId:string}){
 ...
 const handleAmbush=(nearestSubId:string)=>{if(ambushPrompt)return;const sub=subs.find(s=>s.id===nearestSubId);const enemy=sub&&buildCoopEnemy(sub.id,level,g.difficultyMode);if(sub&&enemy)setAmbushPrompt({enemy,subregionId:sub.id})}
 ...
 const explore=()=>{if(!encounterPrompt)return;const sub=encounterPrompt,bossReady=allHaveSubProgress(sub);setEncounterPrompt(undefined);const enemy=bossReady?buildCoopSubregionBoss(sub.id,g.difficultyMode):buildCoopEnemy(sub.id,level,g.difficultyMode);if(enemy)void coop.startMapBattle(sub.id,enemy as unknown as Record<string,unknown>)}
 const handleRegionBoss=()=>{const sub=activeSub??subs[0];if(!sub)return;const enemy=buildCoopRegionBoss(regionId,g.difficultyMode);if(enemy)void coop.startMapBattle(sub.id,enemy as unknown as Record<string,unknown>)}
