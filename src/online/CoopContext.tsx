  ...
  if(enemyHpNow>0){
   ...
   for(const member of membersRef.current){
    ...
    for(let index=0;index<after.length&&enemyHpNow>0;index++){const summon=after[index],attackRoll=1+Math.floor(Math.random()*6),defenseRoll=1+Math.floor(Math.random()*6),resolved=resolveCombatRoll(summon.ataque,summonDefenseBase,attackRoll,defenseRoll)
     ...
     else if(resolved.damage>0){enemyHpNow=Math.max(0,enemyHpNow-resolved.damage);const memberVitals=workingVitals[member.user_id],healAmount=memberVitals?Math.max(0,Math.min(1,Number(memberVitals.maxHp??memberVitals.hp)-Number(memberVitals.hp))):0;if(healAmount>0)workingVitals[member.user_id]={...memberVitals,hp:Number(memberVitals.hp)+healAmount};summonRolls.push({summonName:summon.nome,memberName:member.display_name,damage:resolved.damage,attackType:SUMMON_ATTACK_ANIMATION[summon.tipo]});statusLogs.push(`${summon.nome} (${member.display_name}) ataca: dado ${attackRoll} contra defesa ${defenseRoll}. Causou ${resolved.damage} de dano a ${battle.enemy?.nome}.${healAmount>0?' O vínculo com a fera recupera 1 de vida.':''}`)}
    }
