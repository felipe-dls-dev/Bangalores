...
export function CoopProvider({children}:{children:React.ReactNode}){
 ...
 const coopAttack=async(attackBase:number,defenseBase:number,rollBonus=0,critBoost=false,healChance=0,healAmount=0,label?:string,targetMinionId?:string,forceCrit=false,critChancePct=0,critDamageBonusPct=0,weaponElement:Element='fisico')=>{try{await updateState(current=>{
  ...
  const statusResult=!target&&actual>0&&!selfDamage?applyElementalStatus(enemyStun.status,weaponElement,buffedAttack):{status:enemyStun.status,appliedKind:undefined as string|undefined}
 ...
 const coopAbility=async(label:string,damage:number,effect:string)=>{try{await updateState(current=>{const battle=current.shared_state.battle as any;if(!battle||battle.status!=='playing'||battle.activeUserId!==userId)return current.shared_state
  ...
  const actual=Math.min(Number(battle.enemyHp??0),Math.max(0,damage)),enemyHp=Math.max(0,Number(battle.enemyHp??0)-actual),damageByPlayer={...(battle.damageByPlayer??{}),[userId]:Number(battle.damageByPlayer?.[userId]??0)+actual},actor=membersRef.current.find(m=>m.user_id===userId)?.display_name??'Aventureiro',keepsTurn=effect==='DOUBLE_ATTACK',next=keepsTurn?{activeUserId:userId,initiativeIndex:battle.initiativeIndex,round:battle.round}:nextInitiative(battle),playerBuffs=effect==='WARRIOR_BUFF'?{...(battle.playerBuffs??{}),[userId]:{...(battle.playerBuffs?.[userId]??{}),attackPct:Number(battle.playerBuffs?.[userId]?.attackPct??0)+.1,defensePct:Number(battle.playerBuffs?.[userId]?.defensePct??0)+.1}}:battle.playerBuffs,enemyFearPenalty=effect==='WARRIOR_BUFF'?Number(battle.enemyFearPenalty??0)+1:battle.enemyFearPenalty,groupBuff=effect==='ARCANE_GROUP_BUFF'?{...(battle.groupBuff??{}),roll:1,attackPct:.1,defensePct:.1}:effect==='HUNTER_CRITICAL'?{...(battle.groupBuff??{}),critBoost:true}:battle.groupBuff,extraActions=effect==='DOUBLE_ATTACK'?{...(battle.extraActions??{}),[userId]:1}:battle.extraActions,tauntUserId=effect==='GUARDIAN_TAUNT'?userId:battle.tauntUserId,vitals=(current.shared_state.memberVitals??{}) as Record<string,{hp:number;maxHp:number}>,healTarget=effect==='DRUID_HEAL'?membersRef.current.reduce((worst:{member:OnlineMember;vitals:{hp:number;maxHp:number};ratio:number}|null,member)=>{const v=vitals[member.user_id];if(!v||v.hp<=0)return worst;const ratio=v.hp/Math.max(1,v.maxHp);return!worst||ratio<worst.ratio?{member,vitals:v,ratio}:worst},null):null,healAmount=healTarget?Math.max(0,Math.min(Math.max(1,Math.ceil(healTarget.vitals.maxHp*.3)),healTarget.vitals.maxHp-healTarget.vitals.hp)):0,
 ...
 const resolveEnemyTurn=async()=>{if(roomRef.current?.host_id!==userId)return;try{await updateState(current=>{
  ...
  for(const member of membersRef.current){
   ...
   workingBuffs[member.user_id]=tick.status
   if(tick.damage>0){
  ...
  if(enemyHpNow<=0)return{...current.shared_state,memberVitals:workingVitals,battle:{...battle,enemyHp:0,enemyStatus:enemyStatusTick.status,playerBuffs:workingBuffs,status:'won',activeUserId:null,log:[...(battle.log??[]).slice(-15),...statusLogs]}}
  const wipedByStatus=membersRef.current.length>0&&membersRef.current.every(member=>(workingVitals[member.user_id]?.hp??1)<=0)
  if(wipedByStatus)return{...current.shared_state,memberVitals:workingVitals,battle:{...battle,playerBuffs:workingBuffs,enemyStatus:enemyStatusTick.status,status:'lost',activeUserId:null,log:[...(battle.log??[]).slice(-15),...statusLogs,'A equipe foi derrotada.']}}
  const enemyStun=consumeStun(enemyStatusTick.status)
  if(enemyStun.wasStunned){
   const next=nextInitiative(battle)
   return{...current.shared_state,memberVitals:workingVitals,battle:{...battle,...next,playerBuffs:workingBuffs,enemyStatus:enemyStun.status,turn:Number(battle.turn??1)+1,log:[...(battle.log??[]).slice(-15),...statusLogs,`${battle.enemy?.nome} está atordoado e perde a ação neste turno.`]}}
  }
  const enemyRollBonusStart=Number(battle.enemyRollBonus??0)
  const enemyFear=Number(battle.enemyFearPenalty??0)-rollPenaltyFrom(enemyStun.status)
  const pickTarget=()=>{
  ...
  const strike=(attackBase:number,bonus:number,canApplyStatus:boolean)=>{
   ...
   const defenseRoll=targetStun.wasStunned?1:Math.max(1,Math.min(6,naturalDefenseRoll+Number(battle.groupBuff?.roll??0)+Number(targetVitals.rollBonus??0)+(critDefenseBoost?1:0)-rollPenaltyFrom(targetStun.status)))
