...
import { attackEffect, defenseEffect, resolveCombatRoll, summonBossMinions, type AttackAnimType } from '../store/game'
...
type CoopVitals={hp:number;maxHp:number;level:number;defense:number;shield:number;rollBonus:number;critDefenseBoost:boolean;dodgeBoost?:boolean;weaponAnim?:AttackAnimType}
...
export function CoopProvider({children}:{children:React.ReactNode}){
 ...
 const coopAttack=async(attackBase:number,defenseBase:number,rollBonus=0,critBoost=false,healChance=0,healAmount=0,label?:string,targetMinionId?:string,forceCrit=false,critChancePct=0,critDamageBonusPct=0)=>{try{await updateState(current=>{
  ...
  const defenseRoll=Math.max(1,1+Math.floor(Math.random()*6)-Number(battle.enemyFearPenalty??0))
 ...
 const resolveEnemyTurn=async()=>{if(roomRef.current?.host_id!==userId)return;try{await updateState(current=>{
  ...
  const enemyFear=Number(battle.enemyFearPenalty??0)
  const pickTarget=()=>{
