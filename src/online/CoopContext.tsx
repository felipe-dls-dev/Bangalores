...
type CoopVitals={hp:number;maxHp:number;level:number;attack?:number;defense:number;armor?:number;dodgeChance?:number;elementalResist?:number;effectResist?:number;vigor?:number;energy?:number;energyMax?:number;shield:number;rollBonus:number;critDefenseBoost:boolean;dodgeBoost?:boolean;weaponAnim?:AttackAnimType;resistances?:Element[];locked?:boolean}
...
export function CoopProvider({children}:{children:React.ReactNode}){
 ...
 // freeAction: ação rápida (consumível, auditoria v0.8.84 GAME-004) -- aplica o efeito, registra no log
  ...
  const coopAbility=async(label:string,damage:number,effect:string,freeAction=false,potency=1)=>{try{await updateState(current=>{const battle=current.shared_state.battle as any;if(!battle||battle.status!=='playing'||battle.activeUserId!==userId)return current.shared_state
  ...
  healAmount=healTarget?(downedMember?Math.max(1,Math.ceil(healTarget.vitals.maxHp*Math.min(.9,.3*potency))):Math.max(0,Math.min(Math.max(1,Math.ceil(healTarget.vitals.maxHp*Math.min(.9,.3*potency))),healTarget.vitals.maxHp-healTarget.vitals.hp))):0,
  ...
  playerBuffs=effect==='WARRIOR_BUFF'?{...(battle.playerBuffs??{}),[userId]:{...(battle.playerBuffs?.[userId]??{}),attackPct:.1*potency,defensePct:.1*potency,buffTurnsLeft:3}}:effect==='DOUBLE_ATTACK'?{...(battle.playerBuffs??{}),[userId]:{...(battle.playerBuffs?.[userId]??{}),doubleAttackTurnsLeft:3}}:effect==='ITEM_CLEANSE'?{...(battle.playerBuffs??{}),[userId]:cleanseNegativeCoopBuffs(battle.playerBuffs?.[userId])}:(effect==='DRUID_HEAL'||effect==='PRIEST_REVIVE')&&healTarget?{...(battle.playerBuffs??{}),[healTarget.member.user_id]:cleanseNegativeCoopBuffs(battle.playerBuffs?.[healTarget.member.user_id])}:battle.playerBuffs,
  ...
  groupBuff=effect==='ARCANE_GROUP_BUFF'?{...(battle.groupBuff??{}),roll:1,attackPct:.1*potency,defensePct:.1*potency,arcaneTurnsLeft:3}:effect==='HUNTER_CRITICAL'?{...(battle.groupBuff??{}),critBoost:true,critTurnsLeft:2}:battle.groupBuff,
 ...
 const coopSummon=async(tipo:SummonType,potency=1)=>{try{await updateState(current=>{
  const battle=current.shared_state.battle as any
  if(!battle||battle.status!=='playing'||battle.activeUserId!==userId)return current.shared_state
  const personal=battle.playerBuffs?.[userId]??{}
  const actor=membersRef.current.find(m=>m.user_id===userId)?.display_name??'Aventureiro'
  ...
  const summon=scaleSummon(buildSummon(tipo,level),potency)
 ...
 const resolveEnemyTurn=async()=>{if(roomRef.current?.host_id!==userId)return;try{await updateState(current=>{
  ...
  const strike=(attackBase:number,bonus:number,canApplyStatus:boolean)=>{
   ...
   const defenseBase=intercepting?targetSummon!.defesa:coopMemberDefenseBase(targetVitals,defensePct)
   const dodgeChance=intercepting?0:coopMemberDodge(targetVitals,target.hero_id),rogueDodge=dodgeChance>0&&Math.random()<dodgeChance
   ...
   const{damage,shieldBlocked}=coopEnemyDamage({resolvedDamage:resolved.damage,dodged:rogueDodge,resisted,shield:Number(targetVitals.shield??0),intercepting,elementalEnemy:enemyElement!=='fisico',elementalResist:Number(targetVitals.elementalResist??0)})
