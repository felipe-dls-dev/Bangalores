...
export const useGame = create<GameState>()(persist((set,get)=>({
  ...
  sellEquipment:(id:string)=>{const s=get(),idx=s.equipmentBag.indexOf(id),e=eqById(id);if(idx<0||!e)return;const bag=[...s.equipmentBag];bag.splice(idx,1);const anotherCopy=bag.includes(id)||Object.values(s.equipped).includes(id);const equipmentUpgrades={...s.equipmentUpgrades},equipmentGems={...s.equipmentGems},forgedGemLocked={...s.forgedGemLocked},craftedEffects={...s.craftedEffects},equipmentElements={...s.equipmentElements},equipmentResistances={...s.equipmentResistances};if(!anotherCopy){delete equipmentUpgrades[id];delete equipmentGems[id];delete forgedGemLocked[id];delete craftedEffects[id];delete equipmentElements[id];delete equipmentResistances[id]}set({equipmentBag:bag,equipmentUpgrades,equipmentGems,forgedGemLocked,craftedEffects,equipmentElements,equipmentResistances,gold:s.gold+Math.max(1,Math.floor(e.preco/2))})},
  ...
  ,dismantleEquipment:(id:string)=>{const s=get(),index=s.equipmentBag.indexOf(id),item=eqById(id);if(index<0||!item)return;const yieldInfo=dismantlePreview(item),materials={...s.materials,fragmento_fisico:(s.materials.fragmento_fisico??0)+yieldInfo.physical,essencia_magica:(s.materials.essencia_magica??0)+yieldInfo.magical};const bag=[...s.equipmentBag];bag.splice(index,1);const anotherCopy=bag.includes(id)||Object.values(s.equipped).includes(id);const installed=anotherCopy?[]:s.equipmentGems[id]??[];for(const gem of installed)materials[gem]=(materials[gem]??0)+1;let gemName='';if(Math.random()<yieldInfo.gemChance){const gem=FORGE_GEMS[Math.floor(Math.random()*FORGE_GEMS.length)];materials[gem.id]=(materials[gem.id]??0)+1;gemName=` • Pedra encontrada: ${gem.nome}`};const equipmentUpgrades={...s.equipmentUpgrades},equipmentGems={...s.equipmentGems},forgedGemLocked={...s.forgedGemLocked},craftedEffects={...s.craftedEffects},equipmentElements={...s.equipmentElements},equipmentResistances={...s.equipmentResistances};if(!anotherCopy){delete equipmentUpgrades[id];delete equipmentGems[id];delete forgedGemLocked[id];delete craftedEffects[id];delete equipmentElements[id];delete equipmentResistances[id]}set({equipmentBag:bag,equipmentUpgrades,equipmentGems,forgedGemLocked,craftedEffects,equipmentElements,equipmentResistances,materials,explorationNote:`${item.nome} desmontado: +${yieldInfo.physical} fragmentos físicos, +${yieldInfo.magical} essências mágicas${gemName}.`})}
...
function applyDefeatPenalty(set:any,get:any,reason='Você foi derrotado.'){
 ...
 if(lostEntry){
  ...
  for(const lostId of [id,...storedLost])if(!remainingEquipped.has(lostId)&&!equipmentBag.includes(lostId)){delete equipmentUpgrades[lostId];delete equipmentGems[lostId];delete craftedEffects[lostId];delete forgedGemLocked[lostId];delete equipmentElements[lostId];delete equipmentResistances[lostId]}
...
function enemyAfterDelay(set:any,get:any){
 ...
 if(activeSummons.length&&s.enemy){
  ...
  const survivors=nextSummons.filter(fera=>fera.hp>0);set({summons:survivors,summon:survivors[0],enemyHp,hp:heroHp,...(!survivors.some(fera=>fera.tipo==='arcano')?{combatAttackPct:0,combatDefensePct:0}:{})})
