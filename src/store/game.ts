...
import { SHARED_EQUIPMENT } from '../data/sharedEquipment'
...
})),...EXTRA_EQUIPMENT,...CLASS_OFFHANDS,...CLASS_HEADGEAR,...CLASS_ARMOR,...CLASS_LEGWEAR,...CLASS_BOOTS,...NEW_CLASS_EQUIPMENT,...NEW_HEROES_WEAPONS,...SHARED_EQUIPMENT,...BACKPACKS]
...
interface GameState {
 ...
 enemy?:Enemy; enemyHp:number; combatMinions?:CombatMinion[]; combatTurn:number; combatLog:string[]; coin?:'cara'|'coroa'; playerTurn:boolean; animating:boolean; animationActor?:'hero'|'enemy'; lastDamage?:number; combatRoll?:CombatRoll; fleeRoll?:FleeRoll; heroRollBonus:number; enemyRollBonus:number; enemyFearPenalty:number; heroSkillUses:number; itemSkillUsed:boolean; shield:number; combatAttackPct:number; combatDefensePct:number; classRollBonus:number; classBuffTurns:number; summonBonusDamage:number; lifeWardActive:boolean; extraHeroAttacks:number; guardianTaunt:boolean; groupCriticalBoost:boolean; braced:boolean; braceBonusUsed:boolean; fervor:number; firstStrikeBonus:number; heroStatus?:StatusEffects; enemyStatus?:StatusEffects; supportFx?:{type:'fortificacao'|'cura'|'cura-item'};
...
function playerAttack(set:any,get:any,label:string,bonus=0,alreadyAnimating=false,targetMinionId?:string,forceCrit=false,forceStatusElement?:Element){
 ...
 const summonBondHeal=s.heroId==='conjurador'&&(s.summonBonusDamage??0)>0&&damage>0&&!selfDamage?1:0
 const healProc=druidHealProc(s),healed=Math.random()<healProc.chance,forgedHeal=hasCraftedEffect(s,'cura_forjada')&&Math.random()<.05,curaBonusMult=1+(hasCraftedEffect(s,'cura_bonus')?.1:0),rawHeal=((healed?healProc.amount:0)+(forgedHeal?Math.max(2,Math.round(attackBase*.25)):0))*curaBonusMult+summonBondHeal,healAmount=rawHeal>0?Math.max(0,Math.min(Math.round(rawHeal),maxHp(s)-s.hp)):0
 ...
 addLog(set,`${heroName} ${narration}.${defenseNarration} ${label}: dado ${attackRoll} (${attackEffect(attackRoll)}) contra defesa ${defenseRoll} (${defenseEffect(defenseRoll)}). ${selfDamage?`Recebeu ${selfDamage} de dano.`:`Causou ${damage} de dano${target?` a ${foeName}`:''}.`}${attackRoll===2?' Inimigo recebe +1 na próxima rolagem.':''}${healAmount>0?summonBondHeal>0?` O vínculo com a fera espectral recupera ${healAmount} de vida.`:` A energia natural do equipamento pulsa e recupera ${healAmount} de vida.`:''}${enemyStun.wasStunned?` ${foeName} estava atordoado e não conseguiu se defender.`:''}${statusResult.appliedKind?` ${foeName} fica ${STATUS_LABELS[statusResult.appliedKind]}.`:''}`)
 setTimeout(()=>{
...
function enemyAfterDelay(set:any,get:any){
 ...
 if((s.classBuffTurns??0)>0){
  ...
  if(turnsLeft<=0){
   const expired:any={classBuffTurns:0},label=s.heroId==='guerreiro'?'Ímpeto Marcial':s.heroId==='arcanista'?'Ascensão Arcana':s.heroId==='cacador'?'Marca do Predador':s.heroId==='conjurador'?'Conjurar Fera Espectral':undefined
   ...
   else if(s.heroId==='conjurador')Object.assign(expired,{summonBonusDamage:0})
   set(expired)
...
function enemyAttack(set:any,get:any){
 ...
 const priestDefenseHeal=!resolved.selfDamage&&s.heroId==='sacerdotisa'&&defenseRoll>=5&&Math.random()<.2?1:0
 const hp=Math.min(maxHp(s),Math.max(0,s.hp-raw)+priestDefenseHeal),enemyHp=Math.max(0,s.enemyHp-resolved.selfDamage)
