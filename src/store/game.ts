...
function playerAttack(set:any,get:any,label:string,bonus=0,alreadyAnimating=false){
 ...
 const attackBase=attackValue(s)+bonus,defenseBase=Math.max(0,(s.enemy.dificuldade??1)-2),naturalAttackRoll=Math.floor(Math.random()*6)+1,attackBonus=s.heroRollBonus+(s.classRollBonus??0),attackRoll=Math.min(6,naturalAttackRoll+attackBonus+(hasCraftedEffect(s,'critico')&&naturalAttackRoll===5?1:0)),defenseRoll=Math.floor(Math.random()*6)+1
...
function enemyAttack(set:any,get:any){
 ...
 const attackBase=s.enemy.ataque,defenseBase=defenseValue(s),naturalAttackRoll=Math.floor(Math.random()*6)+1,attackBonus=s.enemyRollBonus,attackRoll=Math.min(6,naturalAttackRoll+attackBonus),naturalDefenseRoll=Math.floor(Math.random()*6)+1,defenseRoll=Math.min(6,naturalDefenseRoll+(s.classRollBonus??0)+(hasCraftedEffect(s,'defesa_perfeita')&&naturalDefenseRoll===5?1:0))
 const dodged=s.heroId==='cacadora'&&Math.random()<.2,resolved=resolveCombatRoll(attackBase,defenseBase,attackRoll,defenseRoll);let raw=dodged?0:resolved.damage,shield=s.shield
