...
export interface GuildMission { id:string; nome:string; descricao:string; tipo:'any'|'specific'|'boss'; quantidade:number; alvo?:string; local?:string; recompensa:{tipo:'gold'|'equipment';valor:number}; dificuldade:number }
export const GUILD_MISSIONS:GuildMission[]=[
 {id:'caca_inicial',nome:'Limpeza das Estradas',descricao:'Derrote quaisquer 3 monstros nas terras de Eldravar.',tipo:'any',quantidade:3,recompensa:{tipo:'gold',valor:12},dificuldade:1},
 {id:'ameaca_goblin',nome:'Ameaça Goblin',descricao:'Derrote 3 inimigos que possuam Goblin no nome.',tipo:'specific',alvo:'goblin',quantidade:3,local:'Floresta Lunargenta',recompensa:{tipo:'gold',valor:22},dificuldade:2},
 {id:'caca_lobos',nome:'Peles para a Guilda',descricao:'Derrote 3 inimigos que possuam Lobo no nome.',tipo:'specific',alvo:'lobo',quantidade:3,local:'Campos ou regiões geladas',recompensa:{tipo:'equipment',valor:1},dificuldade:2},
 {id:'prova_chefes',nome:'Prova do Caçador',descricao:'Derrote qualquer chefe de sub-região.',tipo:'boss',quantidade:1,recompensa:{tipo:'equipment',valor:1},dificuldade:3},
 {id:'veterano_guilda',nome:'Veterano da Guilda',descricao:'Derrote 10 monstros de qualquer espécie.',tipo:'any',quantidade:10,recompensa:{tipo:'equipment',valor:1},dificuldade:4},
 {id:'queda_ignaroth',nome:'A Queda de Ignaroth',descricao:'Derrote o chefe Ignaroth no Ninho do Dragão Vermelho.',tipo:'boss',alvo:'ignaroth',quantidade:1,local:'Pico Escarlate',recompensa:{tipo:'gold',valor:100},dificuldade:6}
]
interface GameState {
...
export const useGame = create<GameState>()(persist((set,get)=>({
  ...
  acceptGuildMission:(id:string)=>{const state=get(),mission=GUILD_MISSIONS.find(m=>m.id===id);if(!mission||state.guildAccepted.includes(id)||state.guildClaimed.includes(id))return;set({guildAccepted:[...state.guildAccepted,id],guildProgress:{...state.guildProgress,[id]:state.guildProgress[id]??0},guildNotice:`Missão aceita: ${mission.nome}.`})},
  claimGuildMission:(id:string)=>{const state=get(),mission=GUILD_MISSIONS.find(m=>m.id===id);if(!mission||state.guildClaimed.includes(id)||(state.guildProgress[id]??0)<mission.quantidade)return;if(mission.recompensa.tipo==='gold')set({gold:state.gold+mission.recompensa.valor,guildClaimed:[...state.guildClaimed,id],guildNotice:`Recompensa recebida: ${mission.recompensa.valor} moedas de ouro.`});else{if(state.equipmentBag.length>=equipmentBagCapacity(state))return;const pool=EQUIPMENT.filter(e=>equipmentLevelAllowed(e,state.xp)&&equipmentClassAllowed(e,state.heroId)&&e.slot!=='bolsa');const reward=pool[Math.floor(Math.random()*pool.length)]??EQUIPMENT[0];set({equipmentBag:[...state.equipmentBag,reward.id],guildClaimed:[...state.guildClaimed,id],guildNotice:`Recompensa recebida: ${reward.nome}.`})}},
