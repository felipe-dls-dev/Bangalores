...
interface GameState {
 ...
 setCombatSpeed:(speed:1|2|3)=>void;toggleAutoCombat:()=>void;toggleLockEquipment:(ref:string)=>void;sortEquipmentBag:(criteria?:'rarity'|'level'|'name'|'slot')=>void;claimDailyReward:()=>{gold:number;materials?:Record<string,number>;message:string}|null;claimChallenge:(id:string)=>{gold:number;message:string}|null;
...
export const useGame = create<GameState>()(persist((set,get)=>({
  ...
  continueGame:()=>{const state=get(),saved=state.activeCampaignId&&state.campaigns[state.activeCampaignId];if(saved){const migrated=migrateEquipmentInstances(saved);set({...migrated,...normalizeAttributes(migrated),campaigns:state.campaigns,activeCampaignId:state.activeCampaignId,guildAccepted:migrated.guildAccepted??[],guildProgress:migrated.guildProgress??{},guildClaimed:migrated.guildClaimed??[],protectionBlessings:migrated.protectionBlessings??0,guildVault:migrated.guildVault??[],guildVaultUpgrades:migrated.guildVaultUpgrades??0,guildNotice:undefined,screen:resumableScreen(migrated.screen),animating:false,animationActor:undefined,lastDamage:undefined,combatRoll:undefined,fleeRoll:undefined,playerTurn:migrated.screen==='combat'&&migrated.enemy?true:(migrated.playerTurn??false)})}else set({screen:state.heroId?'map':'select'})},
  loadCampaign:(id:string)=>{const state=get(),saved=state.campaigns[id];if(!saved)return;const migrated=migrateEquipmentInstances(saved),campaigns=saveActiveCampaign(state);set({...migrated,...normalizeAttributes(migrated),campaigns,activeCampaignId:id,guildAccepted:migrated.guildAccepted??[],guildProgress:migrated.guildProgress??{},guildClaimed:migrated.guildClaimed??[],protectionBlessings:migrated.protectionBlessings??0,guildVault:migrated.guildVault??[],guildVaultUpgrades:migrated.guildVaultUpgrades??0,guildNotice:undefined,screen:resumableScreen(migrated.screen),animating:false,animationActor:undefined,lastDamage:undefined,combatRoll:undefined,fleeRoll:undefined,playerTurn:migrated.screen==='combat'&&migrated.enemy?true:(migrated.playerTurn??false)})},
  ...
  unequip:(slot:Slot)=>{const s=get(),id=s.equipped[slot];if(!id||slot==='bolsa'||s.equipmentBag.length>=equipmentBagCapacity(s))return;const eq={...s.equipped};delete eq[slot];set({equipped:eq,equipmentBag:[...s.equipmentBag,id]})},
   ...
   clearSave:()=>set({screen:'menu',heroId:undefined,hp:0,gold:0,xp:0,inventory:{},equipmentBag:[],protectionBlessings:0,guildVault:[],guildVaultUpgrades:0,equipped:{},territory:'Planícies de Alvora',regionId:'campos_dourados',subregionId:undefined,victories:{},subregionVictories:{},bossesDefeated:[],subregionBossesDefeated:[],currentEvent:undefined,eventResult:undefined,pendingAttackBonus:0,campaigns:{},activeCampaignId:undefined}),
  ...
  ,claimChallenge:(id:string)=>{
    ...
    set({gold:s.gold+challenge.reward,challengeClaimed:{...s.challengeClaimed,[id]:true}})
    return{gold:challenge.reward,message:`Desafio concluído: +${challenge.reward} de ouro!`}
  },
