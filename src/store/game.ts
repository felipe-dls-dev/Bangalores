...
import { abilityPotency, armorMitigation, computeChampionStats, effectResistance, elementalResistance, energyAt, initiativeBonus, mergeBonuses, sanitizeAttributes, spendEnergy, totalDodge, type ChampionStatBreakdown, type StatBonuses } from './heroStats'
...
interface GameState {
 screen:Screen; heroId?:string; hp:number; gold:number; xp:number; attributePoints:number; attr:PrimaryAttributes; allocatedAttr:PrimaryAttributes; permanentLife:number; balanceVersion:number; energy:number; energyTurn:number; lifeWardPct?:number;
...
export function energyNow(s:GameState){const max=championStats(s).energiaMaxima;return energyAt({energy:s.energy??max,turn:s.energyTurn??s.combatTurn},s.combatTurn,max,ATTRIBUTE_RULES.energia.regeneracaoPorRodada)}
...
function spendSkillEnergy(set:any,get:any,cost:number){const s=get() as GameState;const max=championStats(s).energiaMaxima,result=spendEnergy({energy:s.energy??max,turn:s.energyTurn??s.combatTurn},s.combatTurn,max,cost,ATTRIBUTE_RULES.energia.regeneracaoPorRodada);if(result.ok)set({energy:result.state.energy,energyTurn:result.state.turn});return result}
