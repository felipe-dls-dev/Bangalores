...
function playerAttack(set:any,get:any,label:string,bonus=0,alreadyAnimating=false,targetMinionId?:string,forceCrit=false){
 ...
 const attackBase=attackValue(s)+bonus+s.firstStrikeBonus+(!target&&s.talents.includes('cacador')&&s.enemy.boss?2:0),defenseBase=target?0:Math.max(0,(s.enemy.dificuldade??1)-2),naturalAttackRoll=forceCrit||forgedCrit?6:Math.floor(Math.random()*6)+1,attackBonus=s.heroRollBonus+(s.classRollBonus??0),attackRoll=forceCrit||forgedCrit?6:Math.min(6,naturalAttackRoll+attackBonus+((hasCraftedEffect(s,'critico')||s.groupCriticalBoost)&&naturalAttackRoll===5?1:0)),defenseRoll=Math.max(1,Math.floor(Math.random()*6)+1-(s.enemyFearPenalty??0))
 ...
 set({animating:true,playerTurn:false,animationActor:selfDamage?'enemy':'hero',lastDamage:selfDamage||damage,combatRoll,heroRollBonus:0,firstStrikeBonus:0,enemyRollBonus:attackRoll===2?1:s.enemyRollBonus,hp:healAmount>0?s.hp+healAmount:s.hp,fervor})
...
function enemyAttack(set:any,get:any){
 ...
 const attackBase=s.enemy.ataque,defenseBase=defenseValue(s)+(s.braced?2:0),naturalAttackRoll=Math.floor(Math.random()*6)+1,attackBonus=s.enemyRollBonus,attackRoll=Math.max(1,Math.min(6,naturalAttackRoll+attackBonus-(s.heroId==='druida'&&Math.random()<.25?1:0)-(s.enemyFearPenalty??0))),naturalDefenseRoll=Math.floor(Math.random()*6)+1,defenseRoll=Math.min(6,naturalDefenseRoll+(s.classRollBonus??0)+(hasCraftedEffect(s,'defesa_perfeita')&&naturalDefenseRoll===5?1:0))
 ...
 set({shield,animating:true,animationActor:resolved.selfDamage?'hero':'enemy',lastDamage:resolved.selfDamage||raw,combatRoll,playerTurn:false,enemyRollBonus:0,heroRollBonus:attackRoll===2?1:s.heroRollBonus,fervor:defenseRoll===6?Math.min(3,(s.fervor??0)+1):(s.fervor??0)})
 setTimeout(()=>{
