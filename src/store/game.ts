...
interface GameState {
 ...
 difficultyMode:DifficultyMode;talents:string[];materials:Record<string,number>;equipmentUpgrades:Record<string,number>;equipmentGems:Record<string,string[]>;forgedGemLocked:Record<string,boolean>;craftedEffects:Record<string,ForgeEffect>;forgeXp?:number;forgeAttempts?:number;forgeSuccesses?:number;bestiary:Record<string,{encontros:number;vitorias:number}>;discoveredCards:string[];revengeWins:Record<string,number>;dungeonDepth:number;dungeonActive:boolean;storyFlags:string[];storyChapterId:string;storyChoices:Record<string,string>;storyNotice?:string;coopBattlesCompleted:string[];
