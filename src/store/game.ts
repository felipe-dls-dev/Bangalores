...
interface GameState {
 ...
 newGame:(heroId:string)=>void; setScreen:(s:Screen)=>void; startCoopCombat:(enemy:Enemy,subregionId:string)=>void; syncCoopEnemyHp:(hp:number)=>void; receiveCoopEnemyAttack:(damage:number,roll:any)=>void; continueGame:()=>void; loadCampaign:(id:string)=>void; deleteCampaign:(id:string)=>void; acceptGuildMission:(id:string)=>void; claimGuildMission:(id:string)=>void; openRegion:(t:Territory)=>void; openSubregion:(subregionId:string)=>void; startEncounter:(subregionId:string)=>void; startBoss:()=>void;
...
export const useGame = create<GameState>()(persist((set,get)=>({
  ...
  syncCoopEnemyHp:(hp:number)=>{const s=get();if(s.screen!=='combat'||!s.enemy)return;const next=Math.max(0,hp);if(next===s.enemyHp)return;set({enemyHp:next});if(next<=0)victory(set,get)},
