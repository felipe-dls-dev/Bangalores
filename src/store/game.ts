...
export const useGame = create<GameState>()(persist((set,get)=>({
  ...
  startCoopCombat:(enemy:Enemy,subregionId:string)=>{const sub=SUBREGIONS.find(item=>item.id===subregionId),region=sub&&TERRITORIES.find(item=>item.id===sub.regionId),s=get() as GameState;if(!sub||!region)return;const warriorLuck=s.heroId==='guerreiro'&&Math.random()<.5,sets=equipmentSetCounts(s),setShield=sets.khar>=4?3:0,setRoll=enemy.boss&&sets.eclipse>=4?1:0;set({screen:'combat',regionId:region.id,territory:region.nome,subregionId,enemy,enemyHp:enemy.vida,combatTurn:1,combatLog:['Batalha cooperativa iniciada. A iniciativa foi sorteada para todo o grupo.',...(s.heroId==='guardiao'?['Passivo do Guardião: +2 de Defesa.']:[]),...(warriorLuck?['Fortuna do Guerreiro: +1 em todos os dados nesta batalha.']:[]),...(setShield?[`Conjunto de Kholgard: +${setShield} de escudo inicial.`]:[]),...(setRoll?['Conjunto do Sol Negro: +1 nas rolagens contra chefes.']:[])],playerTurn:false,animating:false,combatRoll:undefined,lastDamage:undefined,shield:s.shield+setShield,heroSkillUses:0,itemSkillUsed:false,heroRollBonus:(s.talents.includes('destino')?1:0)+setRoll+(warriorLuck?1:0)+equipmentRollBonus(s),classRollBonus:warriorLuck?1:0,groupCriticalBoost:false,braced:false,fervor:0,combatMinions:[],combatAttackPct:0,combatDefensePct:0,extraHeroAttacks:0,guardianTaunt:false})},
  ...
  defend:()=>{const s=get();if(!s.playerTurn||s.animating||!s.enemy)return;set({braced:true,playerTurn:false});addLog(set,'Postura defensiva: você se prepara para amortecer o próximo golpe do inimigo.');enemyAfterDelay(set,get)},
...
function enemyAttack(set:any,get:any){
 ...
 set({shield,animating:true,animationActor:resolved.selfDamage?'hero':'enemy',lastDamage:resolved.selfDamage||raw,combatRoll,playerTurn:false,enemyRollBonus:0,heroRollBonus:attackRoll===2?1:s.heroRollBonus,braced:false,fervor:defenseRoll===6?Math.min(3,(s.fervor??0)+1):(s.fervor??0)})
 setTimeout(()=>{
