...
interface GameState {
 ...
 attack:(targetMinionId?:string)=>void; heroSkill:()=>void; summonMonster:(tipo:SummonType)=>void; itemSkill:()=>void; useConsumable:(id:string)=>void; flee:()=>void; defend:()=>void; useFervor:()=>void;
...
export const useGame = create<GameState>()(persist((set,get)=>({
  ...
  ,startDungeon:()=>{const s=get(),sub=SUBREGIONS.find(x=>x.id===s.dungeonSubregionId)??SUBREGIONS.find(x=>x.id===s.subregionId)??SUBREGIONS.find(x=>x.regionId===s.regionId)??SUBREGIONS[0],depth=(s.dungeonActive?s.dungeonDepth:0)+1;const enemy=difficultyEnemy(depth%5===0?buildBoss(sub):buildEnemy(sub,deriveLevel(s.xp).lvl+depth),s.difficultyMode);beginCombat(set,get,{...enemy,nome:`Masmorra ${depth}: ${enemy.nome}`,vida:Math.ceil(enemy.vida*(1+depth*.12)),ouro:Math.ceil(enemy.ouro*(1+depth*.18)),dungeon:true});set({dungeonDepth:depth,dungeonActive:true,dungeonSubregionId:sub.id})}
