...
import type { Hero, Equipment, Consumable, Enemy, Territory, Subregion, Slot, Screen, Rarity, GameEvent, CustomCard, EquipmentActiveEffect, EquipmentSetId, ShopCategory, ShopTier } from '../types'
...
interface GameState {
 ...
 summons?:Summon[];
 loot?:Loot; selectedGallery:number; shopMode:'buy'|'sell'; shopVendor?:{category:ShopCategory;tier:ShopTier;npcId:string}; explorationNote?:string; currentEvent?:GameEvent; eventResult?:EventResult; pendingAttackBonus:number; activePotionIds:string[]; regenBoostUntil?:number; lastPassiveHealAt?:number; customCards:CustomCard[]; campaigns:Record<string,CampaignSave>; activeCampaignId?:string; guildAccepted:string[]; guildProgress:Record<string,number>; guildClaimed:string[]; guildNotice?:string;
...
export const useGame = create<GameState>()(persist((set,get)=>({
  ...
  setScreen:(screen:Screen)=>{const state=get();if(isNavigationLocked(state))return;set({screen,shopVendor:undefined,campaigns:saveActiveCampaign(state)})},
  // Entrar na Loja por um NPC especializado (armeiro/couraceiro/boticário, simples ou superior)
  // restringe o catálogo à categoria+tier dele -- setScreen('shop') direto pelo menu continua
  // abrindo o catálogo completo (shopVendor fica undefined nesse caso).
  openShopVendor:(npcId:string,category:ShopCategory,tier:ShopTier)=>{const state=get();if(isNavigationLocked(state))return;set({screen:'shop',shopVendor:{npcId,category,tier},campaigns:saveActiveCampaign(state)})},
