...
interface GameState {
 ...
 newGame:(heroId:string)=>void; setScreen:(s:Screen)=>void; startCoopCombat:(enemy:Enemy,subregionId:string)=>void; syncCoopEnemyHp:(hp:number)=>void; continueGame:()=>void; loadCampaign:(id:string)=>void; deleteCampaign:(id:string)=>void; acceptGuildMission:(id:string)=>void; claimGuildMission:(id:string)=>void; openRegion:(t:Territory)=>void; openSubregion:(subregionId:string)=>void; startEncounter:(subregionId:string)=>void; startBoss:()=>void;
...
export const useGame = create<GameState>()(persist((set,get)=>({
  ...
  startCoopCombat:(enemy:Enemy,subregionId:string)=>{const sub=SUBREGIONS.find(item=>item.id===subregionId),region=sub&&TERRITORIES.find(item=>item.id===sub.regionId);if(!sub||!region)return;set({regionId:region.id,territory:region.nome,subregionId});beginCombat(set,get,enemy)},
