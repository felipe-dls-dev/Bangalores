...
import { CLASS_ELEMENT, DIFFICULTIES, FORGE_BONUS_LABELS, FORGE_BONUS_MATERIAL, FORGE_GEMS, REGION_MATERIALS, STORY_CHAPTERS, TALENTS, type DifficultyMode, type Element, type ForgeBonus, type ForgeChoice, type ForgeEffect } from '../data/expansion'
...
export const TOUR_STEPS:{screen:Screen;title:string;text:string}[] = [
 {screen:'map',title:'Mapa',text:'Bem-vindo a Havendown! O Mapa é seu ponto de partida: escolha regiões e sub-regiões para explorar, começando pelas mais fáceis e avançando conforme sobe de nível.'},
 {screen:'character',title:'Ficha',text:'Acompanhe os atributos do seu herói, distribua pontos ao subir de nível e desbloqueie talentos permanentes na Árvore de Talentos.'},
 {screen:'inventory',title:'Mochila',text:'Guarda seus consumíveis (poções, elixires) e os equipamentos que não estão em uso no momento. Use itens diretamente por aqui ou durante o combate.'},
 {screen:'equipment',title:'Equipamentos',text:'Veja e troque o que está equipado em cada slot — arma, armadura, anéis e mais. Compare os atributos antes de substituir uma peça.'},
 {screen:'shop',title:'Loja',text:'Compre armas, armaduras e consumíveis com o ouro conquistado em combate, ou venda o que você não precisa mais.'},
 {screen:'forge',title:'Forja',text:'Desmonte equipamentos para obter materiais, crie itens novos por receita e instale pedras de melhoria. Atenção: pedras instaladas ficam fixas no item.'},
 {screen:'guild',title:'Guilda',text:'Aceite contratos para ganhar reputação, subir de rank e desbloquear recompensas melhores.'},
 {screen:'chronicle',title:'Crônicas',text:'Acompanhe a campanha narrativa principal e escolha o nível de dificuldade dos seus combates.'},
 {screen:'gallery',title:'Coleção',text:'Toda carta que você descobre — heróis, itens, monstros, chefes e eventos — fica registrada aqui para consulta.'},
 {screen:'coop',title:'Coop',text:'Jogue com outros aventureiros: crie uma sala ou entre com um código para enfrentar batalhas em grupo.'},
 {screen:'tutorial',title:'Tutorial',text:'Este é o manual completo do jogo, com todos os capítulos que você já viu (e mais). Você pode refazer este tour a qualquer momento pelo botão aqui embaixo.'},
]
...
interface GameState {
 ...
 enemy?:Enemy; enemyHp:number; combatMinions?:CombatMinion[]; combatTurn:number; combatLog:string[]; coin?:'cara'|'coroa'; playerTurn:boolean; animating:boolean; animationActor?:'hero'|'enemy'; lastDamage?:number; combatRoll?:CombatRoll; fleeRoll?:FleeRoll; heroRollBonus:number; enemyRollBonus:number; enemyFearPenalty:number; heroSkillUses:number; itemSkillUsed:boolean; shield:number; combatAttackPct:number; combatDefensePct:number; classRollBonus:number; classBuffTurns:number; summon?:Summon; lifeWardActive:boolean; extraHeroAttacks:number; guardianTaunt:boolean; groupCriticalBoost:boolean; braced:boolean; braceBonusUsed:boolean; fervor:number; firstStrikeBonus:number; heroStatus?:StatusEffects; enemyStatus?:StatusEffects; supportFx?:{type:'fortificacao'|'cura'|'cura-item'};
 ...
 difficultyMode:DifficultyMode;talents:string[];materials:Record<string,number>;equipmentUpgrades:Record<string,number>;equipmentGems:Record<string,string[]>;forgedGemLocked:Record<string,boolean>;craftedEffects:Record<string,ForgeEffect>;equipmentElements:Record<string,Element>;equipmentResistances:Record<string,Element>;forgeXp?:number;forgeAttempts?:number;forgeSuccesses?:number;forgeResult?:{success:boolean;message:string;id:number};bestiary:Record<string,{encontros:number;vitorias:number}>;discoveredCards:string[];revengeWins:Record<string,number>;dungeonDepth:number;dungeonActive:boolean;dungeonSubregionId?:string;storyFlags:string[];storyChapterId:string;storyChoices:Record<string,string>;storyNotice?:string;coopBattlesCompleted:string[];tourStep?:number;
...
export function itemSkillEffectText(item:{habilidade:string}){const txt=item.habilidade.toLowerCase();if(txt.includes('escudo'))return'Ativa: concede +3 de Escudo (bloqueia até 3 de dano no próximo golpe recebido). Uso único por batalha.';if(txt.includes('recupere'))return'Ativa: recupera 4 pontos de vida imediatamente. Uso único por batalha.';return'Ativa: realiza um ataque imediato com +3 de dano bônus sobre o seu Ataque atual. Uso único por batalha.'}
...
export const useGame = create<GameState>()(persist((set,get)=>({
  ...
  screen:'menu',hp:0,gold:0,xp:0,attributePoints:0,attr:{vida:0,ataque:0,defesa:0},allocatedAttr:{vida:0,ataque:0,defesa:0},balanceVersion:2,inventory:{},equipmentBag:[],equipped:{},territory:'Planícies de Alvora',regionId:'campos_dourados',world:'havendown',subregionId:undefined,victories:{},subregionVictories:{},bossesDefeated:[],subregionBossesDefeated:[],enemyHp:0,combatTurn:0,combatLog:[],playerTurn:false,animating:false,animationActor:undefined,lastDamage:undefined,combatRoll:undefined,fleeRoll:undefined,heroRollBonus:0,enemyRollBonus:0,enemyFearPenalty:0,heroSkillUses:0,itemSkillUsed:false,shield:0,classRollBonus:0,classBuffTurns:0,summon:undefined,lifeWardActive:false,groupCriticalBoost:false,braced:false,braceBonusUsed:false,fervor:0,firstStrikeBonus:0,heroStatus:{},enemyStatus:{},selectedGallery:0,shopMode:'buy',explorationNote:undefined,currentEvent:undefined,eventResult:undefined,pendingAttackBonus:0,customCards:[],campaigns:{},activeCampaignId:undefined,guildAccepted:[],guildProgress:{},guildClaimed:[],guildNotice:undefined,difficultyMode:'veterano',talents:[],materials:{},equipmentUpgrades:{},equipmentGems:{},forgedGemLocked:{},craftedEffects:{},equipmentElements:{},equipmentResistances:{},forgeResult:undefined,bestiary:{},discoveredCards:[],revengeWins:{},dungeonDepth:0,dungeonActive:false,dungeonSubregionId:undefined,storyFlags:[],storyChapterId:'prologo',storyChoices:{},storyNotice:undefined,
  ...
  itemSkill:(equipmentId?:string)=>{ const s=get(); if(s.itemSkillUsed||!s.playerTurn||s.animating||!s.enemy)return; const equippedIds=Object.values(s.equipped) as (string|undefined)[]; const item=(equipmentId?eqById(equipmentId):undefined)??equippedIds.map(id=>eqById(id)).find(e=>e?.habilidade&&e.slot!=='bolsa'); if(!item||item.slot==='bolsa'||!Object.values(s.equipped).includes(item.id)){addLog(set,'Nenhum equipamento com habilidade ativa.');return} set({itemSkillUsed:true}); const txt=item.habilidade.toLowerCase(); if(txt.includes('escudo')){set({shield:s.shield+3});addLog(set,`${item.nome}: +3 Escudo.`);triggerSupportFx(set,get,'fortificacao');enemyAfterDelay(set,get);return} if(txt.includes('recupere')){const m=maxHp(s);set({hp:Math.min(m,s.hp+4)});addLog(set,`${item.nome}: recuperou 4 de vida.`);triggerSupportFx(set,get,'cura-item');enemyAfterDelay(set,get);return} playerAttack(set,get,item.nome,3,true) },
  useConsumable:(id:string)=>{ const s=get(),it=CONSUMABLES.find(x=>x.id===id);if(!it||(s.inventory[id]??0)<=0)return;if(s.screen==='combat'&&(!s.playerTurn||s.animating||!s.enemy))return;if(s.screen==='combat'&&heroStunned(set,get))return;const inv={...s.inventory,[id]:(s.inventory[id]??0)-1};if(inv[id]<=0)delete inv[id];const value=consumableEffectiveValue(it,s);let resultMessage=`${it.nome} utilizado.`;if(it.tipo==='cura')set({inventory:inv,hp:Math.min(maxHp(s),s.hp+value)});else if(it.tipo==='vida_max'){const success=Math.random()<(LIFE_CHANCE[id]??.35);if(success){const newMax=maxHp(s)+value,newHp=id==='elixir_fenix'?newMax:Math.min(newMax,s.hp+value);resultMessage=`${it.nome}: sucesso! Vida máxima aumentada permanentemente em ${value}.`;set({inventory:inv,attr:{...s.attr,vida:s.attr.vida+value},hp:newHp,explorationNote:resultMessage})}else{resultMessage=`${it.nome}: a tentativa falhou e a vida máxima não aumentou.`;set({inventory:inv,explorationNote:resultMessage})}}else if(it.tipo==='escudo')set({inventory:inv,shield:s.shield+value});else if(it.tipo==='regen_boost'){resultMessage=`${it.nome}: cura acelerada ativada.`;set({inventory:inv,explorationNote:resultMessage})}else{resultMessage=`${it.nome}: +${value} de ataque até o fim do próximo combate.`;set({inventory:inv,pendingAttackBonus:s.pendingAttackBonus+Math.max(1,value),explorationNote:resultMessage})}if(s.screen==='combat'){addLog(set,resultMessage);if(it.tipo==='cura')triggerSupportFx(set,get,'cura-item');else if(it.tipo==='escudo')triggerSupportFx(set,get,'fortificacao');enemyAfterDelay(set,get)} },
  ...
  buyEquipment:(id:string)=>{const s=get(),e=eqById(id),key=`equipment:${id}`,known=s.discoveredCards??[];if(!e||!equipmentClassAllowed(e,s.heroId)||!equipmentLevelAllowed(e,s.xp)||e.raridade==='epico'||e.raridade==='lendario'||s.gold<e.preco||s.equipmentBag.length>=equipmentBagCapacity(s))return;set({gold:s.gold-e.preco,equipmentBag:[...s.equipmentBag,id],discoveredCards:known.includes(key)?known:[...known,key]})},
  ...
  sellEquipment:(id:string)=>{const s=get(),idx=s.equipmentBag.indexOf(id),e=eqById(id);if(idx<0||!e)return;const bag=[...s.equipmentBag];bag.splice(idx,1);const equipmentElements={...s.equipmentElements};delete equipmentElements[id];const equipmentResistances={...s.equipmentResistances};delete equipmentResistances[id];set({equipmentBag:bag,equipmentElements,equipmentResistances,gold:s.gold+Math.max(1,Math.floor(e.preco/2))})},
...
export function defenseValue(s:GameState){const base=stats(s).def;return Math.ceil(base*(1+(s.combatDefensePct??0)))}
...
export function consumableEffectiveValue(it:Consumable,s:GameState){return it.valor+consumableAlquimistaBonus(s)}
...
function resolveExplorationEvent(set:any,get:any,accept:boolean,approach?:'class'){
 ...
 if(event.tipo==='equipamento'){
   ...
   const pool=EQUIPMENT.filter(e=>equipmentLevelAllowed(e,s.xp)&&equipmentClassAllowed(e,s.heroId));const equipment=pickWeightedEquipment(pool)??EQUIPMENT[0];set({...common,equipmentBag:[...s.equipmentBag,equipment.id],eventResult:{message:`O ferreiro entregou: ${equipment.nome}.`,roll,tone:'good'}});return
 }
...
export function applyElementalStatus(status:StatusEffects|undefined,element:Element,attackPower:number,force=false):{status:StatusEffects;appliedKind?:string}{
...
function playerAttack(set:any,get:any,label:string,bonus=0,alreadyAnimating=false,targetMinionId?:string,forceCrit=false,forceStatusElement?:Element){
 ...
 const attackBase=attackValue(s)+bonus+s.firstStrikeBonus+(!target&&s.talents.includes('cacador')&&s.enemy.boss?2:0),defenseBase=target?0:Math.max(0,(s.enemy.dificuldade??1)-2),naturalAttackRoll=forceCrit||forgedCrit?6:Math.floor(Math.random()*6)+1,attackBonus=s.heroRollBonus+(s.classRollBonus??0)-rollPenaltyFrom(s.heroStatus),attackRoll=forceCrit||forgedCrit?6:Math.max(1,Math.min(6,naturalAttackRoll+attackBonus+((hasCraftedEffect(s,'critico')||s.groupCriticalBoost)&&naturalAttackRoll===5?1:0))),defenseRoll=enemyStun.wasStunned?1:Math.max(1,Math.floor(Math.random()*6)+1-(s.enemyFearPenalty??0)-(target?0:rollPenaltyFrom(s.enemyStatus)))
...
function enemyAfterDelay(set:any,get:any){
 const s=get() as GameState
 if(s.summon&&s.summon.hp>0&&s.enemy){
  const summon=s.summon,en=s.enemy
  const attackBase=summon.ataque,defenseBase=Math.max(0,(en.dificuldade??1)-2),attackRoll=Math.floor(Math.random()*6)+1,defenseRoll=Math.floor(Math.random()*6)+1
  ...
  if(resolved.selfDamage>0){
   ...
   set({summon:died?undefined:{...summon,hp},...(died&&summon.tipo==='arcano'?{combatAttackPct:0,combatDefensePct:0}:{})})
   addLog(set,`${summon.nome} ataca, mas erra completamente e sofre ${resolved.selfDamage} de dano com o próprio golpe desajeitado.${died?` ${summon.nome} caiu em combate.`:''}`)
   ...
   set({enemyHp,hp:s.hp+healAmount})
   addLog(set,`${summon.nome} ataca: dado ${attackRoll} (${attackEffect(attackRoll)}) contra defesa ${defenseRoll} (${defenseEffect(defenseRoll)}). Causou ${resolved.damage} de dano a ${en.nome}.${healAmount>0?' O vínculo com a fera recupera 1 de vida.':''}`)
 ...
 if((s.classBuffTurns??0)>0){
  ...
  if(turnsLeft<=0){
   const expired:any={classBuffTurns:0},label=s.heroId==='guerreiro'?'Ímpeto Marcial':s.heroId==='arcanista'?'Ascensão Arcana':s.heroId==='cacador'?'Marca do Predador':undefined
...
function enemyAttack(set:any,get:any){
 ...
 const enemy=s.enemy
 const summon=s.summon&&s.summon.hp>0?s.summon:undefined
 const intercepting=Boolean(summon)&&Math.random()<SUMMON_INTERCEPT_CHANCE[summon!.tipo]
 const attackBase=enemy.ataque,defenseBase=intercepting?summon!.defesa:defenseValue(s)+(s.braced?2:0),naturalAttackRoll=Math.floor(Math.random()*6)+1,attackBonus=s.enemyRollBonus-rollPenaltyFrom(s.enemyStatus),attackRoll=Math.max(1,Math.min(6,naturalAttackRoll+attackBonus-(s.heroId==='druida'&&Math.random()<.25?1:0)-(s.enemyFearPenalty??0))),naturalDefenseRoll=Math.floor(Math.random()*6)+1,defenseRoll=intercepting?Math.max(1,naturalDefenseRoll):Math.max(1,Math.min(6,naturalDefenseRoll+(s.classRollBonus??0)+(hasCraftedEffect(s,'defesa_perfeita')&&naturalDefenseRoll===5?1:0)-rollPenaltyFrom(s.heroStatus)))
 ...
 set({shield,animating:true,animationActor:resolved.selfDamage?'hero':'enemy',lastDamage:resolved.selfDamage||raw,combatRoll,playerTurn:false,enemyRollBonus:0,heroRollBonus:attackRoll===2?1:s.heroRollBonus,heroStatus:statusResult.status,fervor:defenseRoll===6?Math.min(3,(s.fervor??0)+1):(s.fervor??0),...(intercepting?{summon:summonDied?undefined:{...summon!,hp:summonHpAfter},...(summonDied&&summon!.tipo==='arcano'?{combatAttackPct:0,combatDefensePct:0}:{})}:{})})
 setTimeout(()=>{
