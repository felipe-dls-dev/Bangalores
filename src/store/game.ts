...
import { DIFFICULTIES, FORGE_BONUS_LABELS, FORGE_BONUS_MATERIAL, FORGE_GEMS, REGION_MATERIALS, STORY_CHAPTERS, TALENTS, type DifficultyMode, type Element, type ForgeBonus, type ForgeChoice, type ForgeEffect } from '../data/expansion'
...
interface GameState {
 ...
 difficultyMode:DifficultyMode;talents:string[];materials:Record<string,number>;equipmentUpgrades:Record<string,number>;equipmentGems:Record<string,string[]>;forgedGemLocked:Record<string,boolean>;craftedEffects:Record<string,ForgeEffect>;forgeXp?:number;forgeAttempts?:number;forgeSuccesses?:number;forgeResult?:{success:boolean;message:string;id:number};bestiary:Record<string,{encontros:number;vitorias:number}>;discoveredCards:string[];revengeWins:Record<string,number>;dungeonDepth:number;dungeonActive:boolean;storyFlags:string[];storyChapterId:string;storyChoices:Record<string,string>;storyNotice?:string;coopBattlesCompleted:string[];
 ...
 attack:(targetMinionId?:string)=>void; heroSkill:()=>void; itemSkill:()=>void; useConsumable:(id:string)=>void; flee:()=>void; defend:()=>void; useFervor:()=>void;
...
export const useGame = create<GameState>()(persist((set,get)=>({
  ...
  heroSkill:()=>{const s=get();if((s.heroSkillUses??0)>=1||!s.playerTurn||s.animating||!s.enemy)return;if(heroStunned(set,get))return;set({heroSkillUses:(s.heroSkillUses??0)+1});if(s.heroId==='guardiao'){set({guardianTaunt:true});addLog(set,'PROVOCAR: os inimigos passam a priorizar o Guardião.');enemyAfterDelay(set,get);return}if(s.heroId==='guerreiro'){set({combatAttackPct:(s.combatAttackPct??0)+.1,combatDefensePct:(s.combatDefensePct??0)+.1,enemyFearPenalty:(s.enemyFearPenalty??0)+1});addLog(set,'Ímpeto Marcial: Ataque e Defesa base aumentados em 10% (arredondados para cima). Medo: o inimigo sofre -1 em todas as rolagens pelo resto da batalha.');triggerSupportFx(set,get,'fortificacao');enemyAfterDelay(set,get);return}if(s.heroId==='cacadora'){set({extraHeroAttacks:1});addLog(set,'Ataque Duplo: você pode atacar duas vezes neste turno.');return}if(s.heroId==='arcanista'){set({classRollBonus:(s.classRollBonus??0)+2,combatAttackPct:(s.combatAttackPct??0)+.1,combatDefensePct:(s.combatDefensePct??0)+.1});addLog(set,'Ascensão Arcana: +2 nas rolagens e +10% de Ataque e Defesa para o grupo.');triggerSupportFx(set,get,'fortificacao');enemyAfterDelay(set,get);return}if(s.heroId==='druida'){const heal=Math.max(1,Math.ceil(maxHp(s)*.3)),amount=Math.min(heal,maxHp(s)-s.hp);set({hp:s.hp+amount});addLog(set,`Brisa Revigorante: recuperou ${amount} de vida do campeão mais ferido.`);triggerSupportFx(set,get,'cura');enemyAfterDelay(set,get);return}if(s.heroId==='cacador'){set({groupCriticalBoost:true});addLog(set,'Marca do Predador: resultados 5 e 6 passam a causar ataques críticos.');triggerSupportFx(set,get,'fortificacao');enemyAfterDelay(set,get)}},
  ...
  ,craftEquipment:(recipeId?:string,choice?:ForgeChoice)=>{
   ...
   const message=success?`Forja bem-sucedida: ${recipe.nome}${gem?` — ${isAttribute?gem.texto:FORGE_BONUS_LABELS[choice as ForgeBonus]} aplicado ao item`:''}. +${xpGain} XP de Forja.`:`A fabricação de ${recipe.nome} falhou. Metade dos materiais foi perdida, mas você ganhou +${xpGain} XP de Forja.`
...
// congelamento/agarrado penalizam TODAS as rolagens (ataque e defesa) do afetado; atordoamento
// é de uso único — cancela a próxima defesa do afetado OU pula a próxima ação dele, o que
// acontecer primeiro.
export type StatusEffects={bleed?:{turns:number;amount:number};burn?:{turns:number;amount:number};poison?:{turns:number;amount:number};frozen?:number;grabbed?:number;stunned?:boolean}
const ELEMENT_STATUS:Partial<Record<Element,'bleed'|'burn'|'poison'|'frozen'|'grabbed'|'stunned'>>={fisico:'bleed',fogo:'burn',natureza:'poison',gelo:'frozen',sombra:'grabbed',arcano:'stunned'}
export const STATUS_LABELS:Record<string,string>={bleed:'Sangrando',burn:'Pegando fogo',poison:'Envenenado',frozen:'Congelado',grabbed:'Agarrado',stunned:'Atordoado'}
const STATUS_PROC_CHANCE=.3
export function heroWeaponElement(s:GameState):Element{return eqById(s.equipped.mao_direita)?.elemento??'fisico'}
export function heroHasResistance(s:GameState,element:Element){return (Object.values(s.equipped) as (string|undefined)[]).some(id=>eqById(id)?.resistencia===element)}
export function heroResistances(s:GameState):Element[]{return Array.from(new Set((Object.values(s.equipped) as (string|undefined)[]).map(id=>eqById(id)?.resistencia).filter((r):r is Element=>Boolean(r))))}
export function rollPenaltyFrom(status?:StatusEffects){return status?(status.frozen?1:0)+(status.grabbed?1:0):0}
export function applyElementalStatus(status:StatusEffects|undefined,element:Element,attackPower:number):{status:StatusEffects;appliedKind?:string}{
 const kind=ELEMENT_STATUS[element],current=status??{}
 if(!kind||Math.random()>=STATUS_PROC_CHANCE)return{status:current}
...
function applyDefeatPenalty(set:any,get:any,reason='Você foi derrotado.'){
 ...
 const equipmentUpgrades={...s.equipmentUpgrades},equipmentGems={...s.equipmentGems},craftedEffects={...s.craftedEffects},forgedGemLocked={...s.forgedGemLocked}
 ...
 if(lostEntry){
  ...
  for(const lostId of [id,...storedLost])if(!remainingEquipped.has(lostId)){delete equipmentUpgrades[lostId];delete equipmentGems[lostId];delete craftedEffects[lostId];delete forgedGemLocked[lostId]}
...
function playerAttack(set:any,get:any,label:string,bonus=0,alreadyAnimating=false,targetMinionId?:string,forceCrit=false){
 ...
 const statusResult=!target&&damage>0&&!selfDamage?applyElementalStatus(enemyStun.status,heroWeaponElement(s),attackBase):{status:enemyStun.status,appliedKind:undefined as string|undefined}
...
function resolveMinionAttacks(set:any,get:any,onComplete:()=>void){const start=get() as GameState,minions=(start.combatMinions??[]).filter(minion=>minion.hp>0);if(!minions.length){onComplete();return}let index=0;const strike=()=>{const s=get() as GameState,minion=minions[index++];if(!minion||s.screen!=='combat'||s.hp<=0){onComplete();return}const attackRoll=Math.max(1,Math.floor(Math.random()*6)+1-(s.enemyFearPenalty??0)),defenseRoll=Math.floor(Math.random()*6)+1,defenseBase=defenseValue(s)+(s.braced?2:0),resolved=resolveCombatRoll(minion.ataque,defenseBase,attackRoll,defenseRoll),blocked=Math.min(s.shield,resolved.damage),damage=Math.max(0,resolved.damage-blocked),hp=Math.max(0,s.hp-damage);set({hp,shield:s.shield-blocked,animationActor:'enemy',lastDamage:damage,combatRoll:{attacker:'enemy',naturalAttackRoll:attackRoll,attackRoll,attackBonus:0,defenseRoll,attackBase:minion.ataque,defenseBase,attackEffect:attackEffect(attackRoll),defenseEffect:defenseEffect(defenseRoll),damage,selfDamage:0}});addLog(set,`${minion.nome} atacou e causou ${damage} de dano${blocked?` (${blocked} bloqueado)`:''}.`);if(hp<=0){setTimeout(()=>applyDefeatPenalty(set,get,'Você foi derrotado pelos capangas do chefe.'),700);return}if(index<minions.length)setTimeout(strike,500);else setTimeout(onComplete,450)};strike()}
function enemyAttack(set:any,get:any){
 ...
 const enemy=s.enemy
 const attackBase=enemy.ataque,defenseBase=defenseValue(s)+(s.braced?2:0),naturalAttackRoll=Math.floor(Math.random()*6)+1,attackBonus=s.enemyRollBonus-rollPenaltyFrom(s.enemyStatus),attackRoll=Math.max(1,Math.min(6,naturalAttackRoll+attackBonus-(s.heroId==='druida'&&Math.random()<.25?1:0)-(s.enemyFearPenalty??0))),naturalDefenseRoll=Math.floor(Math.random()*6)+1,defenseRoll=Math.max(1,Math.min(6,naturalDefenseRoll+(s.classRollBonus??0)+(hasCraftedEffect(s,'defesa_perfeita')&&naturalDefenseRoll===5?1:0)-rollPenaltyFrom(s.heroStatus)))
 ...
 const enemyElement=enemy.elemento??'fisico',resisted=heroHasResistance(s,enemyElement)
 if(resisted&&raw>0)raw=Math.max(0,raw-1)
 const blocked=Math.min(shield,raw),enemyName=enemy.nome,enemyApproach=enemyApproachPhrase(enemy),heroName=HEROES.find(h=>h.id===s.heroId)?.nome??'Você';raw-=blocked;shield-=blocked
 const statusResult=!dodged&&!resolved.selfDamage&&raw>0&&!resisted?applyElementalStatus(s.heroStatus,enemyElement,attackBase):{status:s.heroStatus??{},appliedKind:undefined as string|undefined}
 ...
 set({shield,animating:true,animationActor:resolved.selfDamage?'hero':'enemy',lastDamage:resolved.selfDamage||raw,combatRoll,playerTurn:false,enemyRollBonus:0,heroRollBonus:attackRoll===2?1:s.heroRollBonus,heroStatus:statusResult.status,fervor:defenseRoll===6?Math.min(3,(s.fervor??0)+1):(s.fervor??0)})
 setTimeout(()=>{
  ...
  addLog(set,dodged?`${enemyName} ${enemyApproach}, mas a Esquiva do Ladino faz o golpe errar completamente.`:`${enemyName} ${enemyApproach}, ${attackTierPhrase(attackRoll)}. ${heroName} ${defenseTierPhrase(defenseRoll)}. Dado ${attackRoll} (${attackEffect(attackRoll)}) contra defesa ${defenseRoll} (${defenseEffect(defenseRoll)}); causou ${raw} de dano${blocked?` (${blocked} bloqueado)`:''}${resisted?' (resistência elemental reduziu o dano)':''}.${attackRoll===2?' Você recebe +1 na próxima rolagem.':''}${statusResult.appliedKind?` Você fica ${STATUS_LABELS[statusResult.appliedKind]}.`:''}`)
