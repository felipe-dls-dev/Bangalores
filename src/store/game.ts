...
// Estado sintético de um herói com o kit inicial (sem pontos, talentos ou progresso): é a fonte da seleção de
// heróis e da galeria, para mostrarem o MESMO cálculo da ficha em vez de números copiados.
export function heroPreviewState(heroId:string,level=1):GameState{
 const st=starter[heroId]??starter.guerreiro,equipped:Partial<Record<Slot,string>>={...st.equipped}
 return{...useGame.getState(),heroId,xp:xpForLevel(level),attr:{...NO_ATTRIBUTES},allocatedAttr:{...NO_ATTRIBUTES},permanentLife:0,talents:[],specializations:{},storyChoices:{},discoveredCards:[],equipped,equipmentUpgrades:{},equipmentGems:{},forgedGemLocked:{},craftedEffects:{},equipmentElements:{},equipmentResistances:{},pendingAttackBonus:0,combatAttackPct:0,combatDefensePct:0,battleStance:'neutra',heroStatus:{},screen:'menu'} as GameState
}
/** Nomes curtos das classes (a Caçadora aparece como "Ladino" em toda a interface). */
export const HERO_CLASS_NAMES:Record<string,string>={guerreiro:'Guerreiro',guardiao:'Guardião',cacadora:'Ladino',arcanista:'Mago',druida:'Druida',cacador:'Caçador',monge:'Monge',sacerdotisa:'Sacerdotisa',conjurador:'Conjurador'}
...
export const TOUR_STEPS:{screen:Screen;title:string;text:string;highlights:string[];tip:string}[] = [
 {screen:'map',title:'Comece pelo Mapa',text:'Explore Havendown e Steelmere por regiões e sub-regiões. Em Havendown você caminha de verdade pelo mapa (WASD, setas ou clique) e passa de uma região pra outra pelas saídas nas bordas, sem precisar voltar ao mapa-mundi.',highlights:['Planícies de Alvora está sempre aberta para viagem rápida','Chefes das sub-regiões e revanche progressiva'],tip:'Comece por Planícies de Alvora: ela nunca se fecha, então você sempre pode voltar por lá.'},
 {screen:'dungeon',title:'Desça às Masmorras',text:'Quando quiser lutar em sequência, escolha uma região como base e desça sala por sala: a cada cinco andares há um chefe, e as recompensas crescem com a profundidade.',highlights:['Chefe a cada cinco salas','Trocar de região encerra a expedição atual'],tip:'Entre numa masmorra do seu nível para ganhar experiência e espólio mais rápido.'},
 {screen:'chronicle',title:'Siga as Crônicas',text:'Aqui ficam a história da campanha, os objetivos de cada ato, as escolhas que mudam o rumo e as missões dos personagens que você conhece pelo caminho.',highlights:['História e missões da campanha','Dificuldade: Aventura, Veterano ou Lendário'],tip:'Quando o objetivo do ato estiver completo, volte aqui para escolher o rumo da história.'},
 {screen:'character',title:'Construa seu herói',text:'Na Ficha você distribui atributos, consulta habilidades e transforma uma das nove classes em uma build própria.',highlights:['12 talentos desbloqueados até o nível 90','Especializações nos níveis 10, 25, 50 e 75'],tip:'Força, Magia, Vigor e Destreza têm efeitos diferentes; leia o resumo antes de investir.'},
 {screen:'equipment',title:'Monte sua build',text:'Equipe até dez espaços, compare atributos e respeite requisitos de nível, classe e afinidade. No fim da tela, o Resumo da build mostra conjuntos e bônus ativos.',highlights:['Conjuntos concedem bônus combinados','Elementos, resistências, pedras e efeitos ativos'],tip:'O maior Poder de ataque nem sempre vence uma boa combinação de conjunto e resistência.'},
 ...
 {screen:'shop',title:'Negocie na Loja',text:'Compre pelo carrinho, filtre o catálogo e venda o que não usa. Cada peça mostra os atributos e se é melhor que a que você usa. Itens épicos e lendários são conquistados em desafios e na Forja.',highlights:['Validação de ouro, nível, classe e espaço','Compra em lote e venda direta'],tip:'Reserve ouro para aprimoramentos e especializações.'},
 ...
 {screen:'guild',title:'Cresça na Guilda',text:'Aceite contratos de caça, chefes e entrega, cumpra os desafios diários e semanais e colete a provisão diária. Os contratos evoluem e concedem ouro, reputação e novos ranks.',highlights:['Ranks de Ferro a Campeão','Desafios e provisão diária'],tip:'Mantenha um contrato ativo enquanto explora para progredir mais rápido.'},
 {screen:'achievements',title:'Veja suas Conquistas',text:'Títulos conquistados, marcos da campanha e o bestiário com as criaturas que você já enfrentou.',highlights:['Títulos para exibir','Marcos do bestiário dão bônus'],tip:'Enfrente a mesma criatura várias vezes para completar os marcos do bestiário.'},
 ...
 {screen:'coop',title:'Aventure-se em grupo',text:'Crie uma sala em tempo real ou entre por código. O grupo confirma o destino e enfrenta o combate em turnos compartilhados.',highlights:['Recompensas por dano e cura realizados','Prontidão, presença e progresso sincronizados'],tip:'O modo online precisa estar configurado para criar salas.'},
...
export function xpForLevel(level:number){let xp=0;for(let l=1;l<Math.floor(level);l++)xp+=costForLevel(l);return xp}
...
// Snapshot dos 6 Records por-instância (forja/gemas/elemento) que precisam viajar junto quando um
// equipamento passa pelo Negociador (coop) -- ver escrowMarketEquipment/refundMarketEquipment/
// completeMarketEquipmentPurchase. Só campos definidos entram no snapshot (fica leve no shared_state).
export type EquipmentForgeSnapshot={upgrade?:number;gems?:string[];gemLocked?:boolean;craftedEffect?:ForgeEffect;element?:Element;resistance?:Element}
...
// Antes, cada classe tinha Defesa própria (2 a 6); agora a Armadura vem SÓ de itens. Para o começo do jogo não ficar mais
// duro, a peça inicial de cada classe dá de 1 a 2 pontos de Armadura extras para aquela classe, o bastante para a mitigação
// do kit ficar a no máximo 1 ponto do valor antigo. Aplicado depois do balanceamento por raridade, então não muda nível,
// preço nem ranking.
export const STARTER_ARMOR_BONUS:Record<string,{item:string;bonus:number}>={guerreiro:{item:'armor_leao_valoria',bonus:2},guardiao:{item:'armor_pesada_khardur',bonus:2},cacadora:{item:'traje_raposa',bonus:1},druida:{item:'druida_peitoral_broto_de_abdendriel',bonus:1},cacador:{item:'cacador_peitoral_vigia_das_cinzas',bonus:1},monge:{item:'traje_andarilhos',bonus:2},sacerdotisa:{item:'manto_ordem_vida',bonus:2},conjurador:{item:'veste_circulo_arcano',bonus:1}}
function applyStarterArmor(items:Equipment[]){
 return items.map(item=>{const owners=Object.entries(STARTER_ARMOR_BONUS).filter(([,entry])=>entry.item===item.id);if(!owners.length)return item
...
export function currentBlessingPrice(s:{equipped:Partial<Record<Slot,string>>}){return protectionBlessingPrice((Object.entries(s.equipped) as [Slot,string|undefined][]).filter(([slot,ref])=>slot!=='bolsa'&&Boolean(ref)).map(([,ref])=>eqById(ref)?.preco??0))}
...
interface Loot { gold:number; xp:number; itemId?:string; equipmentId?:string; missedEquipmentId?:string; title:string; leveledUp?:boolean; newLevel?:number; levelsGained?:number; isNewEquipment?:boolean; isNewItem?:boolean }
interface EventResult { message:string; roll?:number; tone:'good'|'bad'|'neutral' }
interface CombatRoll { attacker:'hero'|'enemy'; naturalAttackRoll:number; attackRoll:number; attackBonus:number; defenseRoll:number; attackBase:number; defenseBase:number; attackEffect:string; defenseEffect:string; damage:number; selfDamage:number; shieldBlocked?:number; dodged?:boolean }
...
// Postura de combate: escolha persistente (dura até trocar de novo, não tem duração em turnos)
// que troca Ataque por Defesa ou vice-versa, sempre como % do total já calculado (equipamentos,
// talentos, especializações etc. inclusos) -- por isso fica isolada de combatAttackPct/
// combatDefensePct, que já são "donos" de vários bônus de classe temporários e são zerados por
// eles ao expirar (ver o bloco de expiração de classBuffTurns).
export type BattleStance='ofensiva'|'defensiva'|'neutra'
export const STANCE_ATTACK_PCT:Record<BattleStance,number>={ofensiva:.2,defensiva:-.2,neutra:0}
export const STANCE_DEFENSE_PCT:Record<BattleStance,number>={ofensiva:-.2,defensiva:.2,neutra:0}
export const STANCE_LABELS:Record<BattleStance,string>={ofensiva:'Ofensiva',defensiva:'Defensiva',neutra:'Neutra'}
...
export interface GuildMission { id:string; nome:string; descricao:string; tipo:'any'|'specific'|'boss'|'delivery'|'material'; quantidade:number; alvo?:string; itemId?:string; materialId?:string; local?:string; destinoId?:string; recompensa:{tipo:'gold'|'equipment';valor:number}; dificuldade:number; rank:GuildRankId; regiaoMinima?:number }
export const GUILD_MISSIONS:GuildMission[]=[
 ...
 // Kip Pé-Ligeiro (batedor medroso da Floresta de Abdendriel, ver src/data/npcs.ts) jura que os
 // lobos da floresta pioraram, mas nunca chegou perto o bastante pra confirmar. Contrato pensado
 // pra ser mais fácil que caca_lobos (2 em vez de 3, dificuldade 1 em vez de 2) -- ele só pediria
 // uma "coragem emprestada", não uma guerra.
 {id:'kip_covarde_coragem',nome:'Coragem Emprestada',descricao:'O batedor Kip jura que os lobos do Bosque de Abdendriel pioraram, mas nunca chegou perto o bastante pra confirmar. Derrote 2 e traga a prova de que ele pode voltar a patrulhar.',tipo:'specific',alvo:'lobo',quantidade:2,local:'Bosque de Abdendriel',destinoId:'lunar_bosque',recompensa:{tipo:'gold',valor:16},dificuldade:1,rank:'ferro'},
 ...
 // Quadro de contratos expandido -- dezenas de caçadas e coletas espalhadas por todas as
 // regiões de Havendown, para dar à Guilda algo relevante para oferecer em qualquer estágio
 // da campanha (cada uma evolui e se repete indefinidamente via evolvedGuildMission, então a
 // lista abaixo cobre variedade, não repetição).
 {id:'hunt_bandidos_estrada',nome:'Estrada Insegura',descricao:'Bandidos voltaram a assaltar as caravanas na Estrada das Planícies. Derrote 4 deles e devolva um pouco de paz aos comerciantes.',tipo:'specific',alvo:'bandido',quantidade:4,local:'Estrada das Planícies de Alvora',destinoId:'campos_estrada',recompensa:{tipo:'gold',valor:16},dificuldade:1,rank:'ferro'},
 {id:'material_fibra_dourada',nome:'Fibra para os Curtumes',descricao:'Precisamos de Fibra Dourada para os curtumes da Guilda. Colete 6 unidades explorando as Planícies de Alvora — ela se solta dos monstros da região.',tipo:'material',materialId:'fibra_dourada',quantidade:6,local:'Estrada das Planícies de Alvora',destinoId:'campos_estrada',recompensa:{tipo:'gold',valor:14},dificuldade:1,rank:'ferro'},
 {id:'caca_pedagio',nome:'O Fim do Pedágio',descricao:'O Mestre do Pedágio virou uma piada cruel nas rotas comerciais — cobra em ouro e sangue. Encerre a extorsão dele na Ponte de Eldrimar.',tipo:'boss',alvo:'mestre do pedágio',quantidade:1,local:'Ponte de Eldrimar',destinoId:'campos_ponte',recompensa:{tipo:'gold',valor:34},dificuldade:2,rank:'bronze'},
 {id:'hunt_guardas_corrompidos',nome:'Guardas Corrompidos',descricao:'Alguns guardas da ponte foram corrompidos e agora atacam quem deveriam proteger. Derrote 4 e devolva a segurança à travessia.',tipo:'specific',alvo:'guarda corrompido',quantidade:4,local:'Ponte de Eldrimar',destinoId:'campos_ponte',recompensa:{tipo:'gold',valor:26},dificuldade:2,rank:'bronze'},
 {id:'material_seiva_lunar',nome:'Seiva para os Curandeiros',descricao:'A Seiva Lunar do Bosque de Abdendriel cura feridas que nenhuma poção alcança. Traga 8 unidades para os curandeiros da Guilda.',tipo:'material',materialId:'seiva_lunar',quantidade:8,local:'Bosque de Abdendriel',destinoId:'lunar_bosque',recompensa:{tipo:'gold',valor:24},dificuldade:2,rank:'bronze'},
 {id:'hunt_corvos_lua',nome:'Corvos da Lua',descricao:'Os Corvos da Lua andam roubando de viajantes — e de nós. Abata 4 e recupere o que puder do que carregam.',tipo:'specific',alvo:'corvo da lua',quantidade:4,local:'Bosque de Abdendriel',destinoId:'lunar_bosque',recompensa:{tipo:'gold',valor:26},dificuldade:2,rank:'bronze'},
 {id:'caca_patrulha_bronze',nome:'Patrulha da Floresta',descricao:'Qualquer monstro que ronde o Bosque de Abdendriel conta. Derrote 6 e mantenha as trilhas seguras para os viajantes.',tipo:'any',quantidade:6,local:'Bosque de Abdendriel',destinoId:'lunar_bosque',recompensa:{tipo:'gold',valor:20},dificuldade:2,rank:'bronze'},
 {id:'caca_rei_goblin',nome:'O Rei da Clareira',descricao:'O Rei Goblin de Abdendriel uniu as tribos da clareira sob o próprio punho. Derrube-o e disperse a horda antes que cresça mais.',tipo:'boss',alvo:'rei goblin',quantidade:1,local:'Clareira dos Goblins',destinoId:'lunar_goblins',recompensa:{tipo:'equipment',valor:1},dificuldade:3,rank:'prata'},
 {id:'hunt_espiritos_runicos',nome:'Ecos do Círculo',descricao:'Os Espíritos Rúnicos do Círculo Monolítico atacam qualquer um que se aproxime das pedras. Acalme 4 deles — de um jeito ou de outro.',tipo:'specific',alvo:'espírito rúnico',quantidade:4,local:'Círculo Monolítico',destinoId:'lunar_monolito',recompensa:{tipo:'gold',valor:36},dificuldade:3,rank:'prata'},
 {id:'caca_guardiao_runico',nome:'O Guardião Desperto',descricao:'O Guardião Rúnico Ancestral desperta sempre que alguém profana o Círculo Monolítico. Derrote-o e as runas voltarão a dormir em paz.',tipo:'boss',alvo:'guardião rúnico ancestral',quantidade:1,local:'Círculo Monolítico',destinoId:'lunar_monolito',recompensa:{tipo:'gold',valor:58},dificuldade:4,rank:'prata'},
 {id:'hunt_aranhas',nome:'Seda Demais',descricao:'O Ninho da Aranha Gigante está transbordando. Elimine 4 aranhas-guardiãs antes que a seda tome conta da trilha inteira.',tipo:'specific',alvo:'aranha',quantidade:4,local:'Ninho da Aranha Gigante',destinoId:'lunar_aranhas',recompensa:{tipo:'gold',valor:38},dificuldade:3,rank:'prata'},
 {id:'caca_rainha_aracnidea',nome:'A Rainha do Ninho',descricao:'A Rainha Aracnídea põe ovos mais rápido do que conseguimos contar. Corte o problema pela raiz — derrote-a no Ninho.',tipo:'boss',alvo:'rainha aracnídea',quantidade:1,local:'Ninho da Aranha Gigante',destinoId:'lunar_aranhas',recompensa:{tipo:'equipment',valor:1},dificuldade:4,rank:'ouro'},
 {id:'material_minerio_cinzento',nome:'Minério para a Forja',descricao:'O ferreiro da Guilda está sem estoque de Minério Cinzento. Traga 10 unidades da Serra de Kaldrum e ele lhe deve um favor.',tipo:'material',materialId:'minerio_cinzento',quantidade:10,local:'Passagem da Serra de Kaldrum',destinoId:'montanhas_passagem',recompensa:{tipo:'gold',valor:32},dificuldade:3,rank:'prata'},
 {id:'hunt_saqueadores_montanha',nome:'Saqueadores da Passagem',descricao:'Saqueadores tomaram a passagem de Kaldrum e cobram "pedágio" a machadadas. Ensine a eles como funciona um contrato de verdade — 5 deles bastam.',tipo:'specific',alvo:'saqueador das montanhas',quantidade:5,local:'Passagem da Serra de Kaldrum',destinoId:'montanhas_passagem',recompensa:{tipo:'gold',valor:34},dificuldade:3,rank:'prata'},
 // Torvald Barbaneve (superintendente hilário das minas, ver src/data/npcs.ts) perde a picareta
 // favorita "toda terça-feira" segundo a própria fala dele -- desta vez ela foi parar com os
 // saqueadores da passagem.
 {id:'torvald_picareta_perdida',nome:'A Picareta Perdida (De Novo)',descricao:'Torvald perdeu a picareta favorita de novo — desta vez ela foi parar com os saqueadores da Passagem de Kaldrum. Derrote 4 e recupere o que sobrar dela.',tipo:'any',quantidade:4,local:'Passagem da Serra de Kaldrum',destinoId:'montanhas_passagem',recompensa:{tipo:'gold',valor:30},dificuldade:3,rank:'prata'},
 {id:'caca_tita_passagem',nome:'O Titã da Passagem',descricao:'O Titã da Passagem bloqueia a única rota segura entre os vales. Derrube-o e reabra o caminho para os mercadores.',tipo:'boss',alvo:'titã da passagem',quantidade:1,local:'Passagem da Serra de Kaldrum',destinoId:'montanhas_passagem',recompensa:{tipo:'gold',valor:62},dificuldade:4,rank:'ouro'},
 {id:'hunt_golens_minerio',nome:'Colapso na Mina',descricao:'Golens de minério andam desmoronando os túneis da mina antiga. Destrua 4 antes que a escavação inteira desabe.',tipo:'specific',alvo:'golem de minério',quantidade:4,local:'Mina dos Anões Caídos',destinoId:'montanhas_mina',recompensa:{tipo:'gold',valor:44},dificuldade:4,rank:'ouro'},
 {id:'caca_ferreiro_caido',nome:'O Ferreiro Caído',descricao:'Dizem que o Mestre Ferreiro Caído ainda forja armas na escuridão da mina — para os inimigos errados. Encerre o trabalho dele.',tipo:'boss',alvo:'mestre ferreiro caído',quantidade:1,local:'Mina dos Anões Caídos',destinoId:'montanhas_mina',recompensa:{tipo:'equipment',valor:1},dificuldade:5,rank:'ouro'},
 {id:'hunt_lobos_gelo',nome:'Alcateia Congelante',descricao:'Uma alcateia de Lobos do Gelo se instalou na Caverna e já atacou duas expedições. Reduza a matilha em 5 e traga a caverna de volta ao silêncio.',tipo:'specific',alvo:'lobo do gelo',quantidade:5,local:'Caverna de Gelo',destinoId:'montanhas_gelo',recompensa:{tipo:'gold',valor:46},dificuldade:4,rank:'ouro'},
 {id:'caca_yeti_alfa',nome:'O Yeti Alfa',descricao:'O Yeti Alfa de Gelo Eterno é maior que qualquer coisa que já subiu essas montanhas. Só ofereço esse contrato a quem já provou o próprio aço.',tipo:'boss',alvo:'yeti alfa',quantidade:1,local:'Caverna de Gelo',destinoId:'montanhas_gelo',recompensa:{tipo:'gold',valor:84},dificuldade:5,rank:'platina'},
 {id:'material_escama_rubra',nome:'Escamas Contra o Fogo',descricao:'Escama Rubra resiste ao fogo como nada que já vi. Colete 8 na Encosta Vulcânica — vamos usá-las para blindar os recrutas.',tipo:'material',materialId:'escama_rubra',quantidade:8,local:'Encosta Vulcânica',destinoId:'pico_encosta',recompensa:{tipo:'gold',valor:48},dificuldade:4,rank:'ouro'},
 {id:'hunt_salamandras',nome:'Fogo na Encosta',descricao:'As Salamandras de Ignaris queimam tudo que tocam nas trilhas da encosta. Abata 5 antes que o fogo se espalhe pela vegetação restante.',tipo:'specific',alvo:'salamandra de ignaris',quantidade:5,local:'Encosta Vulcânica',destinoId:'pico_encosta',recompensa:{tipo:'gold',valor:48},dificuldade:4,rank:'ouro'},
 {id:'caca_guardiao_caldeira',nome:'Guardião da Caldeira',descricao:'O Guardião da Caldeira protege o coração vulcânico do Pico. Derrote-o e teremos acesso seguro ao que resta das minas de escama.',tipo:'boss',alvo:'guardião da caldeira',quantidade:1,local:'Encosta Vulcânica',destinoId:'pico_encosta',recompensa:{tipo:'equipment',valor:1},dificuldade:5,rank:'platina'},
 {id:'hunt_draconatos',nome:'Escamas do Ninho',descricao:'Draconatos escamados patrulham o Ninho do Dragão Vermelho em bandos cada vez maiores. Abata 4 antes que apoiem o próprio Ignaroth.',tipo:'specific',alvo:'draconato',quantidade:4,local:'Ninho do Dragão Vermelho',destinoId:'pico_ninho_dragao',recompensa:{tipo:'gold',valor:78},dificuldade:5,rank:'platina'},
 {id:'material_essencia_sombria',nome:'Essência das Terras Mortas',descricao:'Essência Sombria é rara fora das Terras de Morvath — e perigosa de carregar por muito tempo. Traga 10 unidades e não demore no caminho de volta.',tipo:'material',materialId:'essencia_sombria',quantidade:10,local:'Campos Amaldiçoados',destinoId:'mortas_campos',recompensa:{tipo:'gold',valor:68},dificuldade:5,rank:'platina'},
 {id:'hunt_espectros_errantes',nome:'Névoa da Morte',descricao:'Espectros Errantes vagam pelos Campos Amaldiçoados atacando quem se aproxima demais da névoa. Dissipe 5 deles.',tipo:'specific',alvo:'espectro errante',quantidade:5,local:'Campos Amaldiçoados',destinoId:'mortas_campos',recompensa:{tipo:'gold',valor:66},dificuldade:5,rank:'platina'},
 {id:'caca_lorde_espectral',nome:'O Luto de Morvath',descricao:'O Lorde Espectral de Morvath se recusa a descansar — e leva outros junto com ele. Dê a esse espírito o fim que ele mesmo negou.',tipo:'boss',alvo:'lorde espectral',quantidade:1,local:'Campos Amaldiçoados',destinoId:'mortas_campos',recompensa:{tipo:'gold',valor:96},dificuldade:6,rank:'diamante'},
 {id:'hunt_cavaleiros_mortos',nome:'Ossos Inquietos',descricao:'As Catacumbas de Morvath cospem Cavaleiros Mortos a cada lua cheia. Reduza a horda em 5 antes que alcance a superfície.',tipo:'specific',alvo:'cavaleiro morto',quantidade:5,local:'Catacumbas de Morvath',destinoId:'mortas_catacumbas',recompensa:{tipo:'gold',valor:92},dificuldade:6,rank:'diamante'},
 {id:'caca_necromante_supremo',nome:'O Necromante Supremo',descricao:'O Necromante Supremo comanda tudo que se move nas Catacumbas. Silencie-o e os mortos finalmente descansam.',tipo:'boss',alvo:'necromante supremo',quantidade:1,local:'Catacumbas de Morvath',destinoId:'mortas_catacumbas',recompensa:{tipo:'equipment',valor:1},dificuldade:6,rank:'diamante'},
 {id:'caca_patrulha_diamante',nome:'Sobreviva a Morvath',descricao:'As Terras de Morvath não dão trégua a ninguém. Sobreviva a 14 combates ali e prove que é feito de material mais duro que a maioria.',tipo:'any',quantidade:14,local:'Catacumbas de Morvath',destinoId:'mortas_catacumbas',recompensa:{tipo:'gold',valor:110},dificuldade:6,rank:'diamante'},
 {id:'material_runa_ana',nome:'Runas para a Forja',descricao:'As Runas Anãs de Kholgard alimentam quase todo aprimoramento que fazemos na forja. Traga 8 das Galerias Perdidas.',tipo:'material',materialId:'runa_ana',quantidade:8,local:'Galerias Perdidas de Kholgard',destinoId:'khar_galerias',recompensa:{tipo:'gold',valor:36},dificuldade:3,rank:'prata'},
 {id:'hunt_esqueletos_anoes',nome:'Ecos de Kholgard',descricao:'Esqueletos Anões ainda guardam as Galerias Perdidas como se a fortaleza nunca tivesse caído. Coloque 5 deles para descansar de vez.',tipo:'specific',alvo:'esqueleto anão',quantidade:5,local:'Galerias Perdidas de Kholgard',destinoId:'khar_galerias',recompensa:{tipo:'gold',valor:34},dificuldade:3,rank:'prata'},
 {id:'caca_sentinela_pedra',nome:'A Sentinela de Kholgard',descricao:'A Sentinela de Pedra guarda a entrada mais profunda de Kholgard há séculos. Prove que merece passar.',tipo:'boss',alvo:'sentinela de pedra',quantidade:1,local:'Galerias Perdidas de Kholgard',destinoId:'khar_galerias',recompensa:{tipo:'gold',valor:58},dificuldade:4,rank:'ouro'},
 {id:'hunt_guardioes_labirinticos',nome:'O Labirinto Vigiado',descricao:'O Labirinto de Kholgard está cheio demais de guardiões para qualquer caravana atravessar em segurança. Abra caminho — 5 deles é um bom começo.',tipo:'specific',alvo:'guardião labiríntico',quantidade:5,local:'Labirinto de Kholgard',destinoId:'khar_labirinto',recompensa:{tipo:'gold',valor:46},dificuldade:4,rank:'ouro'},
 {id:'caca_rei_esquecido',nome:'O Rei Esquecido',descricao:'O Rei Esquecido ainda comanda o Labirinto como se seu reino não tivesse caído há mil anos. Lembre-o de que caiu.',tipo:'boss',alvo:'rei esquecido',quantidade:1,local:'Labirinto de Kholgard',destinoId:'khar_labirinto',recompensa:{tipo:'equipment',valor:1},dificuldade:5,rank:'platina'},
 {id:'hunt_cultistas_taurinos',nome:'Culto do Altar Vermelho',descricao:'Cultistas taurinos treinam no Templo do Minotauro para algo que não vai nos agradar. Interrompa os treinos — com força. 4 bastam.',tipo:'specific',alvo:'cultista taurino',quantidade:4,local:'Templo do Minotauro',destinoId:'khar_templo_minotauro',recompensa:{tipo:'gold',valor:68},dificuldade:5,rank:'platina'},
 {id:'caca_minotauro_ancestral',nome:'O Minotauro Ancestral',descricao:'O Minotauro Ancestral é o motivo de o Templo ainda estar de pé. Derrote-o e leve a notícia de volta — vão precisar de provas.',tipo:'boss',alvo:'minotauro ancestral',quantidade:1,local:'Templo do Minotauro',destinoId:'khar_templo_minotauro',recompensa:{tipo:'gold',valor:98},dificuldade:6,rank:'diamante'},
 {id:'material_fragmento_eclipse',nome:'Fragmentos do Sol Negro',descricao:'Fragmentos do Sol Negro só existem depois do Véu partido — e cada um custa caro pra carregar. Traga 10, se sobreviver ao caminho.',tipo:'material',materialId:'fragmento_eclipse',quantidade:10,local:'Portões do Sol Negro',destinoId:'eclipse_portoes',recompensa:{tipo:'gold',valor:130},dificuldade:6,rank:'diamante'},
 {id:'hunt_cavaleiros_sol_negro',nome:'Cavaleiros Sem Rosto',descricao:'Os Portões do Sol Negro são vigiados por cavaleiros que já não lembram por quem lutam. Abata 4 e abra caminho para o resto de nós.',tipo:'specific',alvo:'cavaleiro do sol negro',quantidade:4,local:'Portões do Sol Negro',destinoId:'eclipse_portoes',recompensa:{tipo:'gold',valor:118},dificuldade:6,rank:'diamante'},
 {id:'caca_asterion',nome:'O Último Guardião',descricao:'Asterion jurou proteger o Sol Negro até o fim dos tempos. Mostre a ele que os tempos já acabaram.',tipo:'boss',alvo:'asterion',quantidade:1,local:'Portões do Sol Negro',destinoId:'eclipse_portoes',recompensa:{tipo:'equipment',valor:1},dificuldade:7,rank:'diamante'},
 {id:'hunt_espectros_tempo',nome:'Ecos do Véu Partido',descricao:'A Torre do Véu Partido distorce o tempo e cospe ecos de espectros a cada volta. Encerre 4 desses ecos antes que percam a conta de quantos já são.',tipo:'specific',alvo:'espectro do tempo partido',quantidade:4,local:'Torre do Véu Partido',destinoId:'eclipse_torre',recompensa:{tipo:'gold',valor:150},dificuldade:7,rank:'campeao'},
 {id:'caca_vaelora',nome:'A Senhora do Véu',descricao:'Vaelora, Senhora do Véu, dobra o tempo em torno de si mesma para nunca perder uma luta. Seja a exceção à regra dela.',tipo:'boss',alvo:'vaelora',quantidade:1,local:'Torre do Véu Partido',destinoId:'eclipse_torre',recompensa:{tipo:'gold',valor:210},dificuldade:8,rank:'campeao'},
 {id:'caca_nihraz',nome:'O Imperador do Vazio',descricao:'Nihraz, Imperador do Vazio, é o motivo de existir um Sol Negro para começo de conversa. Se você chegou até aqui, Havendown inteira está torcendo por você. Encerre isso — de uma vez por todas.',tipo:'boss',alvo:'nihraz',quantidade:1,local:'Trono do Vazio',destinoId:'eclipse_trono',recompensa:{tipo:'gold',valor:320},dificuldade:9,rank:'campeao'}
]
...
export const HERO_ULTIMATES: Record<string, { nome: string; descricao: string }> = {
  ...
  monge: { nome: 'Mil Golpes Celestiais', descricao: 'Tempestade de palmas de chi com 250% do dano base (+16% do seu ataque e +2 de Energia).' },
...
interface GameState {
 screen:Screen; heroId?:string; hp:number; gold:number; xp:number; attributePoints:number; attr:PrimaryAttributes; allocatedAttr:PrimaryAttributes; permanentLife:number; balanceVersion:number; energy:number; lifeWardPct?:number;
 inventory:Record<string,number>; equipmentBag:string[]; protectionBlessings:number; crystals:number; guildVault:string[]; guildVaultUpgrades:number; equipped:Partial<Record<Slot,string>>; territory:string; regionId:string; world:string; subregionId?:string; victories:Record<string,number>; subregionVictories:Record<string,number>; bossesDefeated:string[]; subregionBossesDefeated:string[]; regionMapPositions?:Record<string,{x:number;y:number}>; exploredMapTiles?:Record<string,string[]>;
 // Pins que o próprio jogador planta no mapa navegável (lembrete pessoal, tipo "voltar aqui depois")
 // -- por regionId, sem significado de jogo nenhum, só o que o jogador quiser marcar.
 customPins?:Record<string,Array<{x:number;y:number}>>;
 // Alavancas já ativadas (ART-023) -- por leverId, nunca desliga sozinha (igual baú aberto).
 activatedLevers?:Record<string,boolean>;
 // Monólitos descobertos (ART-024), pra viagem rápida entre eles -- ids globais (ver ALL_MONOLITHS).
 discoveredMonoliths?:string[];
 // Paredes ilusórias já reveladas (ART-026) -- chave `${mapId}:x,y`, nunca esquece (igual baú aberto).
 discoveredSecrets?:Record<string,boolean>;
 enemy?:Enemy; enemyHp:number; enemyIntent?:EnemyIntent; combatMinions?:CombatMinion[]; combatTurn:number; itemUsedTurn?:number; combatLog:string[]; coin?:'cara'|'coroa'; playerTurn:boolean; animating:boolean; animationActor?:'hero'|'enemy'; lastDamage?:number; combatRoll?:CombatRoll; fleeRoll?:FleeRoll; heroRollBonus:number; enemyRollBonus:number; enemyFearPenalty:number; heroSkillUses:number; itemSkillUsed:boolean; shield:number; combatAttackPct:number; combatDefensePct:number; classRollBonus:number; classBuffTurns:number; summon?:Summon; summonAttackFx?:{types:AttackAnimType[];nonce:number}; lifeWardActive:boolean; phoenixUsed:boolean; extraHeroAttacks:number; guardianTaunt:boolean; groupCriticalBoost:boolean; battleStance:BattleStance; stanceChangeUsed:boolean; firstStrikeBonus:number; heroStatus?:StatusEffects; enemyStatus?:StatusEffects; supportFx?:{type:'fortificacao'|'cura'|'cura-item'};
 summons?:Summon[];
 ...
 buyConsumable:(id:string)=>void; buyEquipment:(id:string)=>void; sellConsumable:(id:string)=>void; sellEquipment:(id:string)=>void; buyProtectionBlessing:()=>void; buyVaultUpgrade:()=>void; depositToVault:(ref:string)=>void; withdrawFromVault:(ref:string)=>void;
 ...
 // mapa (deriveWanderers filtra por aqui) e não reaparece, igual a um baú aberto. Vale pra
 // qualquer vitória naquele subId (emboscada ao esbarrar nele OU explorar o local pelo botão),
 // não só a colisão em si: uma vez limpa a ameaça daquele ponto, o bicho visível não volta.
 ...
 campfireHealTick:()=>{hp:boolean;energy:boolean}; addEnergy:(amount:number)=>void;
...
// Aprimorar deixou de ser garantido: cada nível-alvo tem sua própria chance de sucesso (+1 é
// bem provável, +2 já é raro, +3 é uma aposta de fim de jogo) e também custa materiais, não só
// ouro. Falhar em +2/+3 arrisca REGREDIR o nível atual (não só "não avançar") -- risco cresce
// junto com a chance de sucesso cair, reforçando que aprimorar até +3 é caro E arriscado.
export const UPGRADE_SUCCESS_CHANCE:Record<1|2|3,number>={1:.85,2:.55,3:.3}
export const UPGRADE_REGRESS_CHANCE:Record<1|2|3,number>={1:0,2:.25,3:.4}
export function equipmentUpgradeMaterialCost(e:Equipment,targetLevel:1|2|3):Record<string,number>{
 const tier=RARITY_TIER[e.raridade??'comum'],cost:Record<string,number>={fragmento_fisico:2+tier+targetLevel*2}
 if(targetLevel>=2)cost.essencia_magica=targetLevel+tier
 return cost
}
...
// Item 46 do Quadro de Contratos: conquistas sem nenhum estado novo pra persistir -- cada
// "check" é recalculado a partir de dados que já existem e só CRESCEM durante a campanha
// (subregionBossesDefeated, discoveredCards, bestiary, forgeXp, guildClaimed, subregionVictories,
// highestDamageDealt, revengeWins), então uma conquista, uma vez verdadeira, nunca "desconquista"
// sozinha -- não precisa de uma lista separada de "já desbloqueadas" nem de migração de save.
// A recompensa é cosmética: um título que o jogador pode exibir (selectedTitle), sem efeito
// nenhum em combate.
export interface Achievement{id:string;nome:string;descricao:string;titulo:string;check:(s:GameState)=>boolean}
export const ACHIEVEMENTS:Achievement[]=[
 {id:'primeiro_chefe',nome:'Primeiro Sangue',descricao:'Derrote seu primeiro chefe de sub-região.',titulo:'Caçador de Chefes',check:s=>s.subregionBossesDefeated.length>=1},
 {id:'dez_chefes',nome:'Flagelo Regional',descricao:'Derrote 10 chefes de sub-região.',titulo:'Flagelo Regional',check:s=>s.subregionBossesDefeated.length>=10},
 {id:'colecionador_25',nome:'Colecionador',descricao:'Descubra 25 cartas na Coleção.',titulo:'Colecionador',check:s=>(s.discoveredCards?.length??0)>=25},
 {id:'colecionador_100',nome:'Mestre da Coleção',descricao:'Descubra 100 cartas na Coleção.',titulo:'Mestre da Coleção',check:s=>(s.discoveredCards?.length??0)>=100},
 {id:'colecionador_250',nome:'Lenda Viva',descricao:'Descubra 250 cartas na Coleção.',titulo:'Lenda Viva',check:s=>(s.discoveredCards?.length??0)>=250},
 {id:'forjador_ativo',nome:'Aprendiz de Forja',descricao:'Realize sua primeira tentativa na Forja (fabricação ou aprimoramento).',titulo:'Aprendiz de Forja',check:s=>(s.forgeAttempts??0)>=1},
 {id:'forjador_mestre',nome:'Mestre Forjador',descricao:'Alcance a maestria máxima de Forjador.',titulo:'Mestre Forjador',check:s=>forgeLevelInfo(s.forgeXp??0).max},
 {id:'naturalista',nome:'Naturalista',descricao:'Vença a mesma criatura 5 vezes em pelo menos 3 espécies diferentes.',titulo:'Naturalista',check:s=>Object.values(s.bestiary).filter(b=>b.vitorias>=5).length>=3},
 {id:'guilda_veterana',nome:'Herói da Guilda',descricao:'Complete 10 contratos da Guilda.',titulo:'Herói da Guilda',check:s=>s.guildClaimed.length>=10},
 {id:'explorador',nome:'Explorador Incansável',descricao:'Vença 100 encontros de exploração no total.',titulo:'Explorador Incansável',check:s=>Object.values(s.subregionVictories).reduce((a,b)=>a+b,0)>=100},
 {id:'golpe_lendario',nome:'Golpe Lendário',descricao:'Cause 50 ou mais de dano em um único golpe.',titulo:'Braço Lendário',check:s=>(s.highestDamageDealt??0)>=50},
...
// Migração de saves para balanceVersion 3 (Força/Magia/Vigor/Destreza).
//  v1 -> v2: reconstrói o que foi GASTO em pontos (regra antiga, mantida).
//  v2 -> v3: ataque -> atributo ofensivo da classe (Força, ou Magia para classes mágicas); vida e defesa
//  alocadas -> Vigor (Armadura é exclusiva de equipamentos, então a antiga Defesa vira Vigor); a Vida
//  permanente que não veio de pontos (poções de vida) vira `permanentLife`, que preserva a Vida exata;
//  a Vida atual é mantida na MESMA proporção da Vida Máxima. Saves incompletos ou corrompidos viram zeros,
//  nunca NaN.
function legacyCount(value:unknown){const n=Number(value);return Number.isFinite(n)&&n>0?Math.floor(n):0}
export function normalizeAttributes(source:any){
 if(source?.balanceVersion>=3&&source?.allocatedAttr&&source?.attr){return{attr:sanitizeAttributes(source.attr),allocatedAttr:sanitizeAttributes(source.allocatedAttr),permanentLife:legacyCount(source.permanentLife),balanceVersion:3}}
 let legacyAttr={vida:legacyCount(source?.attr?.vida),ataque:legacyCount(source?.attr?.ataque),defesa:legacyCount(source?.attr?.defesa)},legacyAlloc:{vida:number;ataque:number;defesa:number}
 if(source?.balanceVersion>=2&&source?.allocatedAttr){legacyAlloc={vida:legacyCount(source.allocatedAttr.vida),ataque:legacyCount(source.allocatedAttr.ataque),defesa:legacyCount(source.allocatedAttr.defesa)}}
 else{const earned=Math.max(0,deriveLevel(legacyCount(source?.xp)).lvl-1),spent=Math.max(0,earned-legacyCount(source?.attributePoints)),defesa=Math.min(legacyAttr.defesa,spent),ataque=Math.min(legacyAttr.ataque,Math.max(0,spent-defesa)),vida=Math.min(legacyAttr.vida,Math.max(0,spent-defesa-ataque));legacyAttr={...legacyAttr,ataque};legacyAlloc={vida,ataque,defesa}}
 const profile=heroStatProfile(source?.heroId),offense=primaryOffense(profile)
 const allocatedAttr:PrimaryAttributes={...NO_ATTRIBUTES,vigor:legacyAlloc.vida+legacyAlloc.defesa};allocatedAttr[offense]+=legacyAlloc.ataque
 const attr:PrimaryAttributes={...allocatedAttr};attr[offense]+=Math.max(0,legacyAttr.ataque-legacyAlloc.ataque);attr.vigor+=Math.max(0,legacyAttr.defesa-legacyAlloc.defesa)
 const permanentLife=Math.max(0,legacyAttr.vida-legacyAlloc.vida)
 const migrated:any={attr,allocatedAttr,permanentLife,balanceVersion:3}
 if(source?.heroId&&Number.isFinite(Number(source?.hp))){
  // Um save incompleto (sem equipamentos, talentos...) não pode derrubar o carregamento: os campos ausentes valem vazio.
  try{
   const safe={...source,...migrated,equipped:source.equipped??{},talents:source.talents??[],specializations:source.specializations??{},storyChoices:source.storyChoices??{},equipmentUpgrades:source.equipmentUpgrades??{},equipmentGems:source.equipmentGems??{}}
   const legacyMax=Math.max(1,(HEROES.find(h=>h.id===source.heroId)?.vida??profile.vidaBase)+legacyAttr.vida+attributeSources(safe).bonus.vidaMaxima!),newMax=maxHp(safe),hp=Math.max(0,Number(source.hp))
   migrated.hp=hp<=0?0:Math.max(1,Math.min(newMax,Math.round(Math.min(1,hp/legacyMax)*newMax)))
  }catch{migrated.hp=Math.max(0,Math.floor(Number(source.hp)))}
 }
 return migrated
}
...
export const useGame = create<GameState>()(persist((set,get)=>({
  ...
  completeCoopFlee:(battleId:string)=>{const s=get(),completedBattles=s.coopBattlesCompleted??[];if(!battleId||completedBattles.includes(battleId))return;set({coopBattlesCompleted:[...completedBattles,battleId],screen:s.subregionId?'region':'map',enemy:undefined,enemyHp:0,combatMinions:[],combatLog:[],playerTurn:false,animating:false,animationActor:undefined,lastDamage:undefined,combatRoll:undefined,fleeRoll:undefined,shield:0,battleStance:'neutra',stanceChangeUsed:false,firstStrikeBonus:0,classBuffTurns:0,summon:undefined,lifeWardActive:false,phoenixUsed:false,groupCriticalBoost:false,heroStatus:{},enemyStatus:{},pendingAttackBonus:0,heroRollBonus:0,enemyRollBonus:0,enemyFearPenalty:0});const completed=get();set({campaigns:saveActiveCampaign(completed)})},
  // A barra de Golpe Supremo (ultimateGauge) nunca carregava no coop -- attack()/enemyAttack()
  // (modo solo) são os dois únicos lugares que somam gainedGauge/enemyGaugeGained no jogo
  // inteiro; os handlers coop nunca replicavam essa soma, então ficava travada em 0% pra sempre,
  // não importa quanto dano você desse ou tomasse. Mesma fórmula do solo, replicada aqui.
  receiveCoopEnemyAttack:(damage:number,roll:any)=>{const s=get();if(s.screen!=='combat'||!s.enemy)return;const blocked=Math.max(0,Number(roll?.shieldBlocked??0)),enemyGaugeGained=damage>0?14:(blocked>0?8:4),ultimateGauge=Math.min(100,(s.ultimateGauge??0)+enemyGaugeGained);set({hp:Math.max(0,s.hp-damage),shield:Math.max(0,s.shield-blocked),ultimateGauge,combatRoll:{attacker:'enemy',naturalAttackRoll:roll.attackRoll,attackRoll:roll.attackRoll,attackBonus:0,defenseRoll:roll.defenseRoll,attackBase:s.enemy.ataque,defenseBase:defenseValue(s),attackEffect:attackEffect(roll.attackRoll),defenseEffect:defenseEffect(roll.defenseRoll),damage,selfDamage:0,shieldBlocked:blocked||undefined,dodged:Boolean(roll?.dodged)},animating:true,animationActor:'enemy',lastDamage:damage});setTimeout(()=>set({animating:false,animationActor:undefined,lastDamage:undefined,combatRoll:undefined}),COMBAT_ROLL_DISPLAY_MS)},
  ...
  continueGame:()=>{const state=get(),saved=state.activeCampaignId&&state.campaigns[state.activeCampaignId];if(saved){const migrated=migrateEquipmentInstances(saved);set({...migrated,...normalizeAttributes(migrated),campaigns:state.campaigns,activeCampaignId:state.activeCampaignId,guildAccepted:migrated.guildAccepted??[],guildProgress:migrated.guildProgress??{},guildClaimed:migrated.guildClaimed??[],protectionBlessings:migrated.protectionBlessings??0,crystals:migrated.crystals??0,guildVault:migrated.guildVault??[],guildVaultUpgrades:migrated.guildVaultUpgrades??0,guildNotice:undefined,screen:resumableScreen(migrated.screen),animating:false,animationActor:undefined,lastDamage:undefined,combatRoll:undefined,fleeRoll:undefined,playerTurn:migrated.screen==='combat'&&migrated.enemy?true:(migrated.playerTurn??false)})}else set({screen:state.heroId?'map':'select'})},
  loadCampaign:(id:string)=>{const state=get(),saved=state.campaigns[id];if(!saved)return;const migrated=migrateEquipmentInstances(saved),campaigns=saveActiveCampaign(state);set({...migrated,...normalizeAttributes(migrated),campaigns,activeCampaignId:id,guildAccepted:migrated.guildAccepted??[],guildProgress:migrated.guildProgress??{},guildClaimed:migrated.guildClaimed??[],protectionBlessings:migrated.protectionBlessings??0,crystals:migrated.crystals??0,guildVault:migrated.guildVault??[],guildVaultUpgrades:migrated.guildVaultUpgrades??0,guildNotice:undefined,screen:resumableScreen(migrated.screen),animating:false,animationActor:undefined,lastDamage:undefined,combatRoll:undefined,fleeRoll:undefined,playerTurn:migrated.screen==='combat'&&migrated.enemy?true:(migrated.playerTurn??false)})},
  ...
  claimGuildMission:(id:string)=>{const state=get(),mission=guildMissionById(id);if(!mission||state.guildClaimed.includes(id))return;const deliveryIndex=mission.tipo==='delivery'&&mission.itemId?state.equipmentBag.findIndex((ref:string)=>equipmentRefMatches(ref,mission.itemId)):-1;const deliverySlot=mission.tipo==='delivery'&&mission.itemId&&deliveryIndex<0?(Object.entries(state.equipped).find(([,ref])=>equipmentRefMatches(ref as string|undefined,mission.itemId))?.[0] as Slot|undefined):undefined;const complete=mission.tipo==='delivery'?(deliveryIndex>=0||Boolean(deliverySlot)):mission.tipo==='material'?(state.materials[mission.materialId!]??0)>=mission.quantidade:(state.guildProgress[id]??0)>=mission.quantidade;if(!complete)return;const reputation=reputationFromClaims(state.guildClaimed),newRank=guildRankFor(reputation+mission.dificuldade),promotion=newRank.id!==guildRankFor(reputation).id?` Promoção alcançada: rank ${newRank.nome}!`:'';const consumeDelivery=(bag:string[])=>{if(deliveryIndex>=0)bag.splice(deliveryIndex,1)};const equippedAfterDelivery=()=>{if(!deliverySlot)return state.equipped;const equipped={...state.equipped};delete equipped[deliverySlot];return equipped};const materialsAfterTurnIn=()=>{if(mission.tipo!=='material'||!mission.materialId)return state.materials;return {...state.materials,[mission.materialId]:(state.materials[mission.materialId]??0)-mission.quantidade}};const turnInLabel=mission.tipo==='delivery'?'Item entregue. ':mission.tipo==='material'?'Materiais entregues. ':''
   ...
   // Contrato 47: entregar qualquer missão da Guilda também conta pro desafio semanal correspondente.
   const challengeProgress=bumpChallengeProgress(state.challengeProgress,['guild_delivery'])
   if(mission.recompensa.tipo==='gold'){const equipmentBag=[...state.equipmentBag];consumeDelivery(equipmentBag);set({gold:state.gold+mission.recompensa.valor,xp,attributePoints,equipmentBag,equipped:equippedAfterDelivery(),materials:materialsAfterTurnIn(),challengeProgress,guildClaimed:[...state.guildClaimed,id],guildNotice:`${turnInLabel}Recompensa recebida: ${mission.recompensa.valor} moedas de ouro.${xpNotice}${promotion}`})}else{if(state.equipmentBag.length>=equipmentBagCapacity(state))return;const pool=EQUIPMENT.filter(e=>equipmentLevelAllowed(e,state.xp)&&equipmentClassAllowed(e,state.heroId)&&e.slot!=='bolsa');const reward=pickWeightedEquipment(pool)??EQUIPMENT[0];const equipmentBag=[...state.equipmentBag];consumeDelivery(equipmentBag);equipmentBag.push(createEquipmentInstance(reward.id));set({equipmentBag,xp,attributePoints,equipped:equippedAfterDelivery(),materials:materialsAfterTurnIn(),challengeProgress,guildClaimed:[...state.guildClaimed,id],guildNotice:`${turnInLabel}Recompensa recebida: ${reward.nome}.${xpNotice}${promotion}`})}},
  ...
  // Posição do herói no mapa navegável de cada região, uma por regionId -- sem isso, qualquer
  // saída da tela (combate, emboscada, etc.) fazia o TileWorldExplorer desmontar e, ao voltar,
  // reiniciar sempre no spawn/marcador da sub-região em vez de onde o jogador realmente estava.
  setRegionMapPosition:(regionId:string,pos:{x:number;y:number})=>{const s=get();set({regionMapPositions:{...s.regionMapPositions,[regionId]:pos}})},
  // Névoa de guerra: cada território guarda a lista de tiles "x,y" já vistos. Chamado com os
  // tiles recém-revelados (raio ao redor do herói) a cada passo -- mescla no set existente em
  // vez de substituir, então nunca "esquece" uma área já explorada.
  revealMapTiles:(regionId:string,tiles:Array<{x:number;y:number}>)=>{
    const s=get(),known=new Set(s.exploredMapTiles?.[regionId]??[])
    let changed=false
    for(const t of tiles){const key=`${t.x},${t.y}`;if(!known.has(key)){known.add(key);changed=true}}
    if(!changed)return
    set({exploredMapTiles:{...s.exploredMapTiles,[regionId]:Array.from(known)}})
  },
  // Pin customizado: clicar num tile já marcado remove; clicar num tile livre marca. Puramente
  // um lembrete visual do jogador, sem qualquer leitura de jogo.
  toggleCustomPin:(regionId:string,x:number,y:number)=>{
    const s=get(),existing:Array<{x:number;y:number}>=s.customPins?.[regionId]??[]
    const already=existing.some(p=>p.x===x&&p.y===y)
    const next=already?existing.filter(p=>!(p.x===x&&p.y===y)):[...existing,{x,y}]
    set({customPins:{...s.customPins,[regionId]:next}})
  },
  // Alavanca (ART-023): ativação é permanente -- não existe ação pra desligar de novo.
  activateLever:(leverId:string)=>{const s=get();if(s.activatedLevers?.[leverId])return;set({activatedLevers:{...s.activatedLevers,[leverId]:true}})},
  // Monólito (ART-024): descobrir é só registrar o id; viajar cruza ALL_MONOLITHS (derivado de
  // todos os RegionMapDef em regionMap.tsx) com a lista de descobertos, então só funciona pra
  // monólitos que o jogador já visitou -- reaproveita regionMapPositions pra já chegar na região
  // de destino na posição exata do monólito, igual quem volta de um combate.
  discoverMonolith:(monolithId:string)=>{const s=get();if((s.discoveredMonoliths??[]).includes(monolithId))return;set({discoveredMonoliths:[...(s.discoveredMonoliths??[]),monolithId]})},
  travelToMonolith:(monolithId:string)=>{
    const s=get()
    if(!(s.discoveredMonoliths??[]).includes(monolithId))return
    const mono=ALL_MONOLITHS.find(m=>m.id===monolithId)
    if(!mono)return
    const region=TERRITORIES.find(t=>t.id===mono.regionId)
    if(!region)return
    set({world:region.mundo??'havendown',regionId:region.id,territory:region.nome,subregionId:undefined,screen:'region',regionMapPositions:{...s.regionMapPositions,[region.id]:{x:mono.x,y:mono.y}}})
  },
  // Parede ilusória (ART-026): só guarda "já vi o efeito aqui", pra não tocar de novo toda vez que
  // o jogador passar pelo mesmo tile depois de descoberto.
  discoverSecret:(key:string)=>{const s=get();if(s.discoveredSecrets?.[key])return;set({discoveredSecrets:{...s.discoveredSecrets,[key]:true}})},
  ...
  setBattleStance:(stance:BattleStance)=>{const s=get();if(!s.playerTurn||s.animating||!s.enemy)return
   if(heroStunned(set,get))return
   if(stance===s.battleStance)return
   const desc=stance==='ofensiva'?`+${STANCE_ATTACK_PCT.ofensiva*100}% de Poder de ataque e ${STANCE_DEFENSE_PCT.ofensiva*100}% de Armadura`:stance==='defensiva'?`+${STANCE_DEFENSE_PCT.defensiva*100}% de Armadura e ${STANCE_ATTACK_PCT.defensiva*100}% de Poder de ataque`:'sem bônus ou penalidade de Poder de ataque/Armadura'
  ...
  useFervor:()=>{const s=get(),cost=fervorEnergyCost();if(!s.playerTurn||s.animating||!s.enemy)return;if(energyNow(s)<cost){addLog(set,`Energia insuficiente para o Fervor de Combate: ${energyNow(s)}/${cost}.`);return}playerAttack(set,get,'Fervor de Combate',0,false,undefined,true)},
   heroSkill:()=>{const s=get();if((s.heroSkillCooldown??0)>0||!s.playerTurn||s.animating||!s.enemy)return;if(s.heroId==='conjurador')return;const skillCost=heroSkillEnergyCost(s.heroId);if(energyNow(s)<skillCost){addLog(set,`Energia insuficiente para ${heroStatProfile(s.heroId).habilidade.nome}: ${energyNow(s)}/${skillCost}.`);return}if(heroStunned(set,get))return;spendSkillEnergy(set,get,skillCost);const potency=heroAbilityPotency(s),pct=(base:number)=>Math.round(base*potency*1000)/1000;set({heroSkillUses:(s.heroSkillUses??0)+1,heroSkillCooldown:3});if(s.heroId==='guardiao'){set({guardianTaunt:true,combatDefensePct:pct(.15),classBuffTurns:3});addLog(set,`PROVOCAR: +${Math.round(pct(.15)*100)}% de Armadura por até 3 turnos (e, em combate cooperativo, os inimigos passam a priorizar o Guardião).`);triggerSupportFx(set,get,'fortificacao');enemyAfterDelay(set,get);return}if(s.heroId==='guerreiro'){const refreshedBleed=s.enemyStatus?.bleed?{...s.enemyStatus,bleed:{...s.enemyStatus.bleed,turns:3}}:s.enemyStatus;set({combatAttackPct:pct(.1),combatDefensePct:pct(.1),enemyFearPenalty:1,classBuffTurns:3,enemyStatus:refreshedBleed});addLog(set,`Ímpeto Marcial: Poder de ataque e Armadura aumentados em ${Math.round(pct(.1)*100)}% e Medo no inimigo (-1 em todas as rolagens dele), por até 3 turnos.${s.enemyStatus?.bleed?' O sangramento do inimigo foi renovado para 3 turnos.':''}`);triggerSupportFx(set,get,'fortificacao');enemyAfterDelay(set,get);return}if(s.heroId==='cacadora'){set({extraHeroAttacks:1,classBuffTurns:3});addLog(set,'Ataque Duplo: você pode atacar duas vezes por turno, por até 3 turnos.');if(s.autoCombat)setTimeout(()=>runAutoCombatTurn(set,get),getCombatDelay(get(),700));return}if(s.heroId==='arcanista'){set({classRollBonus:1,combatAttackPct:pct(.1),combatDefensePct:pct(.1),classBuffTurns:3});addLog(set,`Ascensão Arcana: +1 nas rolagens e +${Math.round(pct(.1)*100)}% de Poder de ataque e Armadura para o grupo, por até 3 turnos.`);triggerSupportFx(set,get,'fortificacao');enemyAfterDelay(set,get);return}if(s.heroId==='druida'){const heal=Math.max(1,Math.ceil(maxHp(s)*pct(.3))),amount=Math.min(heal,maxHp(s)-s.hp),hadStatus=Boolean(s.heroStatus&&Object.keys(s.heroStatus).length);set({hp:s.hp+amount,heroStatus:{}});addLog(set,`Brisa Revigorante: recuperou ${amount} de vida do campeão mais ferido${hadStatus?' e purificou seus efeitos negativos':''}.`);triggerSupportFx(set,get,'cura');enemyAfterDelay(set,get);return}if(s.heroId==='cacador'){set({groupCriticalBoost:true,classBuffTurns:2});addLog(set,'Marca do Predador: resultados 5 e 6 passam a causar ataques críticos para o grupo, por 2 turnos.');triggerSupportFx(set,get,'fortificacao');enemyAfterDelay(set,get);return}if(s.heroId==='monge'){playerAttack(set,get,'Golpe Flamejante',Math.max(1,Math.round(2*potency)),true,undefined,false,'fogo');return}if(s.heroId==='sacerdotisa'){set({lifeWardActive:true,lifeWardPct:Math.min(.9,pct(.3))});addLog(set,`Bênção da Vida: se você for derrotada nesta batalha, sobreviverá uma vez com ${Math.round(Math.min(.9,pct(.3))*100)}% da vida máxima.`);triggerSupportFx(set,get,'fortificacao');enemyAfterDelay(set,get)}},
  summonMonster:(tipo:SummonType)=>{const s=get(),existing=(s.heroSkillUses??0)>0?(s.summons??(s.summon?[s.summon]:[])):[];if(s.heroId!=='conjurador'||existing.length>=2||(s.heroSkillUses??0)>=2||!s.playerTurn||s.animating||!s.enemy)return;const skillCost=heroSkillEnergyCost(s.heroId);if(energyNow(s)<skillCost){addLog(set,`Energia insuficiente para conjurar: ${energyNow(s)}/${skillCost}.`);return}if(heroStunned(set,get))return;spendSkillEnergy(set,get,skillCost);const level=deriveLevel(s.xp).lvl,summon=scaleSummon(buildSummon(tipo,level),heroAbilityPotency(s)),typeLabel=tipo==='atacante'?'ofensiva':tipo==='defensor'?'defensiva':'arcana',summons=[...existing,summon]
   ...
   addLog(set,`Conjurar Fera Espectral (${typeLabel}) • ${summons.length}/2: ${summon.nome} surge ao seu lado com ${summon.maxHp} de vida, ${summon.ataque} de ataque e ${summon.defesa} de defesa.${tipo==='arcano'?' Enquanto estiver viva, concede +10% de Poder de ataque e Armadura a você.':tipo==='defensor'?' Tem alta chance de interceptar ataques inimigos no seu lugar.':' Ataca ao seu lado a cada turno.'}`)
  ...
  useConsumable:(id:string)=>{
   ...
   // Ação rápida (auditoria v0.8.84, GAME-004): um consumível por turno, SEM gastar o ataque. Antes
   // usar uma poção passava a vez ao inimigo, então curar 15 contra um golpe de 14 era um turno
   // perdido por +1 de vida. itemUsedTurn guarda em qual turno o consumível já foi usado.
   if(s.screen==='combat'&&s.itemUsedTurn===s.combatTurn){const message='Você já usou um consumível neste turno. Ataque ou aguarde o próximo turno.';set({explorationNote:message});addLog(set,message);return}
   ...
   const value=consumableEffectiveValue(it,s);let resultMessage:string
   ...
   if(s.screen==='combat'){addLog(set,resultMessage);if(it.tipo==='cura')triggerSupportFx(set,get,'cura-item');else if(it.tipo==='escudo')triggerSupportFx(set,get,'fortificacao');set({itemUsedTurn:s.combatTurn})}
  },
  flee:()=>{const s=get();if(!s.playerTurn||s.animating)return;disarmEnemyPhaseWatchdog();const roll=Math.floor(Math.random()*6)+1;const outcome:FleeRoll['outcome']=roll>=5?'success':roll===4?'neutral':'failed';set({animating:true,playerTurn:false,animationActor:undefined,lastDamage:undefined,combatRoll:undefined,fleeRoll:{roll,outcome}});addLog(set,roll>=5?`Fuga: dado ${roll}. Você conseguiu escapar!`:roll===4?'Fuga: dado 4. Você não escapou, mas manteve sua ação.':`Fuga: dado ${roll}. A tentativa falhou e você perdeu o turno.`);setTimeout(()=>{const current=get();if(current.screen!=='combat'||!current.enemy)return;if(outcome==='success'){set({screen:current.subregionId?'region':'map',subregionId:undefined,explorationNote:undefined,enemy:undefined,pendingAttackBonus:0,shield:0,battleStance:'neutra',stanceChangeUsed:false,firstStrikeBonus:0,classBuffTurns:0,summon:undefined,lifeWardActive:false,groupCriticalBoost:false,heroStatus:{},enemyStatus:{},animating:false,fleeRoll:undefined,heroRollBonus:0,enemyRollBonus:0,dungeonActive:false,dungeonDepth:0});return}if(outcome==='neutral'){set({animating:false,playerTurn:true,fleeRoll:undefined});if(current.autoCombat)setTimeout(()=>runAutoCombatTurn(set,get),getCombatDelay(current,400));return}set({fleeRoll:undefined});enemyAfterDelay(set,get)},getCombatDelay(s,COMBAT_ROLL_DISPLAY_MS))},
  // Guilda: Bênção de Proteção (gasta ouro para evitar a perda de equipamento na derrota) e
  // Depósito (slots gratuitos + slots comprados). Solo apenas: o coop não aplica penalidade de derrota.
  buyProtectionBlessing:()=>{const s=get(),owned=s.protectionBlessings??0;if(owned>=MAX_PROTECTION_BLESSINGS){set({guildNotice:`Você já carrega o máximo de ${MAX_PROTECTION_BLESSINGS} bênçãos.`});return}const price=currentBlessingPrice(s);if(s.gold<price){set({guildNotice:`Ouro insuficiente: a bênção custa ${price}.`});return}set({gold:s.gold-price,protectionBlessings:owned+1,guildNotice:`Bênção de Proteção adquirida por ${price} de ouro: ela evita a perda de equipamento na sua próxima derrota.`})},
  buyVaultUpgrade:()=>{const s=get(),owned=s.guildVaultUpgrades??0,price=vaultUpgradePrice(owned);if(price===null){set({guildNotice:'O depósito já está no tamanho máximo.'});return}if(s.gold<price){set({guildNotice:`Ouro insuficiente: mais ${VAULT_SLOTS_PER_UPGRADE} slots custam ${price}.`});return}set({gold:s.gold-price,guildVaultUpgrades:owned+1,guildNotice:`Depósito ampliado por ${price} de ouro: agora são ${vaultCapacity(owned+1)} slots.`})},
  depositToVault:(ref:string)=>{const s=get(),vault=s.guildVault??[],index=s.equipmentBag.indexOf(ref);if(index<0)return;if(vault.length>=vaultCapacity(s.guildVaultUpgrades)){set({guildNotice:'O depósito está cheio. Compre mais slots ou retire um item.'});return}const bag=[...s.equipmentBag];bag.splice(index,1);set({equipmentBag:bag,guildVault:[...vault,ref],guildNotice:undefined})},
  withdrawFromVault:(ref:string)=>{const s=get(),vault=s.guildVault??[],index=vault.indexOf(ref);if(index<0)return;if(s.equipmentBag.length>=equipmentBagCapacity(s)){set({guildNotice:'Sua bolsa está cheia. Libere espaço antes de retirar o item.'});return}const rest=[...vault];rest.splice(index,1);set({guildVault:rest,equipmentBag:[...s.equipmentBag,ref],guildNotice:undefined})},
  ...
  // Negociador (coop): ao anunciar um item na vitrine da sala, ele sai da bolsa NA HORA (fica em
  // "depósito" só na tela de outro jogador, como shared_state.market) -- sem isso o vendedor
  // podia usar/vender o mesmo item por outro caminho enquanto o anúncio ainda estava de pé.
  // Cancelar (refundMarketItem) ou uma venda mal sucedida devolve pra bolsa.
  escrowMarketItem:(itemId:string,qty:number)=>{const s=get();if((s.inventory[itemId]??0)<qty)return false;const inv={...s.inventory,[itemId]:s.inventory[itemId]-qty};if(inv[itemId]<=0)delete inv[itemId];set({inventory:inv});return true},
  refundMarketItem:(itemId:string,qty:number)=>{const s=get();set({inventory:{...s.inventory,[itemId]:(s.inventory[itemId]??0)+qty}})},
  receiveMarketSale:(gold:number)=>{const s=get();set({gold:s.gold+Math.max(0,Math.floor(gold))})},
  completeMarketPurchase:(itemId:string,qty:number,price:number)=>{const s=get();if(s.gold<price)return;set({gold:s.gold-price,inventory:{...s.inventory,[itemId]:(s.inventory[itemId]??0)+qty}})},
  // Equipamento no Negociador (coop): mesma escrow/refund/complete dos consumíveis, mas cada
  // instância carrega os 6 Records por-ref (upgrade/gemas/trava/efeito forjado/elemento/
  // resistência) que vivem soltos no save -- ver EquipmentForgeSnapshot. refund devolve pra bolsa
  // com a MESMA ref (é rollback de um anúncio ainda meu); complete gera uma ref NOVA pro
  // comprador (evita colisão com refs que ele já possa ter) e reaplica o snapshot nela.
  escrowMarketEquipment:(ref:string)=>{const s=get(),idx=s.equipmentBag.indexOf(ref);if(idx<0||s.lockedEquipment?.[ref])return false;const bag=[...s.equipmentBag];bag.splice(idx,1);const snapshot:EquipmentForgeSnapshot={};if(s.equipmentUpgrades[ref])snapshot.upgrade=s.equipmentUpgrades[ref];if(s.equipmentGems[ref]?.length)snapshot.gems=s.equipmentGems[ref];if(s.forgedGemLocked?.[ref])snapshot.gemLocked=true;if(s.craftedEffects[ref])snapshot.craftedEffect=s.craftedEffects[ref];if(s.equipmentElements[ref])snapshot.element=s.equipmentElements[ref];if(s.equipmentResistances[ref])snapshot.resistance=s.equipmentResistances[ref];const equipmentUpgrades={...s.equipmentUpgrades},equipmentGems={...s.equipmentGems},forgedGemLocked={...s.forgedGemLocked},craftedEffects={...s.craftedEffects},equipmentElements={...s.equipmentElements},equipmentResistances={...s.equipmentResistances};delete equipmentUpgrades[ref];delete equipmentGems[ref];delete forgedGemLocked[ref];delete craftedEffects[ref];delete equipmentElements[ref];delete equipmentResistances[ref];set({equipmentBag:bag,equipmentUpgrades,equipmentGems,forgedGemLocked,craftedEffects,equipmentElements,equipmentResistances});return snapshot},
  refundMarketEquipment:(ref:string,snapshot:EquipmentForgeSnapshot)=>{const s=get();set({equipmentBag:[...s.equipmentBag,ref],...(snapshot.upgrade?{equipmentUpgrades:{...s.equipmentUpgrades,[ref]:snapshot.upgrade}}:{}),...(snapshot.gems?.length?{equipmentGems:{...s.equipmentGems,[ref]:snapshot.gems}}:{}),...(snapshot.gemLocked?{forgedGemLocked:{...s.forgedGemLocked,[ref]:true}}:{}),...(snapshot.craftedEffect?{craftedEffects:{...s.craftedEffects,[ref]:snapshot.craftedEffect}}:{}),...(snapshot.element?{equipmentElements:{...s.equipmentElements,[ref]:snapshot.element}}:{}),...(snapshot.resistance?{equipmentResistances:{...s.equipmentResistances,[ref]:snapshot.resistance}}:{})})},
  completeMarketEquipmentPurchase:(ref:string,price:number,snapshot:EquipmentForgeSnapshot)=>{const s=get(),equipment=eqById(ref);if(!equipment||s.gold<price||s.equipmentBag.length>=equipmentBagCapacity(s))return;const newRef=createEquipmentInstance(equipment.id);set({gold:s.gold-price,equipmentBag:[...s.equipmentBag,newRef],...(snapshot.upgrade?{equipmentUpgrades:{...s.equipmentUpgrades,[newRef]:snapshot.upgrade}}:{}),...(snapshot.gems?.length?{equipmentGems:{...s.equipmentGems,[newRef]:snapshot.gems}}:{}),...(snapshot.gemLocked?{forgedGemLocked:{...s.forgedGemLocked,[newRef]:true}}:{}),...(snapshot.craftedEffect?{craftedEffects:{...s.craftedEffects,[newRef]:snapshot.craftedEffect}}:{}),...(snapshot.element?{equipmentElements:{...s.equipmentElements,[newRef]:snapshot.element}}:{}),...(snapshot.resistance?{equipmentResistances:{...s.equipmentResistances,[newRef]:snapshot.resistance}}:{})})},
  ...
  unequip:(slot:Slot)=>{const s=get(),id=s.equipped[slot];if(!id||slot==='bolsa'||s.equipmentBag.length>=equipmentBagCapacity(s))return;const eq={...s.equipped};delete eq[slot];set({equipped:eq,equipmentBag:[...s.equipmentBag,id]})},
   addAttribute:(k:PrimaryAttributeKey)=>{const s=get();if(s.attributePoints<=0||!PRIMARY_ATTRIBUTES.includes(k))return;const before=maxHp(s),attr={...s.attr,[k]:s.attr[k]+1},allocatedAttr={...s.allocatedAttr,[k]:s.allocatedAttr[k]+1},gain=Math.max(0,maxHp({...s,attr,allocatedAttr})-before);set({attributePoints:s.attributePoints-1,attr,allocatedAttr,hp:s.hp+gain})},
   resetAttributes:()=>{
    const s=get()
    const totalAllocated=PRIMARY_ATTRIBUTES.reduce((sum,key)=>sum+(s.allocatedAttr?.[key]??0),0)
    if(totalAllocated<=0)return
    const newAttr=Object.fromEntries(PRIMARY_ATTRIBUTES.map(key=>[key,Math.max(0,s.attr[key]-(s.allocatedAttr?.[key]??0))])) as unknown as PrimaryAttributes
    set({
      attributePoints:s.attributePoints+totalAllocated,
      attr:newAttr,
      allocatedAttr:{...NO_ATTRIBUTES},
      hp:Math.min(s.hp,maxHp({...s,attr:newAttr,allocatedAttr:{...NO_ATTRIBUTES}})),
   ...
   finishLoot:()=>set({screen:get().subregionId?'region':'map',subregionId:undefined,explorationNote:undefined,enemy:undefined,enemyHp:0,combatLog:[],coin:undefined,playerTurn:false,animating:false,animationActor:undefined,lastDamage:undefined,combatRoll:undefined,fleeRoll:undefined,heroRollBonus:0,enemyRollBonus:0,enemyFearPenalty:0,heroSkillUses:0,itemSkillUsed:false,battleStance:'neutra',stanceChangeUsed:false,firstStrikeBonus:0,classBuffTurns:0,summon:undefined,lifeWardActive:false,groupCriticalBoost:false,heroStatus:{},enemyStatus:{},dungeonActive:false,dungeonDepth:0}),
   clearSave:()=>set({screen:'menu',heroId:undefined,hp:0,gold:0,xp:0,inventory:{},equipmentBag:[],protectionBlessings:0,crystals:0,guildVault:[],guildVaultUpgrades:0,equipped:{},territory:'Planícies de Alvora',regionId:'campos_dourados',subregionId:undefined,victories:{},subregionVictories:{},bossesDefeated:[],subregionBossesDefeated:[],currentEvent:undefined,eventResult:undefined,pendingAttackBonus:0,campaigns:{},activeCampaignId:undefined}),
  ...
  ,turnInStoryQuest:(questId:string)=>{
    ...
    if(quest.questItem){
      questItems[quest.questItem.id]-=quest.questItem.quantity;
      if(questItems[quest.questItem.id]<=0)delete questItems[quest.questItem.id];
    }
    const completedStoryQuests=(s.completedStoryQuests??[]).includes(questId)?s.completedStoryQuests:[...(s.completedStoryQuests??[]),questId];
    const activeStoryQuests={...(s.activeStoryQuests??{})};
    delete activeStoryQuests[questId];
  ...
  ,toggleAutoCombat:()=>{
    ...
    const next=!s.autoCombat
    set({autoCombat:next})
    if(next&&s.screen==='combat'&&s.playerTurn&&!s.animating){
      setTimeout(()=>runAutoCombatTurn(set,get),getCombatDelay(get(),400))
    }
  }
  ,toggleLockEquipment:(ref:string)=>{
    const s=get() as GameState
    set({lockedEquipment:{...(s.lockedEquipment??{}),[ref]:!s.lockedEquipment?.[ref]}})
  }
  ,sortEquipmentBag:(criteria:'rarity'|'level'|'name'|'slot'='rarity')=>{
    const s=get() as GameState
    const RARITY_ORDER:Record<Rarity,number>={heroico:7,mitico:6,lendario:5,epico:4,raro:3,incomum:2,comum:1}
    const sorted=[...s.equipmentBag].sort((a,b)=>{
      const itemA=eqById(a),itemB=eqById(b)
      if(!itemA||!itemB)return 0
      if(criteria==='rarity'){
        const rDiff=(RARITY_ORDER[itemB.raridade??'comum']??1)-(RARITY_ORDER[itemA.raridade??'comum']??1)
        if(rDiff!==0)return rDiff
        return (itemB.nivelMinimo??1)-(itemA.nivelMinimo??1)
      }
      if(criteria==='level')return (itemB.nivelMinimo??1)-(itemA.nivelMinimo??1)
      if(criteria==='slot')return slotOrder.indexOf(itemA.slot)-slotOrder.indexOf(itemB.slot)
      return itemA.nome.localeCompare(itemB.nome,'pt-BR')
    })
    set({equipmentBag:sorted})
  }
  ,claimDailyReward:()=>{
    const s=get() as GameState
    const now=Date.now(),last=s.dailyRewardClaimedAt??0
    if(now-last<20*60*60*1000)return null
    const lvl=deriveLevel(s.xp).lvl,gold=30+lvl*5,mat=REGION_MATERIALS[s.regionId]??REGION_MATERIALS.campos_dourados,matQty=Math.max(2,Math.floor(lvl/5))
    const materials={...(s.materials??{}),[mat.id]:(s.materials?.[mat.id]??0)+matQty}
    set({gold:s.gold+gold,materials,dailyRewardClaimedAt:now})
    return{gold,materials:{[mat.id]:matQty},message:`Recompensa Diária: +${gold} moedas de ouro e +${matQty} ${mat.nome}!`}
  }
  ,claimChallenge:(id:string)=>{
    const s=get() as GameState
    const challenge=activeChallenges().find(c=>c.id===id)
    if(!challenge||s.challengeClaimed?.[id]||(s.challengeProgress?.[id]??0)<challenge.target)return null
    set({gold:s.gold+challenge.reward,crystals:(s.crystals??0)+challenge.crystals,challengeClaimed:{...s.challengeClaimed,[id]:true}})
    return{gold:challenge.reward,crystals:challenge.crystals,message:`Desafio concluído: +${challenge.reward} de ouro e +${crystalsLabel(challenge.crystals)}!`}
  },
  // Cristais de Éter (moeda premium): por enquanto só ganho em jogo. Ainda não há nada para gastar.
  grantCrystals:(amount:number)=>{if(!Number.isFinite(amount)||amount<=0)return;set({crystals:((get() as GameState).crystals??0)+Math.floor(amount)})},
  openMapChest:(chestId:string,chest:{id:string;name:string;contents:{gold:number;materials?:Record<string,number>;consumables?:Record<string,number>}})=>{
    const s=get() as GameState
    if(s.openedChests?.[chestId])return null
    const goldReward=chest.contents.gold||0
    const mats={...(s.materials??{})}
    if(chest.contents.materials){
      for(const [matId,qty] of Object.entries(chest.contents.materials)){
        mats[matId]=(mats[matId]??0)+qty
      }
    }
    const inv={...(s.inventory??{})}
    if(chest.contents.consumables){
      for(const [itemId,qty] of Object.entries(chest.contents.consumables)){
        inv[itemId]=(inv[itemId]??0)+qty
      }
    }
    const parts:string[]=[]
    if(goldReward>0)parts.push(`${goldReward} moedas de ouro`)
    if(chest.contents.materials&&Object.keys(chest.contents.materials).length>0)parts.push(`${Object.values(chest.contents.materials).reduce((a,b)=>a+b,0)} materiais`)
    if(chest.contents.consumables&&Object.keys(chest.contents.consumables).length>0)parts.push('consumíveis valiosos')
    const note=`Você abriu "${chest.name}" e encontrou: ${parts.join(', ')}!`
  ...
  restAtCampfire:(regionId:string,campfire:{id:string;name:string;x:number;y:number})=>{
    set({
      ...
      explorationNote:`Você entrou na área de descanso da fogueira "${campfire.name}". Sua vida e sua Energia se recuperam lentamente enquanto estiver aqui.`
    })
  },
  ...
  // Descansar na fogueira também devolve Energia: +2 a cada tick (o mesmo intervalo da cura), sem passar do máximo.
  // Retorna o que recuperou para o mapa mostrar o popup certo (vida e/ou Energia).
  campfireHealTick:()=>{
    const s=get() as GameState
    const max=maxHp(s),energyMax=championStats(s).energiaMaxima,energyBefore=energyNow(s)
    const hp=s.hp<max,energy=energyBefore<energyMax
    if(!hp&&!energy)return{hp:false,energy:false}
    set({...(hp?{hp:Math.min(max,s.hp+1)}:{}),...(energy?{energy:gainEnergy(energyBefore,energyMax,ATTRIBUTE_RULES.energia.ganhoDescansoPorTick)}:{})})
    return{hp,energy}
  },
  addEnergy:(amount:number)=>{const s=get() as GameState;set({energy:gainEnergy(energyNow(s),championStats(s).energiaMaxima,amount)})},
...
export function attackValue(s:GameState){return Math.ceil(championStats(s).poderBasico*(1+(s.combatAttackPct??0)+STANCE_ATTACK_PCT[s.battleStance??'neutra']))}
/** Armadura total do Campeão (só de equipamentos, gemas e efeitos de equipamento), sem bônus percentuais. */
...
// Save antigo sem Energia gravada (ex.: aberto no meio de um combate): começa cheia, nunca zerada.
// A Energia é do herói, não do combate: sobe atacando (mais com crítico) e descansando na fogueira, e desce ao usar habilidade ou Fervor.
// Save antigo sem Energia gravada (ou com valor inválido) começa cheia, nunca zerada.
export function energyNow(s:GameState){const max=championStats(s).energiaMaxima;return typeof s.energy==='number'&&Number.isFinite(s.energy)?clampEnergy(s.energy,max):max}
/** Custo em Energia do Fervor de Combate (o ataque crítico garantido). */
export function fervorEnergyCost(){return ATTRIBUTE_RULES.energia.custoFervor}
export function heroSkillEnergyCost(heroId?:string){return heroStatProfile(heroId).habilidade.custoEnergia}
/** Potência da habilidade da classe (1 = valor inicial; sobe com o atributo de escala da habilidade). */
export function heroAbilityPotency(s:GameState){const profile=heroStatProfile(s.heroId);return abilityPotency(profile,championStats(s)[profile.habilidade.atributoEscala])}
// O auto guarda a Energia para a habilidade enquanto ela ainda vale a pena neste turno; sem isso o Fervor (mais barato) gastaria tudo antes.
function shouldAutoFervor(s:GameState){
 const cost=fervorEnergyCost(),energy=energyNow(s)
 if(energy<cost)return false
 const skillReady=(s.heroSkillCooldown??0)===0&&(s.heroId==='conjurador'?(s.heroSkillUses??0)<2&&Boolean(selectAutoSummonType(s)):shouldUseAutoHeroSkill(s))
 return !skillReady||energy>=heroSkillEnergyCost(s.heroId)+cost
}
function spendSkillEnergy(set:any,get:any,cost:number){const s=get() as GameState;const result=spendEnergy(energyNow(s),championStats(s).energiaMaxima,cost);if(result.ok)set({energy:result.energy});return result}
...
function equipmentRollBonus(s:GameState){return equipmentRollBonusTotal(s)}
...
export const COMBAT_ROLL_DISPLAY_MS=2500
export function getCombatDelay(s:{combatSpeed?:1|2|3}|undefined,baseMs:number){const speed=s?.combatSpeed??1;return Math.max(120,Math.round(baseMs/speed))}
export function runAutoCombatTurn(set:any,get:any){
  const s=get() as GameState
  if(s.screen!=='combat'||!s.playerTurn||s.animating||!s.enemy||!s.autoCombat)return
  if(heroStunned(set,get))return
  if((s.ultimateGauge??0)>=100){s.ultimateAttack();return}
  // A poção é ação rápida (não gasta o turno): bebe e, logo em seguida, o auto decide o resto do
  // turno (ataque, habilidade...). Se a poção não pôde ser usada, segue a decisão normal.
  if(s.hp<maxHp(s)*.35&&(s.inventory['pocao_cura']??0)>0&&s.itemUsedTurn!==s.combatTurn){
   s.useConsumable('pocao_cura')
   if((get() as GameState).itemUsedTurn===s.combatTurn){setTimeout(()=>runAutoCombatTurn(set,get),getCombatDelay(s,700));return}
  }
  ...
  }else if((s.heroSkillCooldown??0)===0&&energyNow(s)>=heroSkillEnergyCost(s.heroId)&&shouldUseAutoHeroSkill(s)){s.heroSkill();return}
  if(shouldAutoFervor(s)){s.useFervor();return}
...
function selectAutoSummonType(s:GameState){
 if((s.heroSkillUses??0)>=2||energyNow(s)<heroSkillEnergyCost(s.heroId))return undefined
...
// A Magia do Conjurador fortalece a fera (vida e ataque). Potência 1 = fera igual à de antes.
export function scaleSummon(summon:Summon,potency:number):Summon{if(potency<=1)return summon;const hp=Math.max(1,Math.round(summon.maxHp*potency));return{...summon,hp,maxHp:hp,ataque:Math.max(1,Math.round(summon.ataque*potency))}}
...
export function attackEffect(roll:number){return roll===1?'Raspão':roll===2?'Ataque desajeitado':roll<=4?'Ataque normal':roll===5?'Ataque forte':'Ataque crítico'}
...
const ATTACK_TIER_PHRASE:Record<number,string[]>={
 1:['mas o golpe pega apenas de raspão','mas erra o ponto certo e só roça o alvo','mas perde a força no último instante e acerta de raspão'],
...
export function resolveCombatRoll(attackBase:number,defenseBase:number,attackRoll:number,defenseRoll:number,critBonusPct=0){
 // Face 1 do ataque (auditoria v0.8.84, GAME-002): antes era uma "falha crítica" -- 0 de dano e
 // ~10% do ataque de volta em quem golpeou, em 1 de cada 6 golpes, para heróis E inimigos. Agora é
 // um Raspão: metade do dano normal e nenhum dano no atacante. `selfDamage` continua no retorno
 // (sempre 0) só para não quebrar o formato usado por solo, coop, summons e pela UI.
 const effectiveAttack=attackBase+(attackRoll===5?1:0)
 let damage=Math.max(1,effectiveAttack-defenseBase)
 if(attackRoll===1)damage=Math.max(1,Math.floor(damage*.5))
...
export function applyDefeatPenalty(set:any,get:any,reason='Você foi derrotado.'){
 ...
 if(s.heroId==='sacerdotisa'&&s.lifeWardActive){
  const revived=Math.max(1,Math.ceil(maxHp(s)*(s.lifeWardPct??.3)))
 ...
 // Bênção de Proteção (comprada na Guilda): gasta UMA carga em toda derrota em que havia
 // equipamento em risco e evita a perda. Não gasta carga se não havia risco (sem itens vestidos,
 // ou a misericórdia por derrotas seguidas já zerou a chance).
 const atRisk=eligible.length>0&&equipmentLossChance>0
 const blessed=atRisk&&(s.protectionBlessings??0)>0
 const lostEntry=atRisk&&!blessed&&Math.random()<equipmentLossChance?eligible[Math.floor(Math.random()*eligible.length)]:undefined
 ...
 let itemMessage=blessed?'Sua Bênção de Proteção foi consumida e nenhum equipamento foi perdido.':'Nenhum equipamento foi perdido.'
 if(lostEntry){
 ...
 addLog(set,`${reason} ${penalty}`)
  ...
  set({screen:s.subregionId?'region':'map',subregionId:undefined,explorationNote:penalty,gold:s.gold-goldLost,equipped,equipmentBag,protectionBlessings:(s.protectionBlessings??0)-(blessed?1:0),equipmentUpgrades,equipmentGems,forgedGemLocked,craftedEffects,equipmentElements,equipmentResistances,hp:recoveredHp,regionMapPositions:respawnPositions,enemy:undefined,enemyHp:0,ultimateGauge:0,combatMinions:[],pendingAttackBonus:0,shield:0,battleStance:'neutra',stanceChangeUsed:false,firstStrikeBonus:0,classBuffTurns:0,summon:undefined,lifeWardActive:false,groupCriticalBoost:false,heroStatus:{},enemyStatus:{},combatRoll:undefined,fleeRoll:undefined,heroRollBonus:0,enemyRollBonus:0,enemyFearPenalty:0,animating:false,animationActor:undefined,lastDamage:undefined,playerTurn:false,dungeonActive:false,dungeonDepth:0,consecutiveDefeats:priorLosses+1,lastDefeatKey:defeatKey})
}
function playerAttack(set:any,get:any,label:string,bonus=0,alreadyAnimating=false,targetMinionId?:string,forceCrit=false,forceStatusElement?:Element){
 ...
 if(forceCrit&&energyNow(s)<fervorEnergyCost())return
 if(heroStunned(set,get))return
 if(forceCrit)spendSkillEnergy(set,get,fervorEnergyCost())
 ...
 const resolved=resolveCombatRoll(attackBase,defenseBase,attackRoll,defenseRoll,critBonusPct)
 let damage=resolved.damage
 const selfDamage=resolved.selfDamage

 const heroElem=forceStatusElement??heroWeaponElement(s)
 const enemyElem=s.enemy.elemento??(s.enemy.boss?'sombra':'fisico')
 const enemyWeak=ELEMENT_ADVANTAGES[enemyElem]?.weakAgainst??[]
 const enemyStrong=ELEMENT_ADVANTAGES[enemyElem]?.strongAgainst??[]
 const hitsWeakness=!target&&heroElem!=='fisico'&&enemyWeak.includes(heroElem)
 const hitsResistance=!target&&heroElem!=='fisico'&&(heroElem===enemyElem||enemyStrong.includes(heroElem))
 if(hitsWeakness&&damage>0)damage=Math.max(1,Math.round(damage*1.35))
 else if(hitsResistance&&damage>0)damage=Math.max(1,Math.round(damage*0.75))

 let staggerCurrent=s.staggerCurrent??0
 const staggerMax=s.staggerMax??Math.max(12,Math.ceil(s.enemy.vida*.35))
 let isStaggered=s.isStaggered??false
 let staggerBroken=false
 if(isStaggered&&damage>0){
  damage=Math.max(1,Math.round(damage*1.5))
  isStaggered=false
  staggerCurrent=0
  staggerBroken=true
 }else if(!target&&damage>0&&!selfDamage){
  const staggerGain=Math.max(2,Math.floor(damage*.5)+(attackRoll>=5?4:1))
  staggerCurrent+=staggerGain
  if(staggerCurrent>=staggerMax){
   isStaggered=true
   staggerCurrent=staggerMax
  }
 }
 ...
 // Energia: o ataque normal dá 1 (2 no crítico); o Fervor e as habilidades não devolvem nada. Passiva do Monge: golpe forte (5) às vezes rende +1.
 const basicAttack=label==='Ataque'||label==='Ataque direcionado',monkEnergyProc=s.heroId==='monge'&&attackRoll===5&&Math.random()<.25
 const energyGained=(basicAttack?attackEnergyGain(attackRoll===6):0)+(monkEnergyProc?1:0)
 const energy=gainEnergy(energyNow(get() as GameState),championStats(s).energiaMaxima,energyGained)
 ...
 set({animating:true,playerTurn:false,animationActor:selfDamage?'enemy':'hero',lastDamage:selfDamage||damage,combatRoll,heroRollBonus:0,firstStrikeBonus:0,enemyRollBonus:attackRoll===2?1:s.enemyRollBonus,enemyStatus:statusResult.status,hp:healAmount>0?s.hp+healAmount:s.hp,energy,staggerCurrent,isStaggered,highestDamageDealt,ultimateGauge})
 ...
 const weaknessTag=hitsWeakness?' [Fraqueza Elemental +35%]':hitsResistance?' [Resistência Elemental -25%]':''
 const staggerTag=staggerBroken?' [RUPTURA CONSUMIDA: +50% de dano!]':isStaggered?' [RUPTURA DE POSTURA! Inimigo atordoado!]':''
 addLog(set,`${heroName} ${narration}.${defenseNarration} ${label}: dado ${attackRoll} (${attackEffect(attackRoll)}) contra defesa ${defenseRoll} (${defenseEffect(defenseRoll)}). ${selfDamage?`Recebeu ${selfDamage} de dano.`:`Causou ${damage} de dano${target?` a ${foeName}`:''}${weaknessTag}${staggerTag}.`}${attackRoll===2?' Inimigo recebe +1 na próxima rolagem.':''}${healAmount>0?` A energia natural do equipamento pulsa e recupera ${healAmount} de vida.`:''}${enemyStun.wasStunned?` ${foeName} estava atordoado e não conseguiu se defender.`:''}${statusResult.appliedKind?` ${foeName} fica ${STATUS_LABELS[statusResult.appliedKind]}.`:''}`)
 setTimeout(()=>{
  ...
  if(selfDamage){const heroHp=Math.max(0,now.hp-selfDamage);set({hp:heroHp});if(heroHp<=0)setTimeout(()=>applyDefeatPenalty(set,get,'Você sucumbiu após uma falha crítica.'),getCombatDelay(now,900));else enemyAfterDelay(set,get);return}
...
export function ultimateEffects(heroClass:string,atk:number,heroMaxHp:number){
  ...
  else if(heroClass==='monge'){damage+=Math.round(atk*.16);extraEnergy=2}
  ...
  return{damage,bonusHeal,bonusShield,extraEnergy}
}
function playerUltimateAttack(set:any,get:any){
  ...
  const{damage,bonusHeal,bonusShield,extraEnergy}=ultimateEffects(heroClass,atk,heroMaxHp)
  ...
  const nextEnergy=gainEnergy(energyNow(s),championStats(s).energiaMaxima,extraEnergy)
  const highestDamageDealt=Math.max(s.highestDamageDealt??0,damage)
...
function continueEnemyAfterDelay(set:any,get:any){
 ...
 if((s.classBuffTurns??0)>0){
  ...
  if(turnsLeft<=0){
   const expired:any={classBuffTurns:0},label=s.heroId==='guerreiro'?'Ímpeto Marcial':s.heroId==='arcanista'?'Ascensão Arcana':s.heroId==='cacador'?'Marca do Predador':s.heroId==='guardiao'?'Provocar':s.heroId==='cacadora'?'Ataque Duplo':undefined
   ...
   // Ataque Duplo não é um bônus "sticky" como os outros (%/flags que ficam ligados até
   // expirar) -- extraHeroAttacks é consumido a cada ataque, então precisa ser rearmado no
   // início de cada turno restante do buff, não só na ativação.
   const renewed:any={classBuffTurns:turnsLeft}
   if(s.heroId==='cacadora')renewed.extraHeroAttacks=1
   set(renewed)
  }
 }
...
function enemyAttack(set:any,get:any){
 ...
 if(pre.isStaggered){
  set({isStaggered:false,staggerCurrent:0,animating:false,playerTurn:true,animationActor:undefined,lastDamage:undefined,combatRoll:undefined})
  addLog(set,`[RUPTURA] ${pre.enemy.nome} está cambaleando pela quebra de postura e perdeu o turno!`)
  if((get() as GameState).autoCombat)setTimeout(()=>runAutoCombatTurn(set,get),getCombatDelay(get(),500))
  return
 }
 ...
 const attackBase=Math.ceil(enemy.ataque*(intent.type==='heavy'?1.35:1)),defenseBase=intercepting?summon!.defesa:defenseValue(s),naturalAttackRoll=Math.floor(Math.random()*6)+1,attackBonus=s.enemyRollBonus-rollPenaltyFrom(s.enemyStatus),attackRoll=Math.max(1,Math.min(6,naturalAttackRoll+attackBonus-(s.heroId==='druida'&&Math.random()<.25?1:0)-(s.enemyFearPenalty??0))),naturalDefenseRoll=Math.floor(Math.random()*6)+1,defenseRoll=intercepting?Math.max(1,naturalDefenseRoll):Math.max(1,Math.min(6,naturalDefenseRoll+(s.classRollBonus??0)+(hasCraftedEffect(s,'defesa_perfeita')&&naturalDefenseRoll===5?1:0)-rollPenaltyFrom(s.heroStatus)))
 const dodged=!intercepting&&Math.random()<heroDodgeChance(s),resolved=resolveCombatRoll(attackBase,defenseBase,attackRoll,defenseRoll);let raw=dodged?0:resolved.damage,shield=s.shield
 ...
 // Vigor: resistência percentual (retorno decrescente) contra ataques elementais. Físico é coberto pela Armadura.
 if(!intercepting&&enemyElement!=='fisico'&&raw>0)raw=Math.max(1,Math.round(raw*(1-heroElementalResistance(s))))
 ...
 const statusResult=!intercepting&&!dodged&&!resolved.selfDamage&&raw>0&&!resisted&&(naturalAttackRoll===6||intent.type==='status')&&Math.random()>=heroEffectResistance(s)?applyElementalStatus(s.heroStatus,enemyElement,attackBase,intent.type==='status'):{status:s.heroStatus??{},appliedKind:undefined as string|undefined}
 const priestDefenseHeal=!intercepting&&!resolved.selfDamage&&s.heroId==='sacerdotisa'&&defenseRoll>=5&&Math.random()<.2?1:0
 const counterDamage=(!intercepting&&!dodged&&!resolved.selfDamage&&defenseRoll===6)?Math.max(1,Math.round(attackValue(s)*.35)):0
 ...
 const combatRoll:CombatRoll={attacker:'enemy',naturalAttackRoll,attackRoll,attackBonus,defenseRoll,attackBase,defenseBase,attackEffect:attackEffect(attackRoll),defenseEffect:defenseEffect(defenseRoll),damage:raw,selfDamage:resolved.selfDamage,shieldBlocked:blocked,dodged}
 ...
 set({shield,ultimateGauge,animating:true,animationActor:resolved.selfDamage?'hero':'enemy',lastDamage:resolved.selfDamage||raw,combatRoll,playerTurn:false,enemyRollBonus:0,heroRollBonus:attackRoll===2?1:s.heroRollBonus,heroStatus:statusResult.status,...(intercepting?{summons:summonsAfter,summon:summonsAfter[0],...(!summonsAfter.some(fera=>fera.tipo==='arcano')?{combatAttackPct:0,combatDefensePct:0}:{})}:{})})
 setTimeout(()=>{
  const current=get() as GameState
  if(current.screen!=='combat'||!current.enemy)return
  if(resolved.selfDamage){addLog(set,`${enemyName} ${enemyApproach}, ${attackTierPhrase(attackRoll)}, sofrendo ${resolved.selfDamage} de dano do próprio golpe.`);if(enemyHp<=0){victory(set,get);return}set({enemyHp,hp,animating:false,animationActor:undefined,combatRoll:undefined});resolveMinionAttacks(set,get,()=>{const latest=get() as GameState;if(latest.screen==='combat'){set({playerTurn:true,combatTurn:latest.combatTurn+1});if(latest.autoCombat)setTimeout(()=>runAutoCombatTurn(set,get),getCombatDelay(latest,400))}});return}
  set({hp,animationActor:undefined,combatRoll:undefined,playerTurn:false})
  const counterMsg=counterDamage>0?` [CONTRA-ATAQUE! Você bloqueou perfeitamente e contra-atacou ${counterDamage} de dano!]`:''
  ...
  else addLog(set,dodged?`${enemyName} ${enemyApproach}, mas a Esquiva do Ladino faz o golpe errar completamente.`:`${enemyName} ${enemyApproach}, ${attackTierPhrase(attackRoll)}. ${heroName} ${defenseTierPhrase(defenseRoll)}. Dado ${attackRoll} (${attackEffect(attackRoll)}) contra defesa ${defenseRoll} (${defenseEffect(defenseRoll)}); causou ${raw} de dano${blocked?` (${blocked} bloqueado)`:''}${resisted?` (resistência elemental reduziu ${resistanceReduction})`:''}.${attackRoll===2?' Você recebe +1 na próxima rolagem.':''}${statusResult.appliedKind?` Você fica ${STATUS_LABELS[statusResult.appliedKind]}.`:''}${priestDefenseHeal?' A fé da Sacerdotisa recupera 1 de vida.':''}${counterMsg}`)
  if(enemyHp<=0){victory(set,get);return}
  if(hp<=0){setTimeout(()=>applyDefeatPenalty(set,get),getCombatDelay(current,900));return}
  resolveMinionAttacks(set,get,()=>{const latest=get() as GameState;if(latest.screen==='combat'){set({animating:false,animationActor:undefined,lastDamage:undefined,playerTurn:true,combatTurn:latest.combatTurn+1});if(latest.autoCombat)setTimeout(()=>runAutoCombatTurn(set,get),getCombatDelay(latest,400))}})
 },getCombatDelay(s,COMBAT_ROLL_DISPLAY_MS))
}
...
const gold=Math.round(en.ouro*(1+rewardGoldBonus));const before=deriveLevel(s.xp).lvl;const xp=s.xp+(en.xpReward??0);const after=deriveLevel(xp).lvl;const points=s.attributePoints+Math.max(0,after-before);const key=encounterSubId??s.territory;const victories={...s.victories,[s.territory]:(s.victories[s.territory]??0)+1};const subregionVictories={...s.subregionVictories,[key]:(s.subregionVictories[key]??0)+1};const bosses=[...s.bossesDefeated],subBosses=[...s.subregionBossesDefeated];if(en.boss){if(!bosses.includes(String(en.dificuldade)))bosses.push(String(en.dificuldade));
...
// Contrato 47: toda vitória de combate conta pro desafio diário/semanal de "vencer N combates";
// se for um andar de masmorra infinita (en.dungeon), também conta pro desafio de masmorra.
const challengeProgress=bumpChallengeProgress(s.challengeProgress,en.dungeon?['combat_win','dungeon_win']:['combat_win'])
set({gold:s.gold+gold,xp,attributePoints:points,victories,subregionVictories,bossesDefeated:bosses,subregionBossesDefeated:subBosses,guildProgress,equipmentBag,inventory,materials,revengeWins,discoveredCards,equipmentElements,equipmentResistances,challengeProgress,bestiary:{...s.bestiary,[baseName]:{...record,vitorias:record.vitorias+1}},consecutiveDefeats:0,lastDefeatKey:undefined,defeatedWanderers:encounterSubId?{...s.defeatedWanderers,[encounterSubId]:true}:s.defeatedWanderers,screen:'loot',enemy:undefined,enemyHp:0,ultimateGauge:0,animating:false,animationActor:undefined,lastDamage:undefined,playerTurn:false,pendingAttackBonus:0,shield:0,activePotionIds:[],loot:{gold,xp:en.xpReward??0,itemId,equipmentId,missedEquipmentId,title:en.revenge?'VINGANÇA CONCLUÍDA':en.boss?'CHEFE DERROTADO':en.variante==='Campeão'?'CAMPEÃO DERROTADO':'VITÓRIA',leveledUp:after>before,newLevel:after,levelsGained:after-before,isNewEquipment,isNewItem}})}
