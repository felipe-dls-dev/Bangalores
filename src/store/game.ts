...
export function specializationBonuses(s:GameState){const choices=Object.values(s.specializations??{});return{attack:(choices.includes('lenda')?2:0),defense:(choices.includes('defensiva')?1:0)+(choices.includes('baluarte')?2:0)+(choices.includes('lenda')?2:0),life:(choices.includes('vital')?10:0)+(choices.includes('utilidade')?5:0),crit:choices.includes('ofensiva') ? .05 : 0,bossDamage:choices.includes('carrasco')?3:0,loot:choices.includes('tesouro') ? .1 : 0,reward:choices.includes('fortuna') ? .2 : 0}}
...
export function bestiaryDamageBonus(s:GameState,enemy:Enemy|undefined){if(!enemy)return 0;const key=enemy.nome.replace(/^Vingança \d+: /,'').replace(/^(Veterano|Elite|Campeão): /,''),wins=s.bestiary[key]?.vitorias??0;return wins>=BESTIARY_MILESTONES[2].wins?1:0}
...
export function applyElementalStatus(status:StatusEffects|undefined,element:Element,attackPower:number,force=false):{status:StatusEffects;appliedKind?:string}{
 ...
 if(kind==='bleed')return{status:{...current,bleed:{turns:2,amount:Math.max(1,(existing?.amount??0)+Math.round(attackPower*.18))}},appliedKind:kind}
 if(kind==='burn')return{status:{...current,burn:{turns:1,amount:Math.max(2,Math.round(attackPower*.38))}},appliedKind:kind}
 return{status:{...current,poison:{turns:3,amount:Math.max(1,Math.round(attackPower*.16))}},appliedKind:kind}
}
