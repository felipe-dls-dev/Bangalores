...
interface GameState {
 ...
 difficultyMode:DifficultyMode;talents:string[];materials:Record<string,number>;equipmentUpgrades:Record<string,number>;equipmentGems:Record<string,string[]>;craftedEffects:Record<string,ForgeEffect>;bestiary:Record<string,{encontros:number;vitorias:number}>;revengeWins:Record<string,number>;dungeonDepth:number;dungeonActive:boolean;storyFlags:string[];storyChapterId:string;storyChoices:Record<string,string>;storyNotice?:string;
...
export const useGame = create<GameState>()(persist((set,get)=>({
  ...
  ,craftEquipment:(recipeId?:string)=>{const s=get(),recipe=FORGE_RECIPES.find(r=>r.id===recipeId),item=recipe&&eqById(recipe.equipmentId);if(!recipe||!item||s.equipmentBag.length>=equipmentBagCapacity(s)||!equipmentClassAllowed(item,s.heroId)||Object.entries(recipe.materials).some(([id,qty])=>(s.materials[id]??0)<qty))return;const materials={...s.materials};for(const [id,qty] of Object.entries(recipe.materials))materials[id]-=qty;set({materials,equipmentBag:[...s.equipmentBag,item.id],craftedEffects:recipe.effect?{...s.craftedEffects,[item.id]:recipe.effect}:s.craftedEffects,explorationNote:`Forjado: ${recipe.nome}${recipe.effectText?` • ${recipe.effectText}`:''}.`})}
