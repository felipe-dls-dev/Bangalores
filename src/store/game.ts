...
export const GUILD_RANKS:GuildRank[]=[
 {id:'ferro',nome:'Ferro',minimo:0,cor:'#8d8580'},{id:'bronze',nome:'Bronze',minimo:2,cor:'#b87333'},{id:'prata',nome:'Prata',minimo:4,cor:'#c5ccd3'},{id:'ouro',nome:'Ouro',minimo:7,cor:'#e5b84b'},{id:'platina',nome:'Platina',minimo:10,cor:'#74d4c5'},{id:'diamante',nome:'Diamante',minimo:15,cor:'#65bfff'},{id:'campeao',nome:'Campeão',minimo:23,cor:'#d783ff'}
]
export function guildRankFor(reputation:number){return [...GUILD_RANKS].reverse().find(rank=>reputation>=rank.minimo)??GUILD_RANKS[0]}
export interface GuildMission { id:string; nome:string; descricao:string; tipo:'any'|'specific'|'boss'; quantidade:number; alvo?:string; local?:string; recompensa:{tipo:'gold'|'equipment';valor:number}; dificuldade:number; rank:GuildRankId }
export const GUILD_MISSIONS:GuildMission[]=[
 {id:'caca_inicial',nome:'Limpeza das Estradas',descricao:'Derrote quaisquer 3 monstros nas terras de Eldravar.',tipo:'any',quantidade:3,recompensa:{tipo:'gold',valor:12},dificuldade:1,rank:'ferro'},
 {id:'ameaca_goblin',nome:'Ameaça Goblin',descricao:'Derrote 3 inimigos que possuam Goblin no nome.',tipo:'specific',alvo:'goblin',quantidade:3,local:'Floresta Lunargenta',recompensa:{tipo:'gold',valor:22},dificuldade:2,rank:'ferro'},
 {id:'caca_lobos',nome:'Peles para a Guilda',descricao:'Derrote 3 inimigos que possuam Lobo no nome.',tipo:'specific',alvo:'lobo',quantidade:3,local:'Campos ou regiões geladas',recompensa:{tipo:'equipment',valor:1},dificuldade:2,rank:'bronze'},
 {id:'prova_chefes',nome:'Prova do Caçador',descricao:'Derrote qualquer chefe de sub-região.',tipo:'boss',quantidade:1,recompensa:{tipo:'equipment',valor:1},dificuldade:3,rank:'prata'},
 {id:'veterano_guilda',nome:'Veterano da Guilda',descricao:'Derrote 10 monstros de qualquer espécie.',tipo:'any',quantidade:10,recompensa:{tipo:'equipment',valor:1},dificuldade:4,rank:'ouro'},
 {id:'dominio_chefes',nome:'Domínio dos Tiranos',descricao:'Derrote 2 chefes diferentes ou repetidos nas regiões avançadas.',tipo:'boss',quantidade:2,recompensa:{tipo:'gold',valor:85},dificuldade:5,rank:'platina'},
 {id:'queda_ignaroth',nome:'A Queda de Ignaroth',descricao:'Derrote o chefe Ignaroth no Ninho do Dragão Vermelho.',tipo:'boss',alvo:'ignaroth',quantidade:1,local:'Pico Escarlate',recompensa:{tipo:'gold',valor:140},dificuldade:6,rank:'diamante'}
]
interface GameState {
...
export const useGame = create<GameState>()(persist((set,get)=>({
  ...
  startEncounter:(subregionId:string)=>{
    ...
    const lvl=deriveLevel(s.xp).lvl; beginCombat(set,get,buildEnemy(sub,lvl))
  },
  startBoss:()=>{const sub=currentSubregion(get());if(sub)beginCombat(set,get,buildBoss(sub))},
...
}),{name:'bangalores-save-v1',merge:(persisted:any,current:any)=>{const merged={...current,...persisted,equipped:{...(persisted?.equipped??{}),bolsa:persisted?.equipped?.bolsa??'mochila_pequena_8'},regionId:persisted?.regionId??'campos_dourados',subregionVictories:persisted?.subregionVictories??{},subregionBossesDefeated:persisted?.subregionBossesDefeated??[],pendingAttackBonus:persisted?.pendingAttackBonus??0,currentEvent:persisted?.currentEvent,eventResult:persisted?.eventResult,customCards:persisted?.customCards??[],campaigns:persisted?.campaigns??{},guildAccepted:persisted?.guildAccepted??[],guildProgress:persisted?.guildProgress??{},guildClaimed:persisted?.guildClaimed??[],guildNotice:undefined,animating:false,animationActor:undefined,lastDamage:undefined,combatRoll:undefined,fleeRoll:undefined,playerTurn:persisted?.screen==='combat'&&persisted?.enemy?true:(persisted?.playerTurn??false)};if(merged.heroId&&!merged.activeCampaignId){const id=`campaign_legacy_${Date.now()}`;merged.activeCampaignId=id;merged.campaigns={...merged.campaigns,[id]:campaignSnapshot(merged)}}return merged}}))
