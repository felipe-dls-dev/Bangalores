...
export const useGame = create<GameState>()(persist((set,get)=>({
  ...
  heroSkill:()=>{const s=get();if(s.heroSkillUsed||!s.playerTurn||s.animating||!s.enemy)return;set({heroSkillUsed:true});if(s.heroId==='guardiao'){set({guardianTaunt:true});addLog(set,'PROVOCAR: os inimigos passam a priorizar o Guardião.');enemyAfterDelay(set,get);return}if(s.heroId==='guerreiro'){set({combatAttackPct:(s.combatAttackPct??0)+.1,combatDefensePct:(s.combatDefensePct??0)+.1});addLog(set,'Ímpeto Marcial: Ataque e Defesa base aumentados em 10% (arredondados para cima).');triggerSupportFx(set,get,'fortificacao');enemyAfterDelay(set,get);return}if(s.heroId==='cacadora'){set({extraHeroAttacks:1});addLog(set,'Ataque Duplo: você pode atacar duas vezes neste turno.');return}if(s.heroId==='arcanista'){set({classRollBonus:(s.classRollBonus??0)+2,combatAttackPct:(s.combatAttackPct??0)+.1,combatDefensePct:(s.combatDefensePct??0)+.1});addLog(set,'Ascensão Arcana: +2 nas rolagens e +10% de Ataque e Defesa para o grupo.');triggerSupportFx(set,get,'fortificacao');enemyAfterDelay(set,get);return}if(s.heroId==='druida'){const heal=Math.max(1,Math.ceil(maxHp(s)*.3)),amount=Math.min(heal,maxHp(s)-s.hp);set({hp:s.hp+amount});addLog(set,`Renovo de Abdendriel: recuperou ${amount} de vida do campeão mais ferido.`);triggerSupportFx(set,get,'cura');enemyAfterDelay(set,get);return}if(s.heroId==='cacador'){set({groupCriticalBoost:true});addLog(set,'Marca do Predador: resultados 5 e 6 passam a causar ataques críticos.');triggerSupportFx(set,get,'fortificacao');enemyAfterDelay(set,get)}},
