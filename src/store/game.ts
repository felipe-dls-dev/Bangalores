...
interface GameState {
 ...
 difficultyMode:DifficultyMode;talents:string[];materials:Record<string,number>;equipmentUpgrades:Record<string,number>;equipmentGems:Record<string,string[]>;craftedEffects:Record<string,ForgeEffect>;forgeXp?:number;forgeAttempts?:number;forgeSuccesses?:number;bestiary:Record<string,{encontros:number;vitorias:number}>;revengeWins:Record<string,number>;dungeonDepth:number;dungeonActive:boolean;storyFlags:string[];storyChapterId:string;storyChoices:Record<string,string>;storyNotice?:string;
...
function equipmentSetCounts(s:GameState){const names=Object.values(s.equipped).map(id=>eqById(id)?.nome.toLocaleLowerCase('pt-BR')??'');return{lua:names.filter(n=>/lua|lunar|lunargenta/.test(n)).length,cinzas:names.filter(n=>/cinza|dragão|escarlate/.test(n)).length,khar:names.filter(n=>/khar|runa|bronze/.test(n)).length,eclipse:names.filter(n=>/eclipse|sombr|malgor/.test(n)).length}}
...
export const useGame = create<GameState>()(persist((set,get)=>({
  ...
  resolveEvent:(accept:boolean,approach?:'class')=>resolveExplorationEvent(set,get,accept,approach),
...
function beginCombat(set:any,get:any,enemy:Enemy){const coin=Math.random()<.5?'cara':'coroa';const s=get() as GameState,key=enemy.nome.replace(/^(Veterano|Elite|Campeão): /,'');const known=s.bestiary[key]??{encontros:0,vitorias:0},sets=equipmentSetCounts(s),setShield=sets.khar>=4?3:0,setRoll=enemy.boss&&sets.eclipse>=4?1:0;set({screen:'combat',enemy,enemyHp:enemy.vida,combatTurn:1,combatLog:[`${enemy.variante&&enemy.variante!=='Comum'?enemy.variante+' • ':''}Nível ${enemy.nivel??enemy.dificuldade}.`,`Afinidade elemental: ${enemy.elemento??(enemy.boss?'sombra':'físico')}.`,...(setShield?[`Conjunto de Khar-Dur: +${setShield} de escudo inicial.`]:[]),...(setRoll?[`Conjunto do Eclipse: +1 nas rolagens contra chefes.`]:[]),`Moeda: ${coin.toUpperCase()}. ${coin==='cara'?'Você':'Inimigo'} começa.`],coin,playerTurn:coin==='cara',animating:false,animationActor:undefined,lastDamage:undefined,combatRoll:undefined,fleeRoll:undefined,heroRollBonus:(s.talents.includes('destino')?1:0)+setRoll,enemyRollBonus:0,heroSkillUsed:false,itemSkillUsed:false,shield:s.shield+(s.heroId==='guardiao'?2:0)+setShield,bestiary:{...s.bestiary,[key]:{...known,encontros:known.encontros+1}}});if(coin==='coroa')setTimeout(()=>enemyAttack(set,get),800)}
...
function playerAttack(set:any,get:any,label:string,bonus=0,alreadyAnimating=false){
 ...
 const attackBase=attackValue(s)+bonus,defenseBase=Math.max(0,(s.enemy.dificuldade??1)-2),naturalAttackRoll=Math.floor(Math.random()*6)+1,attackBonus=s.heroRollBonus,attackRoll=Math.min(6,naturalAttackRoll+attackBonus+(hasCraftedEffect(s,'critico')&&naturalAttackRoll===5?1:0)),defenseRoll=Math.floor(Math.random()*6)+1
...
function enemyAttack(set:any,get:any){
 ...
 const attackBase=s.enemy.ataque,defenseBase=defenseValue(s),naturalAttackRoll=Math.floor(Math.random()*6)+1,attackBonus=s.enemyRollBonus,attackRoll=Math.min(6,naturalAttackRoll+attackBonus),naturalDefenseRoll=Math.floor(Math.random()*6)+1,defenseRoll=Math.min(6,naturalDefenseRoll+(hasCraftedEffect(s,'defesa_perfeita')&&naturalDefenseRoll===5?1:0))
 ...
 setTimeout(()=>{
  ...
  if(hp<=0){addLog(set,'Você foi derrotado e retornou à região selecionada.');setTimeout(()=>set({screen:current.subregionId?'region':'map',subregionId:undefined,explorationNote:undefined,hp:maxHp({...current,hp} as GameState),enemy:undefined,combatMinions:[],pendingAttackBonus:0,shield:0}),900);return}resolveMinionAttacks(set,get,()=>{const latest=get() as GameState;if(latest.screen==='combat')set({animating:false,animationActor:undefined,lastDamage:undefined,playerTurn:true,combatTurn:latest.combatTurn+1})})
 },COMBAT_ROLL_DISPLAY_MS)
}
