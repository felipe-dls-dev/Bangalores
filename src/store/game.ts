...
export const useGame = create<GameState>()(persist((set,get)=>({
  ...
  receiveCoopSupportFx:(type:'fortificacao'|'cura')=>{const s=get();if(s.screen!=='combat')return;triggerSupportFx(set,get,type)},
...
function triggerSupportFx(set:any,get:any,type:'fortificacao'|'cura'){set({supportFx:{type}});setTimeout(()=>{if((get() as GameState).supportFx?.type===type)set({supportFx:undefined})},1600)}
