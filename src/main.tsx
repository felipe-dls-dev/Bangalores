...
function ArtPreview({image,name,text,stats,className,imgStyle,compareEquipment=false,allowEquip=false}:{image:string;name:string;text?:string;stats?:string;className?:string;imgStyle?:React.CSSProperties;compareEquipment?:boolean;allowEquip?:boolean}){
 ...
 const equipped=useGame(state=>state.equipped),heroId=useGame(state=>state.heroId),xp=useGame(state=>state.xp),equipmentBag=useGame(state=>state.equipmentBag),equip=useGame(state=>state.equip)
...
function TopBar(){
 ...
 const combatLockTitle='Fuja da batalha para acessar outras telas.'
 const goTo=(screen:string)=>{if(inCombat)return;g.setScreen(screen as any);setMenuOpen(false)}
 return <header className="topbar"><nav>{nav.map(([id,label,Icon])=><button key={id} aria-label={label} disabled={inCombat} title={inCombat?combatLockTitle:undefined} className={(g.screen===id||(id==='map'&&g.screen==='region'))?'active':''} onClick={()=>{if(inCombat)return;g.setScreen(id)}}><Icon size={18}/><span className="nav-tooltip">{inCombat?combatLockTitle:label}</span></button>)}</nav><div className="hud"><span className="hud-vital" title="Vida atual e vida máxima"><Heart className="heart" fill="currentColor"/><strong>{g.hp}/{maxHp(g)}</strong></span><span className="hud-resource hud-bag" title={`${g.equipmentBag.length} equipamentos guardados em ${capacity} espaços`}><Backpack size={17}/><strong>{g.equipmentBag.length}/{capacity}</strong></span><span className="hud-resource hud-gold" title={`${g.gold} moedas de ouro`}><Coins size={17}/><strong>{g.gold}</strong></span><span className="hud-level" title={`${g.xp} de experiência total`}><Sparkles size={16}/><small>NÍVEL</small><strong>{level}</strong></span><div className="brand">Bangalore's</div><div className="menu-dropdown-wrap" ref={menuRef}><button className="menu-mini" aria-haspopup="true" aria-expanded={menuOpen} onClick={()=>setMenuOpen(o=>!o)}><Menu size={18}/>Menu<ChevronDown size={14} className={'menu-caret'+(menuOpen?' open':'')}/></button>{menuOpen&&<div className="menu-dropdown" role="menu">
   {auth.user?.email&&<div className="menu-dropdown-account" title={auth.user.email}><Mail size={13}/><span>{auth.user.email}</span></div>}
   {inCombat&&<p className="menu-dropdown-notice">{combatLockTitle}</p>}
   <div className="menu-dropdown-screens">{nav.map(([id,label,Icon])=><button key={id} role="menuitem" disabled={inCombat} title={inCombat?combatLockTitle:undefined} className={g.screen===id?'active':''} onClick={()=>goTo(id)}><Icon size={16}/><span>{label}</span></button>)}</div>
...
function CoopScreen(){const g=useGame(),[name,setName]=React.useState(()=>localStorage.getItem('bangalores-coop-name')??''),[code,setCode]=React.useState(''),[userId,setUserId]=React.useState(''),[room,setRoom]=React.useState<OnlineRoom|null>(null),[members,setMembers]=React.useState<OnlineMember[]>([]),[onlineCount,setOnlineCount]=React.useState(0),[busy,setBusy]=React.useState(false),[notice,setNotice]=React.useState(''),channel=React.useRef<ReturnType<typeof subscribeToOnlineRoom>>(null);const refresh=React.useCallback(async(roomId:string)=>{try{const data=await loadOnlineRoom(roomId);setRoom(data.room);setMembers(data.members)}catch(error){setNotice(error instanceof Error?error.message:'Falha ao atualizar a sala.')}},[]);React.useEffect(()=>()=>{void unsubscribeFromOnlineRoom(channel.current)},[]);const connect=async(result:{room_id:string;room_code:string;user_id:string})=>{setUserId(result.user_id);localStorage.setItem('bangalores-coop-name',name.trim());await refresh(result.room_id);channel.current=subscribeToOnlineRoom(result.room_id,()=>void refresh(result.room_id),presence=>setOnlineCount(Object.keys(presence).length));setCode(result.room_code);setNotice('Sala conectada em tempo real.')};const create=async()=>{setBusy(true);setNotice('');try{await connect(await createOnlineRoom(name,g.heroId))}catch(error){setNotice(error instanceof Error?error.message:'Não foi possível criar a sala.')}finally{setBusy(false)}};const join=async()=>{setBusy(true);setNotice('');try{await connect(await joinOnlineRoom(code,name,g.heroId))}catch(error){setNotice(error instanceof Error?error.message:'Não foi possível entrar na sala.')}finally{setBusy(false)}};const leave=async()=>{if(!room)return;setBusy(true);try{await leaveOnlineRoom(room.id);await unsubscribeFromOnlineRoom(channel.current);channel.current=null;setRoom(null);setMembers([]);setOnlineCount(0);setNotice('Você saiu da sala.')}finally{setBusy(false)}};const me=members.find(member=>member.user_id===userId);return <div className="coop-page"><Panel className="coop-hero"><div><span className="eyebrow">BANGALORE'S ONLINE</span><h1>Cooperativo em tempo real</h1><p>Crie uma sala para dois aventureiros ou entre usando o código compartilhado por um amigo.</p></div>{onlineConfigured?<Wifi className="online"/>:<WifiOff className="offline"/>}</Panel>{!onlineConfigured?<Panel className="coop-setup" title="Configuração necessária"><WifiOff/><h2>Serviço online ainda não conectado</h2><p>A estrutura do modo cooperativo está instalada, mas as credenciais gratuitas do Supabase ainda precisam ser adicionadas ao ambiente do GitHub Pages.</p><code>VITE_SUPABASE_URL<br/>VITE_SUPABASE_ANON_KEY</code><p className="hint">Consulte <b>docs/COOP_SETUP.md</b> para ativar o serviço.</p></Panel>:!room?<div className="coop-entry"><Panel title="Seu aventureiro"><label>Nome visível<input maxLength={24} value={name} onChange={event=>setName(event.target.value)} placeholder="Nome do jogador"/></label><div className="coop-current-hero"><img src={'./'+cardArt(HEROES.find(hero=>hero.id===g.heroId)!)} alt=""/><span><small>HERÓI DESTA CAMPANHA</small><strong>{HEROES.find(hero=>hero.id===g.heroId)?.nome}</strong></span></div></Panel><Panel title="Criar ou entrar"><button className="primary" disabled={busy||!name.trim()} onClick={create}><Users/>Criar sala cooperativa</button><div className="coop-divider"><span/>OU<span/></div><label>Código da sala<input value={code} onChange={event=>setCode(normalizeRoomCode(event.target.value))} maxLength={6} placeholder="ABC123"/></label><button disabled={busy||!name.trim()||code.length!==6} onClick={join}>Entrar na sala</button>{notice&&<p className="coop-notice">{notice}</p>}</Panel></div>:<Panel className="coop-room"><header><div><small>CÓDIGO DA SALA</small><strong>{room.code}</strong></div><button onClick={()=>navigator.clipboard?.writeText(room.code)}><Copy/>Copiar código</button><span className="coop-live"><Wifi/> {onlineCount||members.length} online</span><button className="danger-action" disabled={busy} onClick={leave}><LogOut/>Sair</button></header><div className="coop-members">{members.map((member,index)=>{const hero=HEROES.find(item=>item.id===member.hero_id);return <article key={member.id}><span>{index+1}</span>{hero&&<img src={'./'+cardArt(hero)} alt=""/>}<div><small>{member.user_id===room.host_id?'LÍDER DA SALA':'AVENTUREIRO'}</small><strong>{member.display_name}</strong><em>{hero?.nome??'Herói não escolhido'}</em></div><b className={member.ready?'ready':''}>{member.ready?'PRONTO':'PREPARANDO'}</b></article>})}{members.length<room.max_players&&<article className="waiting"><Users/><strong>Aguardando outro jogador...</strong></article>}</div><footer><p>Presença, reconexão, membros, eventos e estado compartilhado já estão sincronizados. O próximo estágio conectará as ações de combate dos dois heróis ao estado da sala.</p><button className="primary" disabled={!me||busy} onClick={async()=>{if(!room||!me)return;setBusy(true);try{await setMemberReady(room.id,!me.ready,g.heroId);await refresh(room.id)}finally{setBusy(false)}}}>{me?.ready?'Cancelar prontidão':'Marcar como pronto'}</button></footer>{notice&&<p className="coop-notice">{notice}</p>}</Panel>}</div>}
...
const TUTORIAL_CHAPTERS=[
 ...
 ['Elementos e condições','Armas podem ter um elemento de ataque (físico, fogo, gelo, natureza, sombra ou arcano) e cada acerto tem uma chance de aplicar a condição daquele elemento no alvo: Sangrando (físico), Pegando fogo (fogo) e Envenenado (natureza) causam dano ao fim de cada turno por algumas rodadas; Congelado (gelo) e Agarrado (sombra) penalizam todas as rolagens de ataque e defesa do afetado; Atordoado (arcano) cancela a próxima defesa do alvo ou faz com que perca a próxima ação. Equipamentos de defesa podem ter resistência a um elemento, o que bloqueia a condição correspondente e reduz o dano recebido daquele tipo de ataque. Essas propriedades nunca vêm da Loja: só surgem ao forjar um item com sucesso (o elemento reflete a afinidade da sua classe) ou ao derrotar um chefe, que às vezes derruba equipamento já imbuído com o elemento da região.'],
 ...
 ['Equipamentos, classes e conjuntos','Cada slot aceita um tipo de item. Armas favorecem classes específicas e podem aplicar penalidade fora da afinidade; equipamentos exclusivos não podem ser usados por outra classe. Compare atributos e habilidades antes de substituir um item. Conjuntos, elementos, condições e pedras ampliam as combinações.'],
 ['Comércio e carrinho','Na Loja, filtre por categoria e adicione itens ao carrinho. Ouro e produtos só são transferidos após Confirmar compra. O sistema valida dinheiro, nível, classe e espaço da bolsa. No modo de venda, a transação continua direta.'],
 ['Forja, receitas e experiência','Desmonte equipamentos para obter materiais físicos, mágicos e pedras. Receitas informam requisitos, materiais ausentes, nível de forjador e chance de sucesso. Toda tentativa concede experiência de Forja; falhas consomem parte dos materiais. Níveis altos liberam receitas melhores.'],
 ['Aprimoramentos e encaixes','Equipamentos podem ser aprimorados e receber pedras dentro do limite de encaixes. Pedras concedem bônus especializados. Desmontar ou perder o item também remove aprimoramentos, pedras e efeitos forjados vinculados a ele.'],
 ...
 return <article className={canCraft?'ready':locked?'forge-locked':''}>
  ...
  <div>
   ...
   {recipe.attributeChoice&&<div className="forge-attribute-picker">
    <button type="button" className={!choice?'active':''} onClick={()=>setChoice(undefined)}>Sem bônus</button>
    {(['ataque','defesa','vida'] as ForgeAttribute[]).map(stat=>{const option=FORGE_GEMS.find(x=>x.stat===stat)!;return <button type="button" key={stat} className={choice===stat?'active':''} onClick={()=>setChoice(stat)}>{option.texto}</button>})}
    {(Object.keys(FORGE_BONUS_LABELS) as ForgeBonus[]).map(bonus=><button type="button" key={bonus} className={choice===bonus?'active':''} onClick={()=>setChoice(bonus)}>{FORGE_BONUS_LABELS[bonus]}</button>)}
   </div>}
  </div>
...
function MaterialSourceDialog({material,onClose}:{material:{id:string;nome:string;elemento:GameElement;regionId:string};onClose:()=>void}){
 const region=TERRITORIES.find(t=>t.id===material.regionId)
 const subs=SUBREGIONS.filter(s=>s.regionId===material.regionId)
 return <div className="hero-confirm-overlay" role="presentation" onClick={onClose}><section className="material-info-dialog" role="dialog" aria-modal="true" aria-labelledby="material-info-title" onClick={e=>e.stopPropagation()}>
  ...
  <p>Este material não é obtido desmontando equipamentos. Ele é conseguido derrotando inimigos em <b>{region?.nome??'uma região desconhecida'}</b>: inimigos comuns concedem 1 unidade, elites 2 e chefes 3 por vitória.</p>
...
function ForgeScreen(){const g=useGame(),materials=Object.entries(REGION_MATERIALS).map(([regionId,m])=>({...m,regionId})),mastery=forgeLevelInfo(g.forgeXp??0),rate=(g.forgeAttempts??0)?Math.round((g.forgeSuccesses??0)/(g.forgeAttempts??1)*100):0;const [materialInfo,setMaterialInfo]=React.useState<typeof materials[number]|undefined>(undefined);return <div className="forge-page"><Panel className="forge-header"><span className="eyebrow">OFICINA DE HAVENDOWN</span><h1>Forja</h1><p>Transforme espólios em materiais, produza itens por receita e instale pedras de melhoria.</p><div className="forge-mastery"><div><small>NÍVEL DE FORJADOR</small><strong>{mastery.level}</strong></div><div className="forge-xp"><span>{mastery.max?'Maestria máxima':`${mastery.progress}/${mastery.next} XP`}</span><div className="xp-track"><div style={{width:`${mastery.max?100:Math.min(100,mastery.progress/mastery.next*100)}%`}}/></div></div><div><small>SUCESSOS</small><strong>{g.forgeSuccesses??0}/{g.forgeAttempts??0}</strong><span>{rate}% de êxito</span></div></div><p className="forge-warning">Toda tentativa concede XP de Forja. Falhas consomem metade dos materiais; níveis maiores liberam receitas poderosas e aumentam a chance de sucesso.</p><ForgeResultBanner/></Panel><Panel title="Materiais e fabricação"><div className="material-grid">{materials.map(m=><button key={m.id} type="button" onClick={()=>setMaterialInfo(m)} title="Ver onde conseguir este material"><b>{g.materials[m.id]??0}</b><small>{m.nome}</small></button>)}</div><div className="forge-items">{g.equipmentBag.slice(0,12).map((id,index)=>{const item=EQUIPMENT.find(e=>e.id===id);return item&&<article key={`${id}-${index}`}><span><strong>{item.nome} +{g.equipmentUpgrades[id]??0}</strong><small>Aprimorar: {10+((g.equipmentUpgrades[id]??0)+1)*10} ouro</small></span><button disabled={(g.equipmentUpgrades[id]??0)>=3} onClick={()=>g.upgradeEquipment(id)}>Aprimorar</button></article>})}</div></Panel><RecipeCatalog/><ForgeSalvagePanel/>{materialInfo&&<MaterialSourceDialog material={materialInfo} onClose={()=>setMaterialInfo(undefined)}/>}</div>}
...
function MapGuildMissions(){const g=useGame();const missions=GUILD_MISSIONS.filter(m=>g.guildAccepted.includes(m.id)&&!g.guildClaimed.includes(m.id));if(!missions.length)return <section className="map-mission-empty"><strong>Missão ativa</strong><Trophy/><p>Nenhum contrato ativo.</p><button onClick={()=>g.setScreen('guild')}>Visitar a Guilda</button></section>;return <section className="map-active-missions"><div className="map-mission-title"><strong>Missões ativas</strong><span>{missions.length}</span></div>{missions.map(m=>{const progress=m.tipo==='delivery'&&m.itemId?(g.equipmentBag.includes(m.itemId)?1:0):Math.min(m.quantidade,g.guildProgress[m.id]??0),ready=progress>=m.quantidade;return <article className={ready?'ready':''} key={m.id}><header><Trophy/><div><small>{ready?'OBJETIVO CONCLUÍDO':'CONTRATO DA GUILDA'}</small><strong>{m.nome}</strong></div></header><p>{m.descricao}</p><div className="map-mission-progress"><span>{m.tipo==='delivery'?'Entrega':'Progresso'} <b>{progress}/{m.quantidade}</b></span><div className="xp-track"><div style={{width:`${Math.min(100,progress/m.quantidade*100)}%`}}/></div></div><footer><span>{m.recompensa.tipo==='gold'?`${m.recompensa.valor} ouro`:'Equipamento compatível'}</span>{ready&&<button onClick={()=>g.setScreen('guild')}>Resgatar na Guilda</button>}</footer></article>})}<button className="map-mission-guild-link" onClick={()=>g.setScreen('guild')}>Abrir Guilda</button></section>}
...
function CharacterScreen(){const g=useGame();const h=HEROES.find(x=>x.id===g.heroId)!;const li=levelInfo(g.xp),permanentLife=Math.max(0,g.attr.vida-g.allocatedAttr.vida);return <div className="char-grid"><Panel className="portrait-panel"><ArtPreview className="portrait" image={cardArt(h)} name={h.nome} text={h.habilidade} stats={`Ataque ${attackValue(g)} • Defesa ${defenseValue(g)} • Vida ${maxHp(g)}`}/><h1>{h.nome}</h1><p>{h.habilidade}</p><div className="points-box">Pontos disponíveis <strong>{g.attributePoints}</strong></div></Panel><Panel title="Atributos"><AttrRow label="Vida" value={`${g.hp}/${maxHp(g)}`} n={g.allocatedAttr.vida} detail={permanentLife?`+${permanentLife} de bônus permanente`:undefined} onPlus={()=>g.addAttribute('vida')} disabled={!g.attributePoints}/><AttrRow label="Ataque" value={attackValue(g)} n={g.allocatedAttr.ataque} detail={g.pendingAttackBonus?`+${g.pendingAttackBonus} temporário até o fim do combate`:undefined} onPlus={()=>g.addAttribute('ataque')} disabled={!g.attributePoints}/><AttrRow label="Defesa" value={defenseValue(g)} n={g.allocatedAttr.defesa} onPlus={()=>g.addAttribute('defesa')} disabled={!g.attributePoints}/><h3 className="subhead">Progressão</h3><Stat label="Nível" value={li.lvl}/><div className="xp-track"><div style={{width:`${Math.min(100,li.progress/li.next*100)}%`}}/></div><Stat label="XP do nível" value={`${li.progress}/${li.next}`}/><Stat label="XP necessária para o próximo nível" value={li.next-li.progress}/><Stat label="Experiência total" value={g.xp}/><Stat label="Ouro" value={g.gold}/></Panel><TalentPanel/></div>}
function BestiaryPanel(){const g=useGame();return <Panel title="Bestiário"><div className="bestiary-list">{Object.entries(g.bestiary).length?Object.entries(g.bestiary).map(([name,r])=><span key={name}><strong>{name}</strong><small>{r.encontros} encontros • {r.vitorias} vitórias</small></span>):<p className="muted">Enfrente criaturas para revelar seus registros.</p>}</div></Panel>}
...
function EquipmentScreen(){const g=useGame();const capacity=equipmentBagCapacity(g);const dualWielding=equipmentWeaponClass(EQUIPMENT.find(x=>x.id===g.equipped.mao_direita))==='facas'
 const itemBonus=equipmentStatBonus(g)
 const elementalEntries=(Object.values(g.equipped) as (string|undefined)[]).filter((id):id is string=>Boolean(id)).map(id=>{const el=g.equipmentElements[id],res=g.equipmentResistances[id];if(!el&&!res)return null;const e=EQUIPMENT.find(x=>x.id===id);return e?{id,name:e.nome,kind:el?'Elemento':'Resistência',value:ELEMENT_LABELS[el??res!]}:null}).filter((x):x is {id:string;name:string;kind:string;value:string}=>Boolean(x))
 return <div className="equipment-layout"><Panel title="Slots equipados"><div className="slot-grid">{SLOT_ORDER.map((slot)=>{const id=g.equipped[slot],e=EQUIPMENT.find(x=>x.id===id);if(!e)return <button key={slot} className={'slot '+(slot==='botas'?'boots':'')}><span>{slotNames[slot]}</span><div className="slot-empty">{slot==='mao_esquerda'&&dualWielding?'Ocupada pelas facas':'Vazio'}</div></button>;const effective=equipmentAttackForHero(e,g.heroId);const affinity=compatibilityLabel(e,g.heroId);const bagSlot=slot==='bolsa';const stats=bagSlot?`${e.capacidade??8} espaços`:`Ataque +${effective} • Defesa +${e.defesa} • Vida +${e.vida}`;return <button key={slot} className={'slot '+(slot==='botas'?'boots':'')+(bagSlot?' backpack-slot':'')} onClick={bagSlot?undefined:()=>g.unequip(slot)}><span>{slotNames[slot]}</span><ArtPreview className="slot-art-preview" image={cardArt(e)} name={e.nome} text={`${e.habilidade} • ${affinity}${elementalNote(g,e.id)}`} stats={stats}/><div className="slot-info"><strong>{e.nome}</strong><div className="slot-stats">{bagSlot?`CAPACIDADE ${e.capacidade??8}`:<>ATQ +{effective}{effective!==e.ataque?` (base +${e.ataque})`:''} • DEF +{e.defesa} • VIDA +{e.vida}</>}</div><small className="slot-effect">{affinity}{elementalNote(g,e.id)}</small></div><small className="slot-remove">{bagSlot?'equipe outra bolsa para trocar':'clique para retirar'}</small></button>})}</div></Panel><div className="equipment-side-col"><Panel title="Atributos e bônus" className="equipment-attr-panel"><Stat label="Vida máxima" value={`(${maxHp(g)-itemBonus.life}+${itemBonus.life})=${maxHp(g)}`}/><Stat label="Ataque" value={`(${attackValue(g)-itemBonus.atk}+${itemBonus.atk})=${attackValue(g)}`}/><Stat label="Defesa" value={`(${defenseValue(g)-itemBonus.def}+${itemBonus.def})=${defenseValue(g)}`}/><hr/><div className="equipment-elemental-list">{elementalEntries.length?elementalEntries.map(entry=><div key={entry.id} className="equipment-elemental-row"><span>{entry.name}</span><b className={entry.kind==='Elemento'?'elemental-attack':'elemental-resist'}>{entry.kind}: {entry.value}</b></div>):<p className="muted">Nenhum bônus elemental equipado — forje um item com sucesso ou derrote um chefe para conseguir um.</p>}</div></Panel><Panel title={`Equipamentos guardados ${g.equipmentBag.length}/${capacity}`}><div className="item-grid compact">{g.equipmentBag.map((id,idx)=>{const e=EQUIPMENT.find(x=>x.id===id)!;const effective=equipmentAttackForHero(e,g.heroId);const allowed=equipmentClassAllowed(e,g.heroId);const levelAllowed=equipmentLevelAllowed(e,g.xp);const required=equipmentRequiredLevel(e);const fits=e.slot!=='bolsa'||g.equipmentBag.length<=(e.capacidade??8);const dualLocked=e.slot==='mao_esquerda'&&equipmentWeaponClass(EQUIPMENT.find(x=>x.id===g.equipped.mao_direita))==='facas';const equipLabel=!allowed?'Impossível equipar':!levelAllowed?`Requer nível ${required}`:!fits?`Reduza para ${e.capacidade} itens`:dualLocked?'Facas ocupam as duas mãos':'Equipar';const stats=e.slot==='bolsa'?`Capacidade ${e.capacidade??8} espaços`:`Ataque +${effective}${effective!==e.ataque?` (base +${e.ataque})`:''} • Defesa +${e.defesa} • Vida +${e.vida}`;return <ItemCard key={id+idx} image={cardArt(e)} rarity={cardRarity(e,'Equipamento')} name={e.nome} subtitle={e.slot==='bolsa'?`${e.capacidade} espaços`:slotNames[e.slot]} footer={`${e.habilidade} • ${compatibilityLabel(e,g.heroId)}${elementalNote(g,id)}`} previewStats={stats}><button className={!allowed||!fits||dualLocked?'equip-impossible':!levelAllowed?'equip-level-locked':''} disabled={!allowed||!levelAllowed||!fits||dualLocked} title={!allowed?compatibilityLabel(e,g.heroId):!levelAllowed?`Disponível no nível ${required}`:!fits?'Há equipamentos demais para esta bolsa':dualLocked?'Combate com facas exige as duas mãos livres':undefined} onClick={()=>g.equip(id)}>{equipLabel}</button></ItemCard>})}</div></Panel></div></div>}
function ItemCard({image,name,subtitle,footer,previewStats,previewAllowEquip=false,rarity='comum',children}:{image:string;name:string;subtitle?:string;footer?:string;previewStats?:string;previewAllowEquip?:boolean;rarity?:Rarity;children?:React.ReactNode}){const equipment=EQUIPMENT.find(item=>item.nome===name);const emblem=equipment?cardEmblem(equipment,'Equipamento'):undefined;const owner=equipment?.classeExclusiva??(equipment?equipmentAffinity(equipment):undefined);const emblemLabel=owner?classNames[owner]:'Universal';return <article className={`item-card item-rarity-${rarity}`}><div className="item-art-wrap"><ArtPreview image={image} name={name} text={footer} stats={previewStats??subtitle} compareEquipment={Boolean(equipment)} allowEquip={previewAllowEquip}/>{emblem&&<img className="item-class-emblem" src={'./'+emblem} alt={emblemLabel} title={`Classe: ${emblemLabel}`}/>}</div><div className="item-copy"><div className="item-title-row"><strong>{name}</strong><span className={`mini-rarity rarity-${rarity}`}>{rarityLabel[rarity]}</span></div>{subtitle&&<span>{subtitle}</span>}{footer&&<small>{footer}</small>}{children}</div></article>}
function ShopScreen(){
 const g=useGame()
 ...
 const valid=count>0&&total<=g.gold&&g.equipmentBag.length+equipmentCount<=equipmentBagCapacity(g)&&lines.every(line=>line.kind==='c'||(equipmentClassAllowed(line.item as any,g.heroId)&&equipmentLevelAllowed(line.item as any,g.xp)))
...
function ShopRow({image,name,rarity='comum',emblem,emblemLabel,meta,description,price,afford=true,actionLabel,actionDisabled=false,actionTitle,onAction,previewStats,compareEquipment=false}:{image:string;name:string;rarity?:Rarity;emblem?:string;emblemLabel?:string;meta:string;description:string;price:number;afford?:boolean;actionLabel:string;actionDisabled?:boolean;actionTitle?:string;onAction:()=>void;previewStats?:string;compareEquipment?:boolean}){
 return <article className={`shop-row item-rarity-${rarity}${afford?'':' unaffordable'}`}>
...
function CombatScreen(){
 ...
 const useCoopItemSkill=()=>{if(g.itemSkillUsed||!myTurn)return;const item=itemAbilities[0];if(!item)return;const txt=item.habilidade.toLowerCase()
  ...
  void coop.coopAttack(attackValue(g)+3+bossBonus,Math.max(0,(e.dificuldade??1)-2),rollBonus,critBoost,heal.chance,heal.amount,item.nome,undefined,false,critChancePct,critDamageBonusPct,heroWeaponElement(g))
 ...
 const statusKindsOf=(status:any)=>(['bleed','burn','poison','frozen','grabbed','stunned'] as const).filter(k=>status?.[k])
...
function GuildScreen(){
 ...
 <div className="guild-mission-grid">{GUILD_MISSIONS.map(m=>{const accepted=g.guildAccepted.includes(m.id),claimed=g.guildClaimed.includes(m.id),progress=missionProgress(m),ready=accepted&&progress>=m.quantidade,equipmentReward=m.recompensa.tipo==='equipment',deliveryItem=m.itemId?EQUIPMENT.find(e=>e.id===m.itemId):undefined,deliveryEquippedOnly=m.tipo==='delivery'&&Boolean(m.itemId)&&!g.equipmentBag.includes(m.itemId!)&&Object.values(g.equipped).includes(m.itemId!),requiredRank=GUILD_RANKS.find(r=>r.id===m.rank)!,locked=rankIndex<GUILD_RANKS.findIndex(r=>r.id===m.rank);const claimMission=()=>{if(deliveryEquippedOnly&&!window.confirm(`${deliveryItem?.nome} está equipado em um herói. Ao entregá-lo você perderá este equipamento e seus bônus enquanto ele estiver equipado. Deseja continuar?`))return;g.claimGuildMission(m.id)};return <article className={`guild-mission${claimed?' claimed':ready?' ready':accepted?' active':locked?' locked':''}`} key={m.id}><header><span>{locked?<Shield/>:m.tipo==='delivery'?<Package/>:m.tipo==='boss'?<Trophy/>:m.tipo==='specific'?<Sword/>:<Shield/>}</span><div><small>{m.tipo==='delivery'?'CONTRATO DE ENTREGA':m.tipo==='boss'?'CONTRATO DE CHEFE':m.tipo==='specific'?'CAÇA ESPECÍFICA':'MISSÃO DE CAÇA'}</small><h2>{m.nome}</h2></div><b>{'◆'.repeat(m.dificuldade)}</b></header><span className="guild-required-rank" style={{'--rank-color':requiredRank.cor} as React.CSSProperties}>Rank mínimo: <strong>{requiredRank.nome}</strong> · +{m.dificuldade} reputação</span><p>{m.descricao}</p>{deliveryItem&&<span className="guild-delivery-item"><Package/>Item solicitado: <strong>{deliveryItem.nome}</strong>{progress>0?(deliveryEquippedOnly?<em>Equipado — será perdido ao entregar</em>:<em>Disponível para entrega</em>):<em>Não está na bolsa</em>}</span>}{m.local&&<span className="guild-location"><Map/>Região indicada: {m.local}</span>}{m.destinoId&&<button className="guild-fast-travel" disabled={locked} title={locked?`Alcance o rank ${requiredRank.nome} para viajar para esta missão.`:`Viajar para ${m.local}`} onClick={()=>g.openSubregion(m.destinoId!)}><Map/><span><small>VIAGEM RÁPIDA</small><strong>{locked?'Destino bloqueado':m.local}</strong></span><ArrowRight/></button>}<div className="guild-progress"><div><span>{m.tipo==='delivery'?'Item na bolsa':'Progresso'}</span><strong>{progress}/{m.quantidade}</strong></div><div className="xp-track"><div style={{width:`${progress/m.quantidade*100}%`}}/></div></div><div className="guild-reward"><span>{equipmentReward?<Package/>:<Coins/>}<small>RECOMPENSA</small><strong>{equipmentReward?'Equipamento compatível':`${m.recompensa.valor} moedas de ouro`}</strong></span>{claimed?<button disabled>Concluída</button>:locked?<button className="rank-locked" disabled>Requer rank {requiredRank.nome}</button>:!accepted?<button onClick={()=>g.acceptGuildMission(m.id)}>Aceitar missão</button>:ready?<button className="primary" disabled={equipmentReward&&bagFull} title={equipmentReward&&bagFull?'Libere espaço nos equipamentos guardados.':deliveryEquippedOnly?'Este item está equipado e será perdido ao entregar.':''} onClick={claimMission}>{equipmentReward&&bagFull?'Bolsa cheia':m.tipo==='delivery'?'Entregar item':'Resgatar recompensa'}</button>:<button disabled>{m.tipo==='delivery'?'Item necessário':'Em andamento'}</button>}</div></article>})}</div></div>
}
