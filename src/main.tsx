...
function EquipmentScreen(){const g=useGame();return <div className="equipment-layout"><Panel title="Slots equipados"><div className="slot-grid">{SLOT_ORDER.map((slot)=>{const id=g.equipped[slot],e=EQUIPMENT.find(x=>x.id===id);if(!e)return <button key={slot} className={'slot '+(slot==='botas'?'boots':'')}><span>{slotNames[slot]}</span><div className="slot-empty">Vazio</div></button>;const effective=equipmentAttackForHero(e,g.heroId);const affinity=compatibilityLabel(e,g.heroId);return <button key={slot} className={'slot '+(slot==='botas'?'boots':'')} onClick={()=>g.unequip(slot)}><span>{slotNames[slot]}</span><ArtPreview className="slot-art-preview" image={cardArt(e)} name={e.nome} text={`${e.habilidade} • ${affinity}`} stats={`Ataque +${effective} • Defesa +${e.defesa} • Vida +${e.vida}`}/><div className="slot-info"><strong>{e.nome}</strong><div className="slot-stats">ATQ +{effective}{effective!==e.ataque?` (base +${e.ataque})`:''} • DEF +{e.defesa} • VIDA +{e.vida}</div><small className="slot-effect">{affinity}</small></div><small className="slot-remove">clique para retirar</small></button>})}</div></Panel><Panel title={`Equipamentos guardados ${g.equipmentBag.length}/8`}><div className="item-grid compact">{g.equipmentBag.map((id,idx)=>{const e=EQUIPMENT.find(x=>x.id===id)!;const effective=equipmentAttackForHero(e,g.heroId);const allowed=equipmentClassAllowed(e,g.heroId);const levelAllowed=equipmentLevelAllowed(e,g.xp);const required=equipmentRequiredLevel(e);const equipLabel=!allowed?'Impossível equipar':!levelAllowed?`Requer nível ${required}`:'Equipar';return <ItemCard key={id+idx} image={cardArt(e)} rarity={cardRarity(e,'Equipamento')} name={e.nome} subtitle={slotNames[e.slot]} footer={`${e.habilidade} • ${compatibilityLabel(e,g.heroId)}`} previewStats={`Ataque +${effective}${effective!==e.ataque?` (base +${e.ataque})`:''} • Defesa +${e.defesa} • Vida +${e.vida}`}><button className={!allowed?'equip-impossible':!levelAllowed?'equip-level-locked':''} disabled={!allowed||!levelAllowed} title={!allowed?compatibilityLabel(e,g.heroId):!levelAllowed?`Disponível no nível ${required}`:undefined} onClick={()=>g.equip(id)}>{equipLabel}</button></ItemCard>})}</div></Panel></div>}
