...
function MaterialSourceDialog({material,onClose}:{material:ForgeMaterialEntry;onClose:()=>void}){
 ...
 return <div className="hero-confirm-overlay" role="presentation" onClick={onClose}><section className="material-info-dialog" role="dialog" aria-modal="true" aria-labelledby="material-info-title" onClick={e=>e.stopPropagation()}>
  ...
  {material.kind==='region'&&<p>Este material não é obtido desmontando equipamentos. Ele é conseguido derrotando inimigos em <b>{region?.nome??'uma região desconhecida'}</b>: inimigos comuns concedem 1 unidade, elites 2 e chefes 3 por vitória.</p>}
  {material.kind==='dismantle'&&<p>Obtido desmontando equipamentos na Oficina de desmontagem (mais abaixo). Qualquer peça rende uma quantidade; peças de raridade maior e slots mais pesados rendem mais.</p>}
  {material.kind==='gem'&&<p>{material.texto} — obtida ao desmontar equipamentos (a chance aumenta com a raridade e o número de encaixes da peça) ou recuperada ao desmontar uma peça que já tinha esta pedra instalada. Usada para instalar bônus de atributo nos encaixes de uma peça, ou para refinar o bônus de atributo/efeito especial de uma receita na Forja.</p>}
...
function ForgeTutorialPanel(){
 ...
 return <section className="panel forge-tutorial">
  ...
  {open&&<div className="mechanics-grid forge-tutorial-grid">
   ...
   <div className="mechanics-card"><small>6. DEPOIS DE FORJAR</small><p>Na Oficina de desmontagem você recicla itens indesejados em materiais e instala pedras extras nos encaixes livres dos equipamentos ativos — pedras removidas são destruídas, não devolvidas. Aprimoramentos (+1 a +3) custam ouro e materiais, e reforçam os atributos base do item — mas não são garantidos: +1 tem boa chance de sucesso, +2 é raro e +3 é uma aposta; falhar em +2 ou +3 arrisca regredir o nível da peça.</p></div>
  </div>}
