 ...
 return <div className="combat-page premium-combat combat-v033">
   ...
   <Panel className="combat-consumables-panel combat-consumables-area">
      ...
      <div className="combat-consumables">
       {consumables.length?consumables.map(({item,qty})=>{const desc=consumableDescription(item,g);return <article key={item.id} className={`combat-consumable rarity-${cardRarity(item,'Consumível')}`} title={desc}><span className="consumable-qty">{qty}</span><div className="combat-consumable-art"><ArtPreview image={cardArt(item)} name={item.nome} text={desc} stats={`${item.tipo} • Valor ${consumableEffectiveValue(item,g)} • Quantidade ${qty}`}/></div><strong>{item.nome}</strong><small>{desc}</small><button disabled={disabled} onClick={()=>performUseConsumable(item.id)}>USAR</button></article>}):<div className="consumables-empty"><FlaskConical/><span>Seus consumíveis aparecerão aqui durante o combate.</span></div>}
      </div>
   </Panel>
