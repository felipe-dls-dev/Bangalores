...
function GuildHerald(){
 ...
 const ready=missions.filter(m=>g.guildAccepted.includes(m.id)&&!g.guildClaimed.includes(m.id)&&guildMissionProgress(g,m)>=m.quantidade)
 const suggestions=ready.length?ready:missions.filter(m=>!g.guildAccepted.includes(m.id)&&!g.guildClaimed.includes(m.id)&&rankIndex>=GUILD_RANKS.findIndex(r=>r.id===m.rank)).slice(0,3)
...
function GuildScreen(){
 ...
 const visibleMissions=GUILD_MISSIONS.filter(m=>GUILD_MISSION_CATEGORIES.find(c=>c.id===category)!.match(m.tipo))
