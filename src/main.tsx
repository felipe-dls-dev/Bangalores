...
function useCardTilt(enabled:boolean){
 ...
 React.useEffect(()=>{
  ...
  const onMove=(event:PointerEvent)=>{
   ...
   el.style.setProperty('--tilt-ry',`${((px-.5)*14).toFixed(2)}deg`)
   el.style.setProperty('--tilt-gx',`${(px*100).toFixed(1)}%`)
   el.style.setProperty('--tilt-gy',`${(py*100).toFixed(1)}%`)
  }
  const onLeave=()=>{el.style.setProperty('--tilt-rx','0deg');el.style.setProperty('--tilt-ry','0deg')}
