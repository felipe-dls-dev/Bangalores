...
import { energyAt } from './store/heroStats'
...
const TUTORIAL_CHAPTERS=[
 ...
 ['Combate e turnos','Em seu turno, ataque, use a habilidade do herói, ative uma habilidade de equipamento, consuma um item, assuma a postura defensiva ou tente fugir. A postura defensiva concede +20% de Armadura (e −20% de Poder de ataque) contra qualquer golpe (incluindo capangas) e dura até o fim da batalha ou até ser desativada; a primeira ativação de cada batalha não consome o turno, então você ainda pode agir depois — reativá-la mais tarde já custa o turno normalmente. Rolagens 6 de ataque ou defesa acumulam Fervor; ao chegar a 3, o Fervor de Combate gasta o medidor em um ataque crítico garantido. Buffs e debuffs de habilidades de herói (como Ímpeto Marcial, Ascensão Arcana e Marca do Predador) duram no máximo 3 turnos consecutivos — Marca do Predador dura 2 — e se dissipam sozinhos ao final desse período. Chefes podem mudar de fase, convocar capangas e fazer cada capanga atacar em seu próprio turno.'],
...
function CombatScreen(){
 ...
 const energyClock=isCoop?Number(battle?.round??1):g.combatTurn
 const energyMaxNow=championStats(g).energiaMaxima
 const energyAvailable=energyAt({energy:g.energy??energyMaxNow,turn:g.energyTurn??energyClock},energyClock,energyMaxNow,ATTRIBUTE_RULES.energia.regeneracaoPorRodada)
 const skillEnergyCost=heroSkillEnergyCost(g.heroId)
 const hasSkillEnergy=energyAvailable>=skillEnergyCost
 const spendCoopEnergy=()=>{useGame.setState({energy:Math.max(0,energyAvailable-skillEnergyCost),energyTurn:energyClock})}
   ...
   <Panel title="Ações" className="combat-actions-panel combat-actions-area">
      ...
      <div className="combat-actions-grid">
       ...
       {g.heroId==='conjurador'&&heroSkillUses<2&&currentSummons.length<2
        ...
        :<button className="premium-action" disabled={disabled||g.heroId==='conjurador'||heroSkillUses>=heroSkillLimit||((g.heroSkillCooldown??0)>0)||!hasSkillEnergy} title={g.heroId==='conjurador'?'Você já mantém duas feras espectrais nesta batalha.':(g.heroSkillCooldown??0)>0?`Recarga: ${g.heroSkillCooldown} turnos`:!hasSkillEnergy?`Energia insuficiente: ${Math.floor(energyAvailable)}/${skillEnergyCost}. Regenera ${ATTRIBUTE_RULES.energia.regeneracaoPorRodada} por rodada.`:`Custa ${skillEnergyCost} de Energia.`} onClick={()=>isCoop?useCoopHeroSkill():g.heroSkill()}><Sparkles/>{g.heroId==='conjurador'?'Limite de feras atingido':heroSkillNames[g.heroId??'']??'Habilidade do herói'}{(g.heroSkillCooldown??0)>0?` (${g.heroSkillCooldown}t)`:g.heroId==='conjurador'?` (${Math.min(heroSkillUses,heroSkillLimit)}/${heroSkillLimit})`:` (${skillEnergyCost} EN)`}</button>}
