...
function EquipmentRulesPanel(){const g=useGame(),sets=equipmentSetCounts(g),weapon=EQUIPMENT.find(e=>e.id===g.equipped.mao_direita),equipped=(Object.values(g.equipped) as string[]).map(id=>EQUIPMENT.find(e=>e.id===id)).filter(Boolean);return <Panel className="build-summary" title="Resumo completo da build"><div className="build-summary-grid"><span><small>ATAQUE FINAL</small><strong>{attackValue(g)}</strong></span><span><small>DEFESA FINAL</small><strong>{defenseValue(g)}</strong></span><span><small>VIDA MÁXIMA</small><strong>{maxHp(g)}</strong></span><span><small>ELEMENTO</small><strong>{heroWeaponElement(g)}</strong></span></div><h3>Conjuntos</h3><div className="build-tags">{SET_BONUSES.map((set,index)=>{const count=[sets.lua,sets.cinzas,sets.khar,sets.eclipse][index];return <span className={count>=2?'active':''} key={set.nome}><b>{set.nome}</b> {count}/4 • {count>=4?set.four:count>=2?set.two:'bônus inativo'}</span>})}</div><h3>Efeitos equipados</h3><div className="build-tags"><span>Arma: {weapon?.nome??'nenhuma'} ({heroWeaponElement(g)})</span>{heroResistances(g).map(r=><span key={r}>Resistência: {r}</span>)}{equipped.filter(e=>e?.activeEffect).map(e=><span key={e!.id}>{e!.nome}: {e!.activeEffect!.description}</span>)}</div><details><summary>Regras elementais</summary><p><b>Elementos:</b> {ELEMENTS.join(' • ')}</p><p><b>Condições:</b> {STATUS_INFO.map(s=>`${s[0]}: ${s[1]}`).join(' • ')}</p></details></Panel>}
...
function ForgeTutorialPanel(){
 const [open,setOpen]=React.useState(false),g=useGame(),weapon=EQUIPMENT.find(e=>e.id===g.equipped.mao_direita)
