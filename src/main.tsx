...
function GuildScreen(){const g=useGame();const active=g.guildAccepted.filter(id=>!g.guildClaimed.includes(id)).length;const completed=GUILD_MISSIONS.filter(m=>g.guildAccepted.includes(m.id)&&!g.guildClaimed.includes(m.id)&&(g.guildProgress[m.id]??0)>=m.quantidade).length;const bagFull=g.equipmentBag.length>=equipmentBagCapacity(g);return <div className="guild-page"><Panel className="guild-head"><button onClick={()=>g.setScreen('map')}><ArrowLeft/>Voltar ao mapa</button><div><span className="eyebrow">SALÃO DOS AVENTUREIROS</span><h1>Guilda de Eldravar</h1><p>Aceite contratos, derrote os alvos durante suas viagens e retorne para receber a recompensa.</p></div><Shield className="guild-crest"/></Panel><div className="guild-summary"><span><ScrollText/><small>MISSÕES ATIVAS</small><strong>{active}</strong></span><span><Trophy/><small>PRONTAS PARA RESGATE</small><strong>{completed}</strong></span><span><Package/><small>ESPAÇO NA BOLSA</small><strong>{g.equipmentBag.length}/{equipmentBagCapacity(g)}</strong></span></div>{g.guildNotice&&<div className="guild-notice"><Sparkles/>{g.guildNotice}</div>}<div className="guild-mission-grid">{GUILD_MISSIONS.map(m=>{const accepted=g.guildAccepted.includes(m.id),claimed=g.guildClaimed.includes(m.id),progress=Math.min(m.quantidade,g.guildProgress[m.id]??0),ready=accepted&&progress>=m.quantidade;const equipmentReward=m.recompensa.tipo==='equipment';return <article className={`guild-mission${claimed?' claimed':ready?' ready':accepted?' active':''}`} key={m.id}><header><span>{m.tipo==='boss'?<Trophy/>:m.tipo==='specific'?<Sword/>:<Shield/>}</span><div><small>{m.tipo==='boss'?'CONTRATO DE CHEFE':m.tipo==='specific'?'CAÇA ESPECÍFICA':'MISSÃO DE CAÇA'}</small><h2>{m.nome}</h2></div><b>{'◆'.repeat(m.dificuldade)}</b></header><p>{m.descricao}</p>{m.local&&<span className="guild-location"><Map/>Região indicada: {m.local}</span>}<div className="guild-progress"><div><span>Progresso</span><strong>{progress}/{m.quantidade}</strong></div><div className="xp-track"><div style={{width:`${progress/m.quantidade*100}%`}}/></div></div><div className="guild-reward"><span>{equipmentReward?<Package/>:<Coins/>}<small>RECOMPENSA</small><strong>{equipmentReward?'Equipamento compatível':`${m.recompensa.valor} moedas de ouro`}</strong></span>{claimed?<button disabled>Concluída</button>:!accepted?<button onClick={()=>g.acceptGuildMission(m.id)}>Aceitar missão</button>:ready?<button className="primary" disabled={equipmentReward&&bagFull} title={equipmentReward&&bagFull?'Libere espaço nos equipamentos guardados.':''} onClick={()=>g.claimGuildMission(m.id)}>{equipmentReward&&bagFull?'Bolsa cheia':'Resgatar recompensa'}</button>:<button disabled>Em andamento</button>}</div></article>})}</div></div>}
