...
function EquipmentScreen(){const g=useGame();const capacity=equipmentBagCapacity(g);const dualWielding=equipmentWeaponClass(EQUIPMENT.find(x=>x.id===g.equipped.mao_direita))==='facas';return <div className="equipment-layout"><Panel title="Slots equipados"><div className="slot-grid">{SLOT_ORDER.map((slot)=>{const id=g.equipped[slot],e=EQUIPMENT.find(x=>x.id===id);if(!e)return <button key={slot} className={'slot '+(slot==='botas'?'boots':'')}><span>{slotNames[slot]}</span><div className="slot-empty">{slot==='mao_esquerda'&&dualWielding?'Ocupada pelas facas':'Vazio'}</div></button>;const effective=equipmentAttackForHero(e,g.heroId);const affinity=compatibilityLabel(e,g.heroId);const bagSlot=slot==='bolsa';const stats=bagSlot?`${e.capacidade??8} espaços`:`Ataque +${effective} • Defesa +${e.defesa} • Vida +${e.vida}`;return <button key={slot} className={'slot '+(slot==='botas'?'boots':'')+(bagSlot?' backpack-slot':'')} onClick={bagSlot?undefined:()=>g.unequip(slot)}><span>{slotNames[slot]}</span><ArtPreview className="slot-art-preview" image={cardArt(e)} name={e.nome} text={`${e.habilidade} • ${affinity}`} stats={stats}/><div className="slot-info"><strong>{e.nome}</strong><div className="slot-stats">{bagSlot?`CAPACIDADE ${e.capacidade??8}`:<>ATQ +{effective}{effective!==e.ataque?` (base +${e.ataque})`:''} • DEF +{e.defesa} • VIDA +{e.vida}</>}</div><small className="slot-effect">{affinity}</small></div><small className="slot-remove">{bagSlot?'equipe outra bolsa para trocar':'clique para retirar'}</small></button>})}</div></Panel><Panel title={`Equipamentos guardados ${g.equipmentBag.length}/${capacity}`}><div className="item-grid compact">{g.equipmentBag.map((id,idx)=>{const e=EQUIPMENT.find(x=>x.id===id)!;const effective=equipmentAttackForHero(e,g.heroId);const allowed=equipmentClassAllowed(e,g.heroId);const levelAllowed=equipmentLevelAllowed(e,g.xp);const required=equipmentRequiredLevel(e);const fits=e.slot!=='bolsa'||g.equipmentBag.length<=(e.capacidade??8);const dualLocked=e.slot==='mao_esquerda'&&equipmentWeaponClass(EQUIPMENT.find(x=>x.id===g.equipped.mao_direita))==='facas';const equipLabel=!allowed?'Impossível equipar':!levelAllowed?`Requer nível ${required}`:!fits?`Reduza para ${e.capacidade} itens`:dualLocked?'Facas ocupam as duas mãos':'Equipar';const stats=e.slot==='bolsa'?`Capacidade ${e.capacidade??8} espaços`:`Ataque +${effective}${effective!==e.ataque?` (base +${e.ataque})`:''} • Defesa +${e.defesa} • Vida +${e.vida}`;return <ItemCard key={id+idx} image={cardArt(e)} rarity={cardRarity(e,'Equipamento')} name={e.nome} subtitle={e.slot==='bolsa'?`${e.capacidade} espaços`:slotNames[e.slot]} footer={`${e.habilidade} • ${compatibilityLabel(e,g.heroId)}`} previewStats={stats}><button className={!allowed||!fits||dualLocked?'equip-impossible':!levelAllowed?'equip-level-locked':''} disabled={!allowed||!levelAllowed||!fits||dualLocked} title={!allowed?compatibilityLabel(e,g.heroId):!levelAllowed?`Disponível no nível ${required}`:!fits?'Há equipamentos demais para esta bolsa':dualLocked?'Combate com facas exige as duas mãos livres':undefined} onClick={()=>g.equip(id)}>{equipLabel}</button></ItemCard>})}</div></Panel></div>}
