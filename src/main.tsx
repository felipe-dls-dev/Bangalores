...
function GuildScreen(){
 ...
 return <div className="guild-page">{modernGuild?<ScreenMasthead className="guild-masthead" assetUrl={assetUrl} art={SCENE_ART.allies} eyebrow="Salão dos aventureiros" title="Guilda de Havendown" lead={<>Rank <b>{rank.nome}</b> com {reputation} pontos de reputação{nextRank?<>; faltam {nextRank.minimo-reputation} para {nextRank.nome}</>:null}. Contratos entregues liberam missões mais valiosas.</>} stats={[{label:'Missões ativas',value:active},{label:'Prontas',value:completed,hot:completed>0},{label:'Rank',value:rank.nome}]} action={{...guildNextStep(active,completed),icon:<ScrollText size={20}/>,onClick:()=>{setGuildTab('contracts');window.setTimeout(()=>focusSection('#guild-panel-contracts'),0)}}} secondary={[{label:'Voltar ao mapa',icon:<ArrowLeft size={16}/>,onClick:()=>g.setScreen('map')}]} aside={<div className="masthead-npc"><img src={assetUrl(GUILD_LEADER.retrato)} alt=""/><div><small>{GUILD_LEADER.titulo}</small><strong>{GUILD_LEADER.nome}</strong><p>“{guildLeaderLine(g,active,completed,reputation)}”</p></div></div>}/>:<><Panel className="guild-head"><button onClick={()=>g.setScreen('map')}><ArrowLeft/>Voltar ao mapa</button><div><span className="eyebrow">SALÃO DOS AVENTUREIROS</span><h1>Guilda de Havendown</h1><p>Aceite contratos, aumente sua reputação e conquiste acesso às missões mais valiosas.</p></div><Shield className="guild-crest"/></Panel><div className="guild-summary"><span><ScrollText/><small>MISSÕES ATIVAS</small><strong>{active}</strong></span><span><Trophy/><small>PRONTAS</small><strong>{completed}</strong></span><span><Shield/><small>RANK</small><strong>{rank.nome}</strong></span></div>
