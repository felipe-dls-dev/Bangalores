...
function ArtPreview({image,name,text,stats,className,imgStyle,compareEquipment=false,allowEquip=false}:{image:string;name:string;text?:string;stats?:string;className?:string;imgStyle?:React.CSSProperties;compareEquipment?:boolean;allowEquip?:boolean}){
 ...
 // Nem o encaixe de pedras nem o refino de bônus da Forja apareciam na janela de detalhes do
 // item -- o jogador só descobria o que uma peça podia receber abrindo a Forja e procurando a
 // receita dela manualmente. socketCount vem do mesmo cálculo usado no Painel de Forja
 // (equipmentSocketCount); a receita (attributeChoice/effect) é a mesma usada por RecipeCard.
 const forgeRecipe=equipment?FORGE_RECIPES.find(r=>r.equipmentId===equipment.id):undefined
 const socketCount=equipment?equipmentSocketCount(equipment):0
