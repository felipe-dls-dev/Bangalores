...
const WORLD_MAP_IMAGE='./assets/maps/eldravar-v2.png'
function MapScreen(){
 React.useEffect(()=>{const image=document.querySelector<HTMLImageElement>('.map-wrap>img');if(image)image.src=WORLD_MAP_IMAGE},[])
 const g=useGame(),li=levelInfo(g.xp),regions=[...TERRITORIES].sort((a,b)=>a.dificuldade-b.dificuldade)
 const completedSubregions=SUBREGIONS.filter(sub=>g.subregionBossesDefeated.includes(sub.id)).length
 const completedRegions=regions.filter(region=>{const subs=SUBREGIONS.filter(sub=>sub.regionId===region.id);return subs.length>0&&subs.every(sub=>g.subregionBossesDefeated.includes(sub.id))}).length
 return <div className="map-layout map-layout-with-index"><Panel className="map-region-index"><div className="map-index-head"><span className="eyebrow">EXPLORAÇÃO</span><h2>Regiões de Eldravar</h2><p>Destinos organizados por nível de dificuldade.</p></div><div className="map-index-totals"><span><Map/><small>REGIÕES</small><strong>{completedRegions}/{regions.length}</strong></span><span><ScrollText/><small>SUB-REGIÕES</small><strong>{completedSubregions}/{SUBREGIONS.length}</strong></span></div><div className="map-region-list">{regions.map(region=>{const subs=SUBREGIONS.filter(sub=>sub.regionId===region.id).sort((a,b)=>a.nivelMin-b.nivelMin),done=subs.filter(sub=>g.subregionBossesDefeated.includes(sub.id)).length,completed=subs.length>0&&done===subs.length;return <section className={`map-index-region${completed?' completed':''}`} key={region.id}><button className="map-index-region-button" onClick={()=>g.openRegion(region)}><span className="map-index-difficulty">{region.dificuldade}</span><span><strong>{region.nome}</strong><small>Nível {region.nivelMin}–{region.nivelMax}</small></span><b>{done}/{subs.length}</b></button><div className="map-index-subs">{subs.map(sub=>{const subDone=g.subregionBossesDefeated.includes(sub.id);return <button className={subDone?'completed':''} key={sub.id} onClick={()=>g.openSubregion(sub.id)}><i>{subDone?'✓':'◆'}</i><span>{sub.nome}<small>Nível {sub.nivelMin}–{sub.nivelMax}</small></span></button>})}</div></section>})}</div></Panel><Panel className="map-panel"><div className="map-wrap"><img src="./assets/maps/eldravar.png" alt="Mapa de Eldravar"/>{TERRITORIES.map(t=>{const subs=SUBREGIONS.filter(s=>s.regionId===t.id);const completed=subs.length>0&&subs.every(s=>g.subregionBossesDefeated.includes(s.id));return <button key={t.id} className={`map-sub-pin map-territory-pin${completed?' completed':''}`} style={{left:`${t.x*100}%`,top:`${t.y*100}%`}} onClick={()=>g.openRegion(t)} title={`Explorar ${t.nome}`} aria-label={`Explorar território ${t.nome}`}><span>{completed?'✓':'◆'}</span><b>{t.nome}</b></button>})}{SUBREGIONS.map(sub=>{const point=SUBREGION_MAP_POINTS[sub.id];if(!point)return null;const completed=g.subregionBossesDefeated.includes(sub.id);return <button key={`sub_${sub.id}`} className={`map-sub-pin${completed?' completed':''}`} style={{left:`${point[0]*100}%`,top:`${point[1]*100}%`}} onClick={()=>g.openSubregion(sub.id)} title={`Acessar diretamente: ${sub.nome}`} aria-label={`Acessar ${sub.nome}`}><span>{completed?'✓':'◆'}</span><b>{sub.nome}</b></button>})}</div></Panel><Panel title="Resumo do herói" className="summary"><Stat label="Vida" value={`${g.hp}/${maxHp(g)}`}/><Stat label="Ataque" value={attackValue(g)}/><Stat label="Defesa" value={defenseValue(g)}/><Stat label="Ouro" value={g.gold}/><Stat label="Equip. guardados" value={`${g.equipmentBag.length}/${equipmentBagCapacity(g)}`}/><hr/><div className="level-row"><strong>Nível {li.lvl}</strong><span>{li.progress}/{li.next} XP</span></div><div className="xp-track"><div style={{width:`${Math.min(100,li.progress/li.next*100)}%`}}/></div><p className="muted">Experiência total: {g.xp}</p><p className={g.attributePoints?'points hot':'points'}>Pontos de atributo: {g.attributePoints}</p><hr/><strong>Exploração de Eldravar</strong><p className="muted">Todos os losangos e nomes da lista dão acesso direto aos destinos.</p><p className="hint">Marcadores verdes indicam locais cujos chefes já foram derrotados.</p></Panel></div>
}
