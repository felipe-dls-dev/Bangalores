...
function App(){
 ...
 return <div className="app-shell" data-region-theme={regionTheme}><CoopBattleSync/><SaveIndicator/>
...
const TUTORIAL_CHAPTERS=[
 ...
 ['Mapa-mundi e viagem rápida','No mapa-mundi, os marcadores abrem regiões diretamente. A viagem rápida (clicar num marcador ou num nome da lista lateral) só fica liberada para Planícies de Alvora e para regiões cujos chefes já foram todos derrotados; as demais só são alcançadas caminhando pelas passagens entre os mapas navegáveis, descritas a seguir. Marcadores verdes indicam chefes já derrotados; o cadeado mostra destinos ainda bloqueados.'],
...
function TalentPanel(){const g=useGame(),level=levelInfo(g.xp).lvl,specializations=g.specializations??{};return <Panel title="Talentos e especializações"><div className="system-list">{TALENTS.map(t=><button key={t.id} disabled={level<t.level||g.talents.includes(t.id)} title={g.talents.includes(t.id)?'Talento já desbloqueado':level<t.level?`Disponível no nível ${t.level}`:undefined} onClick={()=>g.unlockTalent(t.id)}><strong>{g.talents.includes(t.id)?'✓ ':''}{t.nome}</strong><small>Nível {t.level} • {t.texto}</small></button>)}</div><h3 className="subhead">Caminhos da build e Subclasses</h3>{SPECIALIZATION_CHOICES.map(tier=>{const isSubclassTier=tier.level===30&&Boolean(g.heroId&&HERO_SUBCLASSES[g.heroId]);const options=isSubclassTier?HERO_SUBCLASSES[g.heroId!]:tier.options;return <section className={`specialization-tier${isSubclassTier?' subclass-tier':''}`} key={tier.level}><strong>Nível {tier.level}{isSubclassTier?' • Subclasse Heroica':''}</strong><div>{options.map(option=><button className={specializations[String(tier.level)]===option.id?'selected':''} disabled={level<tier.level||Boolean(specializations[String(tier.level)])} title={level<tier.level?`Disponível no nível ${tier.level}`:specializations[String(tier.level)]&&specializations[String(tier.level)]!==option.id?'Você já escolheu outro caminho neste nível':undefined} onClick={()=>g.chooseSpecialization(tier.level,option.id)} key={option.id}><b>{option.nome}</b><small>{(option as any).passiva??option.texto}</small></button>)}</div></section>})}<button className="danger-action" disabled={!Object.keys(specializations).length} title={!Object.keys(specializations).length?'Nenhum caminho escolhido ainda':undefined} onClick={g.resetSpecializations}>Redefinir caminhos (Gratuito)</button></Panel>}
...
function ForgeScreen(){const g=useGame()
 ...
 const mastery=forgeLevelInfo(g.forgeXp??0),rate=(g.forgeAttempts??0)?Math.round((g.forgeSuccesses??0)/(g.forgeAttempts??1)*100):0;const totalMaterials=Object.values(g.materials).reduce((sum,n)=>sum+n,0);const [materialInfo,setMaterialInfo]=React.useState<ForgeMaterialEntry|undefined>(undefined);return <div className="forge-page"><Panel className="forge-header"><span className="eyebrow">OFICINA DE HAVENDOWN</span><h1>Forja</h1><p>Transforme espólios em materiais, produza itens por receita e instale pedras de melhoria.</p><div className="forge-summary-strip"><span><small>NÍVEL DE FORJADOR</small><strong>{mastery.level}</strong></span><span><small>SUCESSO</small><strong>{rate}%</strong></span><span><small>MATERIAIS</small><strong>{totalMaterials}</strong></span></div><div className="screen-intro"><small>ANTES DE FORJAR</small><p>Use o painel de materiais para entender de onde vêm os insumos, depois compare custo, risco e nível exigido em cada receita.</p></div><div className="forge-mastery"><div><small>NÍVEL DE FORJADOR</small><strong>{mastery.level}</strong></div><div className="forge-xp"><span>{mastery.max?'Maestria máxima':`${mastery.progress}/${mastery.next} XP`}</span><div className="xp-track"><div style={{width:`${mastery.max?100:Math.min(100,mastery.progress/mastery.next*100)}%`}}/></div></div><div><small>SUCESSOS</small><strong>{g.forgeSuccesses??0}/{g.forgeAttempts??0}</strong><span>{rate}% de êxito</span></div></div><p className="forge-warning">Toda tentativa concede XP de Forja. Falhas consomem metade dos materiais; níveis maiores liberam receitas poderosas e aumentam a chance de sucesso.</p></Panel><NpcBanner name={BLACKSMITH.nome} title={BLACKSMITH.titulo} line={blacksmithLine(mastery.level,g.forgeAttempts??0,totalMaterials)} image={BLACKSMITH.retrato}/><ForgeResultDialog/><ForgeTutorialPanel/><Panel title="Materiais e fabricação"><div className="material-grid">{materials.map(m=><button key={m.id} type="button" onClick={()=>setMaterialInfo(m)} title="Ver onde conseguir este material"><b>{g.materials[m.id]??0}</b><small>{m.nome}</small></button>)}</div><div className="forge-items">{g.equipmentBag.slice(0,12).map((id,index)=>{const item=equipmentByRef(id);if(!item)return null;const upLevel=g.equipmentUpgrades[id]??0
   ...
   const upgradeFails=g.equipmentUpgradeFails?.[`${id}:${targetLevel}`]??0
   const chance=Math.round(Math.min(.95,UPGRADE_SUCCESS_CHANCE[targetLevel]+upgradeFails*.08)*100)
...
function MapScreen(){
 ...
 const startRegionId=regions[0]?.id
 // A trava de viagem rápida não pode prender o jogador DENTRO da própria região atual: sem
 // isso, sair pra Mochila/Ficha e voltar pelo Mapa não dava como reentrar em Abdendriel (ou
 // qualquer região ainda bloqueada) mesmo já estando lá, de pé, no meio da exploração --
 // "resumir onde parei" não é a mesma coisa que "pular pra um destino ainda não conquistado".
 const canFastTravel=(id:string,completed:boolean)=>!getRegionMap(id)||id===startRegionId||completed||id===g.regionId
 return <div className="map-layout map-layout-with-index"><Panel className="map-region-index"><div className="map-index-head"><span className="eyebrow">EXPLORAÇÃO</span><h2>Regiões de {worldInfo.label}</h2><p>Destinos organizados por nível de dificuldade.</p></div><div className="screen-intro"><small>COMO LER O MAPA</small><p>Cada região e sub-região tem um nível recomendado. O painel lateral resume sua build atual e ajuda a decidir se vale avançar ou revisar o equipamento.</p></div>{world==='havendown'&&otherUnlocked&&<motion.div {...rarityMotionProps('raro',.04)} className="world-travel-card"><small>ROTAS ENTRE MUNDOS</small><p>Quando quiser mudar de cenário, siga para Steelmere e use o outro mapa como base da viagem.</p><button className="map-travel-button" onClick={()=>g.travelWorld(otherWorld)}><Plane size={16}/>Viajar para Steelmere</button></motion.div>}{otherUnlocked&&world!=='havendown'&&<motion.div {...rarityMotionProps('raro',.04)}><button className="map-travel-button" onClick={()=>g.travelWorld(otherWorld)}><Plane size={16}/>Voltar para {WORLD_MAPS[otherWorld].label}</button></motion.div>}<div className="map-index-totals"><span><Map/><small>REGIÕES</small><strong>{completedRegions}/{regions.length}</strong></span><span><ScrollText/><small>SUB-REGIÕES</small><strong>{completedSubregions}/{worldSubregions.length}</strong></span></div><div className="map-region-list">{regions.map((region,index)=>{const subs=worldSubregions.filter(sub=>sub.regionId===region.id).sort((a,b)=>a.nivelMin-b.nivelMin),done=subs.filter(sub=>g.subregionBossesDefeated.includes(sub.id)).length,completed=subs.length>0&&done===subs.length,unlocked=canFastTravel(region.id,completed);return <motion.section {...rarityMotionProps(completed?'incomum':'comum',index*.035)} className={`map-index-region${completed?' completed':''}${unlocked?'':' locked'}`} key={region.id}><button className="map-index-region-button" disabled={!unlocked} title={unlocked?undefined:'Derrote todos os chefes desta região para liberar a viagem rápida. Chegue até lá caminhando pelas passagens entre as regiões no mapa.'} onClick={()=>g.openRegion(region)}><span className="map-index-difficulty">{region.dificuldade}</span><span><strong>{region.nome}</strong><small>Nível {region.nivelMin}–{region.nivelMax}</small></span><b>{done}/{subs.length}</b></button>{subs.length>0?<div className="map-index-subs">{subs.map(sub=>{const subDone=g.subregionBossesDefeated.includes(sub.id);return <button className={subDone?'completed':''} key={sub.id} onClick={()=>g.openSubregion(sub.id)}><i>{subDone?'✓':'◆'}</i><span>{sub.nome}<small>Nível {sub.nivelMin}–{sub.nivelMax}</small></span></button>})}</div>:<p className="map-index-soon">Sub-regiões em construção.</p>}</motion.section>})}</div></Panel><Panel className="map-panel"><div className="screen-intro"><small>DESTINO ATUAL</small><p>Toque nos marcadores do mapa para viajar direto a uma região já liberada. As demais só liberam a viagem rápida depois que todos os seus chefes forem derrotados — até lá, chegue até elas caminhando pelas passagens entre os mapas.</p></div><motion.div {...rarityMotionProps('epico',.06)} className="map-wrap"><img src={worldInfo.base} alt={`Mapa de ${worldInfo.label}`}/>{regions.map((t,i)=>{const subs=worldSubregions.filter(s=>s.regionId===t.id);const completed=subs.length>0&&subs.every(s=>g.subregionBossesDefeated.includes(s.id));const unlocked=canFastTravel(t.id,completed);return <button key={t.id} className={`map-sub-pin map-territory-pin${completed?' completed':''}${unlocked?'':' locked'}${pinEdgeClass(t.x,t.y)}`} style={{left:`${t.x*100}%`,top:`${t.y*100}%`,'--i':i} as React.CSSProperties} disabled={!unlocked} onClick={()=>g.openRegion(t)} title={unlocked?`Explorar ${t.nome}`:`Derrote todos os chefes de ${t.nome} para liberar a viagem rápida. Chegue lá caminhando pelas passagens entre as regiões.`} aria-label={unlocked?`Explorar território ${t.nome}`:`${t.nome} bloqueado para viagem rápida`}><span>{completed?'✓':unlocked?'◆':'🔒'}</span><b>{t.nome}</b></button>})}</motion.div></Panel><Panel title="Resumo do herói" className="summary"><Stat label="Vida" value={`${g.hp}/${maxHp(g)}`}/><Stat label="Ataque" value={attackValue(g)}/><Stat label="Defesa" value={defenseValue(g)}/><Stat label="Ouro" value={g.gold}/><Stat label="Equip. guardados" value={`${g.equipmentBag.length}/${equipmentBagCapacity(g)}`}/><hr/><div className="level-row"><strong>Nível {li.lvl}</strong><span>{li.progress}/{li.next} XP</span></div><div className="xp-track"><div style={{width:`${Math.min(100,li.progress/li.next*100)}%`}}/></div><p className="muted">Experiência total: {g.xp}</p><p className={g.attributePoints?'points hot':'points'}>Pontos de atributo: {g.attributePoints}</p><hr/><strong>Exploração de {worldInfo.label}</strong><p className="muted">Marcadores e nomes na lista viajam direto ao destino, mas só depois que a região liberar a viagem rápida.</p><p className="hint">Marcadores verdes indicam chefes já derrotados; o cadeado mostra regiões ainda bloqueadas para viagem rápida.</p></Panel></div>
}
  ...
  return <div className="two-col">
    <Panel title="Consumíveis">
      <div className="inventory-search-bar">
        <Search size={14}/>
        <input type="text" placeholder="Filtrar poções e elixires..." value={search} onChange={e=>setSearch(e.target.value)}/>
   ...
   <Panel title="Ações" className="combat-actions-panel combat-actions-area">
      ...
      <div className="combat-actions-grid">
       ...
       {g.heroId==='conjurador'&&heroSkillUses<2&&currentSummons.length<2
        ...
        :<button className="premium-action" disabled={disabled||g.heroId==='conjurador'||heroSkillUses>=heroSkillLimit||((g.heroSkillCooldown??0)>0)} title={g.heroId==='conjurador'?'Você já mantém duas feras espectrais nesta batalha.':(g.heroSkillCooldown??0)>0?`Recarga: ${g.heroSkillCooldown} turnos`:undefined} onClick={()=>isCoop?useCoopHeroSkill():g.heroSkill()}><Sparkles/>{g.heroId==='conjurador'?'Limite de feras atingido':heroSkillNames[g.heroId??'']??'Habilidade do herói'}{(g.heroSkillCooldown??0)>0?` (${g.heroSkillCooldown}t)`:heroSkillLimit>1?` (${Math.min(heroSkillUses,heroSkillLimit)}/${heroSkillLimit})`:''}</button>}
