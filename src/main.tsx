...
function GuildScreen(){
 ...
 const missionProgress=(mission:(typeof GUILD_MISSIONS)[number])=>mission.tipo==='delivery'&&mission.itemId?((g.equipmentBag.some(ref=>equipmentBaseId(ref)===mission.itemId)||Object.values(g.equipped).some(ref=>ref&&equipmentBaseId(ref)===mission.itemId))?1:0):mission.tipo==='material'&&mission.materialId?Math.min(mission.quantidade,g.materials[mission.materialId]??0):Math.min(mission.quantidade,g.guildProgress[mission.id]??0)
