...
function EquipmentScreen(){const g=useGame();const capacity=equipmentBagCapacity(g);const dualWielding=equipmentWeaponClass(EQUIPMENT.find(x=>x.id===g.equipped.mao_direita))==='facas'
 ...
 return <div className="equipment-layout"><Panel title="Slots equipados"><div className="slot-grid">{SLOT_ORDER.map((slot)=>{const id=g.equipped[slot],e=EQUIPMENT.find(x=>x.id===id);if(!e)return <button key={slot} className={'slot '+(slot==='botas'?'boots':'')}><span>{slotNames[slot]}</span><div className="slot-empty">{slot==='mao_esquerda'&&dualWielding?'Ocupada pelas facas':'Vazio'}</div></button>;const effective=equipmentAttackForHero(e,g.heroId);const gems=equipmentGemBonus(id,g);const affinity=compatibilityLabel(e,g.heroId);const bagSlot=slot==='bolsa';const stats=bagSlot?`${e.capacidade??8} espaços`:`Ataque +${effective+gems.atk} • Defesa +${e.defesa+gems.def} • Vida +${e.vida+gems.life}`;return <button key={slot} className={'slot '+(slot==='botas'?'boots':'')+(bagSlot?' backpack-slot':'')} onClick={bagSlot?undefined:()=>g.unequip(slot)}><span>{slotNames[slot]}</span><ArtPreview className="slot-art-preview" image={cardArt(e)} name={e.nome} text={`${e.habilidade} • ${affinity}${elementalNote(g,e.id)}${gemNote(id,g)}`} stats={stats}/><div className="slot-info"><strong>{e.nome}</strong><div className="slot-stats">{bagSlot?`CAPACIDADE ${e.capacidade??8}`:<>ATQ +{effective+gems.atk}{effective!==e.ataque||gems.atk?` (base +${e.ataque}${gems.atk?` +${gems.atk} pedra`:''})`:''} • DEF +{e.defesa+gems.def}{gems.def?` (+${gems.def} pedra)`:''} • VIDA +{e.vida+gems.life}{gems.life?` (+${gems.life} pedra)`:''}</>}</div><small className="slot-effect">{affinity}{elementalNote(g,e.id)}{gemNote(id,g)}</small></div><small className="slot-remove">{bagSlot?'equipe outra bolsa para trocar':'clique para retirar'}</small></button>})}</div></Panel><div className="equipment-side-col"><Panel title="Atributos e bônus" className="equipment-attr-panel"><Stat label="Vida máxima" value={`(${maxHp(g)-itemBonus.life}+${itemBonus.life})=${maxHp(g)}`}/><Stat label="Ataque" value={`(${attackValue(g)-itemBonus.atk}+${itemBonus.atk})=${attackValue(g)}`}/><Stat label="Defesa" value={`(${defenseValue(g)-itemBonus.def}+${itemBonus.def})=${defenseValue(g)}`}/><hr/><div className="equipment-elemental-list">{elementalEntries.length?elementalEntries.map(entry=><div key={entry.id} className="equipment-elemental-row"><span>{entry.name}</span><b className={entry.kind==='Elemento'?'elemental-attack':'elemental-resist'}>{entry.kind}: {entry.value}</b></div>):<p className="muted">Nenhum bônus elemental equipado — forje um item com sucesso ou derrote um chefe para conseguir um.</p>}</div></Panel><Panel title={`Equipamentos guardados ${g.equipmentBag.length}/${capacity}`}><div className="item-grid compact">{g.equipmentBag.map((id,idx)=>{const e=EQUIPMENT.find(x=>x.id===id)!;const effective=equipmentAttackForHero(e,g.heroId);const gems=equipmentGemBonus(id,g);const allowed=equipmentClassAllowed(e,g.heroId);const levelAllowed=equipmentLevelAllowed(e,g.xp);const required=equipmentRequiredLevel(e);const fits=e.slot!=='bolsa'||g.equipmentBag.length<=(e.capacidade??8);const dualLocked=e.slot==='mao_esquerda'&&equipmentWeaponClass(EQUIPMENT.find(x=>x.id===g.equipped.mao_direita))==='facas';const equipLabel=!allowed?'Impossível equipar':!levelAllowed?`Requer nível ${required}`:!fits?`Reduza para ${e.capacidade} itens`:dualLocked?'Facas ocupam as duas mãos':'Equipar';const stats=e.slot==='bolsa'?`Capacidade ${e.capacidade??8} espaços`:`Ataque +${effective+gems.atk}${effective!==e.ataque||gems.atk?` (base +${e.ataque})`:''} • Defesa +${e.defesa+gems.def} • Vida +${e.vida+gems.life}`;return <ItemCard key={id+idx} image={cardArt(e)} rarity={cardRarity(e,'Equipamento')} name={e.nome} subtitle={e.slot==='bolsa'?`${e.capacidade} espaços`:slotNames[e.slot]} footer={`${e.habilidade} • ${compatibilityLabel(e,g.heroId)}${elementalNote(g,id)}${gemNote(id,g)}`} previewStats={stats}><button className={!allowed||!fits||dualLocked?'equip-impossible':!levelAllowed?'equip-level-locked':''} disabled={!allowed||!levelAllowed||!fits||dualLocked} title={!allowed?compatibilityLabel(e,g.heroId):!levelAllowed?`Disponível no nível ${required}`:!fits?'Há equipamentos demais para esta bolsa':dualLocked?'Combate com facas exige as duas mãos livres':undefined} onClick={()=>g.equip(id)}>{equipLabel}</button></ItemCard>})}</div></Panel></div></div>}
