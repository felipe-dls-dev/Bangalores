...
function RegionMapView({region,subs,level,selectedSub}:{region:Territory;subs:Subregion[];level:number;selectedSub?:Subregion}){
 ...
 return <section className="regionmap-shell">
  <div className="regionmap-stage"><div className="regionmap-stage-title"><Map size={18}/><span>Região de {region.nome}</span></div><TileWorldExplorer map={map} initialPosition={startLoc?{x:startLoc.x,y:startLoc.y}:undefined} paused={Boolean(encounterPrompt||activeNpc||g.ambush)} onEnterLocation={handleEnter} locationStatus={locationStatus} npcs={npcs} npcStatus={npcStatus} onInteractNpc={setActiveNpc} onAmbush={subId=>g.triggerAmbush(subId)}/></div>
