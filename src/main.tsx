...
function ArtPreview({image,name,text,stats,className,imgStyle,compareEquipment=false,allowEquip=false,instanceRef}:{image:string;name:string;text?:string;stats?:string;className?:string;imgStyle?:React.CSSProperties;compareEquipment?:boolean;allowEquip?:boolean;instanceRef?:string}){
 ...
 // que realmente é (nos dois lados), podendo levar a trocar por uma peça pior por engano.
 const instanceBonus=(ref:string|undefined,eq:Equipment|undefined)=>{if(!ref||!eq)return undefined;const gems=equipmentGemBonus(ref,{equipmentGems} as any),up=equipmentUpgrades?.[ref]??0,classDelta=eq.statsByClass?.[heroId??''];return{attack:(eq.ataque?up:0)+(classDelta?.ataque??0)+gems.atk,defense:(eq.defesa?Math.floor((up+1)/2):0)+(classDelta?.defesa??0)+gems.def,life:up+(classDelta?.vida??0)+gems.life}}
 const currentBonus=instanceBonus(currentRef,currentEquipment)
 const candidateBonus=instanceBonus(instanceRef,equipment)
