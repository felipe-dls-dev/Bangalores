...
function RegionMapView({region,subs,level,selectedSub}:{region:Territory;subs:Subregion[];level:number;selectedSub?:Subregion}){
 ...
 const [activeSub,setActiveSub]=React.useState<Subregion|undefined>(selectedSub)
 ...
 const handleEnter=(subId:string)=>{const sub=subs.find(s=>s.id===subId);if(sub)setActiveSub(sub)}
 const closeModal=()=>{setActiveSub(undefined);if(g.subregionId)g.openRegion(region)}
 return <div className="regionmap-outer"><span className="regionmap-proto-badge">PROTÓTIPO • Navegação espacial</span><TileWorldExplorer map={map} initialPosition={startLoc?{x:startLoc.x,y:startLoc.y}:undefined} paused={Boolean(activeSub)} onEnterLocation={handleEnter} locationStatus={locationStatus}/>{activeSub&&<div className="regionmap-modal-backdrop" onClick={closeModal}><div className="regionmap-modal" onClick={e=>e.stopPropagation()}><button className="regionmap-modal-close" onClick={closeModal} aria-label="Fechar"><X/></button><SubregionCard sub={activeSub} level={level}/></div></div>}</div>
}
