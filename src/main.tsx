...
function ForgeScreen(){const g=useGame(),materials=Object.entries(REGION_MATERIALS).map(([,m])=>m),mastery=forgeLevelInfo(g.forgeXp??0),rate=(g.forgeAttempts??0)?Math.round((g.forgeSuccesses??0)/(g.forgeAttempts??1)*100):0;return <div className="forge-page"><Panel className="forge-header"><span className="eyebrow">OFICINA DE HAVENDOWN</span><h1>Forja</h1><p>Transforme espólios em materiais, produza itens por receita e instale pedras de melhoria.</p><div className="forge-mastery"><div><small>NÍVEL DE FORJADOR</small><strong>{mastery.level}</strong></div><div className="forge-xp"><span>{mastery.max?'Maestria máxima':`${mastery.progress}/${mastery.next} XP`}</span><div className="xp-track"><div style={{width:`${mastery.max?100:Math.min(100,mastery.progress/mastery.next*100)}%`}}/></div></div><div><small>SUCESSOS</small><strong>{g.forgeSuccesses??0}/{g.forgeAttempts??0}</strong><span>{rate}% de êxito</span></div></div><p className="forge-warning">Toda tentativa concede XP de Forja. Falhas consomem metade dos materiais; níveis maiores liberam receitas poderosas e aumentam a chance de sucesso.</p><ForgeResultBanner/></Panel><Panel title="Materiais e fabricação"><div className="material-grid">{materials.map(m=><span key={m.id}><b>{g.materials[m.id]??0}</b><small>{m.nome}</small></span>)}</div><div className="forge-items">{g.equipmentBag.slice(0,12).map((id,index)=>{const item=EQUIPMENT.find(e=>e.id===id);return item&&<article key={`${id}-${index}`}><span><strong>{item.nome} +{g.equipmentUpgrades[id]??0}</strong><small>Aprimorar: {10+((g.equipmentUpgrades[id]??0)+1)*10} ouro</small></span><button disabled={(g.equipmentUpgrades[id]??0)>=3} onClick={()=>g.upgradeEquipment(id)}>Aprimorar</button></article>})}</div></Panel><RecipeCatalog/><ForgeSalvagePanel/></div>}
