...
function ForgeResultBanner(){const g=useGame(),result=g.forgeResult;const title=result?.kind==='upgrade'?(result.success?'Aprimoramento bem-sucedido!':'Aprimoramento falhou'):(result?.success?'Forja bem-sucedida!':'Fabricação falhou');return <AnimatePresence>{result&&<motion.div key={result.id} className={`forge-result-banner ${result.success?'success':'failure'}`} initial={{opacity:0,y:-10,scale:.96}} animate={{opacity:1,y:0,scale:1}} exit={{opacity:0,y:-10,scale:.96}} transition={{duration:.3}} role="status" aria-live="assertive">{result.success?<CheckCircle2/>:<XCircle/>}<div><strong>{title}</strong><span>{result.message}</span></div></motion.div>}</AnimatePresence>}
