...
function CoopBattleSync(){
 ...
 React.useEffect(()=>{if(battle?.status!=='won'||!battle.id||!battle.subregionId||!battle.enemy)return;const damage=battle.damageByPlayer??{},healing=battle.healingByPlayer??{},resisted=battle.damageResistedByPlayer??{},contributors=new Set([...Object.keys(damage),...Object.keys(healing),...Object.keys(resisted)]),contributions=Object.fromEntries([...contributors].map(id=>[id,Math.max(0,Number(damage[id])||0)+Math.max(0,Number(healing[id])||0)+Math.max(0,Number(resisted[id])||0)])),total=Object.values(contributions).reduce((sum,value)=>sum+value,0),mine=contributions[coop.userId]??0,share=total>0?mine/total:1/Math.max(1,coop.members.length);completeVictory(battle.id,battle.subregionId,battle.enemy,share)},[battle?.id,battle?.status,battle?.subregionId,battle?.enemy,battle?.damageByPlayer,battle?.healingByPlayer,battle?.damageResistedByPlayer,coop.userId,coop.members.length,completeVictory])
   ...
   <Panel title="Ações" className="combat-actions-panel combat-actions-area">
      ...
      <div className="combat-actions-grid">
       ...
       <div className="stance-row" role="group" aria-label="Postura de combate">
        <button className={`premium-action${currentStance==='ofensiva'?' active':''}`} disabled={disabled} title="Postura Ofensiva: +20% de Ataque e -20% de Defesa até trocar de postura." onClick={()=>performSetStance('ofensiva')}><Swords/>Ofensiva</button>
        <button className={`premium-action${currentStance==='neutra'?' active':''}`} disabled={disabled} title="Postura Neutra: sem bônus ou penalidade de Ataque/Defesa." onClick={()=>performSetStance('neutra')}><Scale/>Neutra</button>
        <button className={`premium-action${currentStance==='defensiva'?' active':''}`} disabled={disabled} title="Postura Defensiva: +20% de Defesa e -20% de Ataque até trocar de postura." onClick={()=>performSetStance('defensiva')}><ShieldHalf/>Defensiva</button>
       </div>
