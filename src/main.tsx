...
function CoopBattleSync(){
 ...
 React.useEffect(()=>{const roomId=coop.room?.id,userId=coop.userId,g=useGame.getState(),myMaxHp=maxHp(g),myDefense=defenseValue(g),level=levelInfo(xp).lvl,rollBonus=g.classRollBonus??0,critDefenseBoost=hasCraftedEffect(g,'defesa_perfeita'),dodgeBoost=hasCraftedEffect(g,'esquiva_forjada'),weaponAnim=heroWeaponAnimationType(g.equipped.mao_direita),key=`${roomId}:${userId}:${hp}:${myMaxHp}:${myDefense}:${level}:${shield}:${rollBonus}:${critDefenseBoost}:${dodgeBoost}:${weaponAnim}`;if(!roomId||!userId||publishedVitals.current===key)return;publishedVitals.current=key;void coop.publishProgress(subregionVictories,{hp,maxHp:myMaxHp,level,defense:myDefense,shield,rollBonus,critDefenseBoost,dodgeBoost,weaponAnim})},[coop.room?.id,coop.userId,hp,xp,shield,subregionVictories,screen,coop.publishProgress])
