...
function CombatDiceRoll({roll}:{roll:{attacker:'hero'|'enemy';naturalAttackRoll:number;attackRoll:number;attackBonus:number;defenseRoll:number;attackBase:number;defenseBase:number;attackEffect:string;defenseEffect:string;damage:number;selfDamage:number;shieldBlocked?:number}}){return <motion.aside className={`combat-dice-roll ${roll.attacker}`} initial={{opacity:0,y:-18,scale:.9}} animate={{opacity:1,y:0,scale:1}} exit={{opacity:0,y:-12}} aria-live="assertive"><small>{roll.attacker==='hero'?'SEU TESTE DE ATAQUE':'ATAQUE DO INIMIGO'}</small><div className="combat-dice-pair"><div><span>ATAQUE • BASE {roll.attackBase}</span><motion.b className="combat-die attack-die" animate={{rotate:[0,110,250,370,360],scale:[.75,1.18,.88,1]}} transition={{duration:.55}}>{roll.attackRoll}</motion.b><em><strong>{roll.attackEffect}</strong>{roll.attackBonus>0&&<u>Rolagem {roll.naturalAttackRoll} + {roll.attackBonus}</u>}</em></div><i>VS</i><div><span>DEFESA • BASE {roll.defenseBase}</span><motion.b className="combat-die defense-die" animate={{rotate:[0,-120,-260,-370,-360],scale:[.75,1.18,.88,1]}} transition={{duration:.55}}>{roll.defenseRoll}</motion.b><em><strong>{roll.attackRoll===1?'Não se aplica':roll.defenseEffect}</strong></em></div></div><p>{roll.selfDamage?'FALHA CRÍTICA — DANO NO ATACANTE':'DANO'} <strong>{roll.selfDamage||roll.damage}</strong>{roll.shieldBlocked?` • ESCUDO BLOQUEOU ${roll.shieldBlocked}`:''}</p></motion.aside>}
