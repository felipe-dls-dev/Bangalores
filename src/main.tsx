...
function RegionMapView({region,subs,level,selectedSub}:{region:Territory;subs:Subregion[];level:number;selectedSub?:Subregion}){
 ...
 return <section className="regionmap-shell">
  <div className="regionmap-stage"><div className="regionmap-stage-title"><Map size={18}/><span>Região de {region.nome}</span></div><TileWorldExplorer map={map} initialPosition={startLoc?{x:startLoc.x,y:startLoc.y}:undefined} paused={Boolean(encounterPrompt)} onEnterLocation={handleEnter} locationStatus={locationStatus}/></div>
  ...
  <footer className="regionmap-dialogue"><div className="regionmap-dialogue-avatar"><img src="/assets/maps/sprites/adventurer/down_1.png" alt=""/></div><p>{ready?'O caminho à frente ficou silencioso. Algo poderoso aguarda nesta área.':sub.desafios[0]??'Explore a região para descobrir novos desafios.'}</p><span>▼</span></footer>
