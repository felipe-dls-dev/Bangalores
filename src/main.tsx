...
function TalentPanel(){const g=useGame(),level=levelInfo(g.xp).lvl,specializations=g.specializations??{};return <Panel title="Talentos e especializações"><div className="system-list">{TALENTS.map(t=><button key={t.id} disabled={level<t.level||g.talents.includes(t.id)} title={g.talents.includes(t.id)?'Talento já desbloqueado':level<t.level?`Disponível no nível ${t.level}`:undefined} onClick={()=>g.unlockTalent(t.id)}><strong>{g.talents.includes(t.id)?'✓ ':''}{t.nome}</strong><small>Nível {t.level} • {t.texto}</small></button>)}</div><h3 className="subhead">Caminhos da build</h3>{SPECIALIZATION_CHOICES.map(tier=><section className="specialization-tier" key={tier.level}><strong>Nível {tier.level}</strong><div>{tier.options.map(option=><button className={specializations[String(tier.level)]===option.id?'selected':''} disabled={level<tier.level||Boolean(specializations[String(tier.level)])} title={level<tier.level?`Disponível no nível ${tier.level}`:specializations[String(tier.level)]&&specializations[String(tier.level)]!==option.id?'Você já escolheu outro caminho neste nível':undefined} onClick={()=>g.chooseSpecialization(tier.level,option.id)} key={option.id}><b>{option.nome}</b><small>{option.texto}</small></button>)}</div></section>)}<button className="danger-action" disabled={!Object.keys(specializations).length||g.gold<100+Object.keys(specializations).length*75} title={!Object.keys(specializations).length?'Nenhum caminho escolhido ainda':g.gold<100+Object.keys(specializations).length*75?'Ouro insuficiente':undefined} onClick={g.resetSpecializations}>Redefinir caminhos ({100+Object.keys(specializations).length*75} ouro)</button></Panel>}
