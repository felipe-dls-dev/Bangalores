...
import { ATTRIBUTE_HINTS, ATTRIBUTE_LABELS, ATTRIBUTE_RULES, PRIMARY_ATTRIBUTES, heroStatProfile, type PrimaryAttributeKey } from './data/heroStatProfiles'
import { ITEM_STAT_KEYS, ITEM_STAT_LABELS, ITEM_STAT_SHORT, STYLE_LABELS, equipmentStatScore, formatItemStats, formatItemStatsShort, offenseWeights, type ItemStatKey, type ItemStats } from './data/equipmentAttributes'
import { shopEquipmentChips, type ShopStatChip, type ShopVerdict } from './ui/shopStats'
...
import { SCENE_ART, availableTalentCount, bagUpgrades, chapterScene, characterNextStep, consumableTotals, forgeReadyCount, guildNextStep } from './ui/screenIdentity'
import { campChapter, continueTarget } from './ui/campData'
...
// Leva a uma seção da tela (ação principal dos cabeçalhos do modo Moderno) e põe o foco no primeiro botão ativo dela,
// para quem usa teclado continuar dali.
function focusSection(selector:string){const el=document.querySelector<HTMLElement>(selector);if(!el)return;el.scrollIntoView({behavior:effectsReduced()?'auto':'smooth',block:'start'});el.querySelector<HTMLElement>('button:not(:disabled),input,select')?.focus({preventScroll:true})}
...
// Ordem por assunto (a mesma dos grupos do modo Moderno, em navGroups.ts): Expedição (mapa, masmorras, crônicas), Herói
// (ficha, equipamento, mochila), Vila (loja, forja, guilda), Conquistas (conquistas e bestiário, coleção), Coop e Tutorial.
const nav=[['map','Mapa',Map],['dungeon','Masmorras',Castle],['chronicle','Crônicas',History],['character','Ficha',ScrollText],['equipment','Equipamento',Shield],['inventory','Mochila',Backpack],['shop','Loja',ShoppingBag],['forge','Forja',Wand2],['guild','Guilda',Landmark],['achievements','Conquistas',Trophy],['gallery','Coleção',Images],['coop','Coop',Users],['tutorial','Tutorial',BookOpen]] as const
...
function equipmentStatParts(e:Equipment,ref:string|undefined,g:{heroId?:string;equipmentUpgrades:Record<string,number>;equipmentGems:Record<string,string[]>}){
 const b=equipmentInstanceBreakdown(e,ref,g as any),detail:Partial<Record<ItemStatKey,string>>={}
 for(const key of ITEM_STAT_KEYS)detail[key]=statDetail(b.base[key],b.upgrade[key],b.gems[key])
 return{total:b.total,text:formatItemStats(b.total,detail)}
}
// Atributos da peça (sem forja nem pedras) para o herói ativo, em uma linha: "Força +5 • Destreza +3 • Armadura +4".
function equipmentStatsText(e:Equipment,heroId=activeHeroId()){return formatItemStats(equipmentBaseStats(e,heroId))}
// Aviso quando a peça dá Força a quem ataca com Magia (ou o contrário): o atributo existe, mas não aumenta o ataque.
function offenseMismatchNote(stats:ItemStats,heroId?:string){const w=offenseWeights(heroId);const wasted=(['forca','magia'] as const).filter(key=>stats[key]>0&&w[key]<.5);return wasted.length?`${wasted.map(key=>ITEM_STAT_LABELS[key]).join(' e ')} não ${wasted.length>1?'aumentam':'aumenta'} o ataque da sua classe.`:''}
...
function artStats(card:any,kind?:string){
 ...
 if(kind==='Equipamento')return card.slot==='bolsa'?`Nível ${equipmentRequiredLevel(card)} • Capacidade ${card.capacidade??8} espaços`:`Nível ${equipmentRequiredLevel(card)} • ${equipmentStatsText(card)}`
...
// Compara TODOS os atributos que as duas peças dão (Força, Magia, Vigor, Destreza, Armadura, Vida, Energia, Esquiva), com o
// aprimoramento e as pedras de cada instância, e um Índice (equipmentStatScore) que resume quanto a peça vale para a classe:
// com oito atributos possíveis, comparar número a número deixou de bastar.
function EquipmentComparison({item,current,currentTotal,candidateTotal,heroId}:{item:(typeof EQUIPMENT)[number];current?:typeof item;currentTotal?:ItemStats;candidateTotal?:ItemStats;heroId?:string}){
 const statsOf=(equipment:typeof item,total?:ItemStats)=>total??equipmentBaseStats(equipment,heroId)
 const candidate=statsOf(item,candidateTotal),equipped=current?statsOf(current,currentTotal):undefined
 const keys=ITEM_STAT_KEYS.filter(key=>candidate[key]||equipped?.[key])
 const score=(stats:ItemStats)=>Math.round(equipmentStatScore(stats,heroId)*10)/10
 const num=(n:number)=>n.toLocaleString('pt-BR',{maximumFractionDigits:1})
 const metric=(label:string,value:number,currentValue?:number,suffix='',title?:string)=>{const delta=currentValue===undefined?0:Math.round((value-currentValue)*10)/10;return <span key={label} title={title}><small>{label}</small><b>{value>=0?'+':''}{num(value)}{suffix}</b>{currentValue!==undefined&&delta!==0&&<em className={delta>0?'better':'worse'}>{delta>0?'+':''}{num(delta)}{suffix}</em>}</span>}
...
function ArtPreview({image,name,text,stats,className,imgStyle,compareEquipment=false,allowEquip=false,instanceRef}:{image:string;name:string;text?:string;stats?:string;className?:string;imgStyle?:React.CSSProperties;compareEquipment?:boolean;allowEquip?:boolean;instanceRef?:string}){
 ...
 const instanceTotal=(ref:string|undefined,eq:Equipment|undefined)=>{if(!ref||!eq)return undefined;return equipmentInstanceBreakdown(eq,ref,pseudoState).total}
...
function CardFrame({card,kind,artStyle,frameTheme,attackFx,attackFxCritical,supportFx,tilt,holoMode}:{card:any;kind:string;artStyle?:React.CSSProperties;frameTheme?:string;attackFx?:AttackAnimType;attackFxCritical?:boolean;supportFx?:'fortificacao'|'cura'|'cura-item';tilt?:boolean;holoMode?:'off'|'full'|'frame'|'art'}){
 ...
 // Equipamento: as três gemas da moldura mostram o ataque da peça (Força ou Magia), a Armadura e a Vida que ela dá (Vida +
 // Vigor × 2); os demais atributos (Destreza, Energia, Esquiva) aparecem no texto da carta e na comparação.
 const itemStats=kind==='Equipamento'&&card.slot&&card.slot!=='bolsa'?equipmentBaseStats(card,activeHeroId()):undefined
 const attack=itemStats?itemStats.forca+itemStats.magia:card.ataque??0,defense=itemStats?itemStats.armadura:card.defesa??(enemy?Math.max(0,(card.dificuldade??1)-2):0),life=itemStats?itemStats.vida+itemStats.vigor*ATTRIBUTE_RULES.vigorVidaPorPonto:card.vida??(kind==='Consumível'?card.valor??0:0)
 ...
 return <div className="app-shell" data-region-theme={regionTheme} data-ui-mode={uiMode}><CoopBattleSync/><SaveIndicator/>
   ...
   <AnimatePresence mode="wait">
    <motion.main key={g.screen} className="screen" initial={{opacity:0,y:8}} animate={{opacity:1,y:0}} exit={{opacity:0,y:-8}} transition={{duration:.22}}>
      {g.screen==='menu'&&<MainMenu/>}{g.screen==='select'&&<HeroSelect/>}{g.screen==='camp'&&uiMode==='modern'&&<CampScreen assetUrl={assetUrl} heroArt={hero=>cardArt(hero)} classLabel={id=>classNames[id]??id} chapterArt={storyChapterArt} crystalIcon={size=><CrystalIcon size={size}/>} onGo={target=>g.setScreen(target as any)}/>}{g.screen==='map'&&<MapScreen/>}{g.screen==='guild'&&<GuildScreen/>}{g.screen==='chronicle'&&<ChronicleScreen/>}{g.screen==='dungeon'&&<DungeonScreen/>}{g.screen==='achievements'&&<AchievementsScreen/>}{g.screen==='forge'&&<ForgeScreen/>}{g.screen==='region'&&<><RegionScreen/><RegionRevengePanel/></>}{g.screen==='event'&&<EventScreen/>}{g.screen==='character'&&<CharacterScreen/>}{g.screen==='inventory'&&<InventoryScreen/>}{g.screen==='equipment'&&<EquipmentScreen/>}{g.screen==='shop'&&<ShopScreen/>}{g.screen==='gallery'&&<GalleryScreen/>}{g.screen==='tutorial'&&<TutorialScreen/>}{g.screen==='coop'&&<PersistentCoopScreen assetUrl={assetUrl}/>}{g.screen==='combat'&&<CombatScreen/>}{g.screen==='bossIntro'&&<BossIntro/>}{g.screen==='loot'&&<LootScreen/>}{g.screen==='cardCreator'&&<CardCreatorScreen/>}
...
function TutorialScreen(){
 ...
 // Modo Moderno: o tour guiado vira a ação principal do cabeçalho (o botão repetido da rota recomendada sai).
 const modern=useUiMode()==='modern'
 return <div className="tutorial-page">
  {modern?<ScreenMasthead className="tutorial-masthead" assetUrl={assetUrl} eyebrow="Acampamento • Manual do aventureiro" title="Aprenda Bangalore's" lead="Dos primeiros passos às builds avançadas: consulte uma regra ou refaça o passeio guiado pelas telas." stats={[{label:'Capítulos',value:chapters.length},{label:'Primeiros passos',value:QUICK_START.length}]} action={{label:'Iniciar tour guiado',detail:'Um passeio pelas telas principais',icon:<Sparkles size={20}/>,onClick:g.startTour}} secondary={[{label:'Abrir todos os capítulos',icon:<BookOpen size={16}/>,onClick:()=>setOpen(chapters.map((_,i)=>i))}]}/>:<header className="tutorial-hero"><BookOpen/><div><span className="eyebrow">MANUAL DO AVENTUREIRO</span><h1>Aprenda Bangalore's</h1><p>Dos primeiros passos às builds avançadas: consulte uma regra ou refaça o passeio guiado pelas telas.</p></div></header>}
  <section className="tutorial-start"><div className="tutorial-start-head"><div><span className="eyebrow">ROTA RECOMENDADA</span><h2>Seu início em 5 passos</h2></div>{!modern&&<button className="primary" onClick={g.startTour}><Sparkles size={15}/>Iniciar tour guiado</button>}</div><div className="tutorial-start-grid">{QUICK_START.map(([number,title,text])=><article key={number}><b>{number}</b><div><strong>{title}</strong><p>{text}</p></div></article>)}</div></section>
...
function RecipeCard({recipe,item,g,mastery}:{recipe:typeof FORGE_RECIPES[number],item:Equipment,g:any,mastery:{level:number}}){
 ...
 return <article className={canCraft?'ready':locked?'forge-locked':''}>
  ...
  <div>
   ...
   <span className="forge-base-stats">{item.slot==='bolsa'?`Capacidade: ${item.capacidade} espaços`:equipmentStatsText(item)} • {equipmentSocketCount(item)} encaixe{equipmentSocketCount(item)===1?'':'s'}</span>
...
function TalentPanel(){
 ...
 return <Panel title="Árvore de talentos" className="talent-tree-panel"><div className="talent-tree-wrap">
...
function EquipmentRulesPanel(){const g=useGame(),sets=equipmentSetCounts(g),weapon=equipmentByRef(g.equipped.mao_direita),equipped=(Object.values(g.equipped) as string[]).map(id=>equipmentByRef(id)).filter(Boolean);return <Panel className="build-summary" title="Resumo da build"><div className="build-summary-grid"><span><small>PODER DE ATAQUE</small><strong>{attackValue(g)}</strong></span><span><small>ARMADURA</small><strong>{armorValue(g)}</strong></span><span><small>VIDA MÁXIMA</small><strong>{maxHp(g)}</strong></span><span><small>ELEMENTO</small><strong>{heroWeaponElement(g)}</strong></span></div><h3>Conjuntos</h3><div className="build-tags">{SET_BONUSES.map((set,index)=>{const count=[sets.lua,sets.cinzas,sets.khar,sets.eclipse][index];return <span className={count>=2?'active':''} key={set.nome}><b>{set.nome}</b> {count}/4 • {count>=4?set.four:count>=2?set.two:'bônus inativo'}</span>})}</div><h3>Efeitos equipados</h3><div className="build-tags"><span>Arma: {weapon?.nome??'nenhuma'} ({heroWeaponElement(g)})</span>{heroResistances(g).map(r=><span key={r}>Resistência: {r}</span>)}{equipped.filter(e=>e?.activeEffect).map(e=><span key={e!.id}>{e!.nome}: {e!.activeEffect!.description}</span>)}</div><details><summary>Regras elementais</summary><p><b>Elementos:</b> {ELEMENTS.join(' • ')}</p><p><b>Condições:</b> {STATUS_INFO.map(s=>`${s[0]}: ${s[1]}`).join(' • ')}</p></details></Panel>}
...
function ForgeScreen(){const g=useGame()
 ...
 // Modo Moderno: cabeçalho da oficina com a cena da forja, o ferreiro ao lado e a ação do que dá para fazer agora.
 const modernForge=useUiMode()==='modern',readyCount=modernForge?forgeReadyCount(g):0
 const openBench=(bench:ForgeWorkbench)=>{setActiveWorkbench(bench);window.setTimeout(()=>focusSection('.forge-workbench-shell'),0)}
 const forgeAction=readyCount>0?{label:'Forjar receita pronta',detail:`${readyCount} ${readyCount===1?'receita pronta':'receitas prontas'} agora`,icon:<Sparkles size={20}/>,onClick:()=>openBench('craft')}:upgradeCount>0?{label:'Aprimorar equipamento',detail:'Reforce armas e armaduras (+1 a +3)',icon:<ArrowUpDown size={20}/>,onClick:()=>openBench('upgrade')}:{label:'Ver receitas',detail:'Materiais que faltam em cada uma',icon:<Sparkles size={20}/>,onClick:()=>openBench('craft')}
 return <div className="forge-page forge-redesign"><ForgeResultDialog/>{modernForge?<ScreenMasthead className="forge-masthead" assetUrl={assetUrl} art={SCENE_ART.forge} eyebrow="Oficina de Havendown" title="Forja" lead={<>Maestria de forja nível {mastery.level}. Receitas prontas aparecem primeiro no catálogo; as demais mostram quantos materiais faltam.</>} stats={[{label:'Prontas agora',value:readyCount,hot:readyCount>0},{label:'Maestria',value:`Nível ${mastery.level}`},{label:'Taxa de sucesso',value:`${rate}%`},{label:'Materiais',value:totalMaterials}]} action={forgeAction} aside={<div className="masthead-npc"><img src={assetUrl(BLACKSMITH.retrato)} alt=""/><div><small>{BLACKSMITH.titulo}</small><strong>{BLACKSMITH.nome}</strong><p>“{blacksmithLine(mastery.level,g.forgeAttempts??0,totalMaterials)}”</p></div></div>}/>:<section className="forge-hero-v2"><div className="forge-hero-copy"><span className="eyebrow">OFICINA DE HAVENDOWN</span><h1>Forja</h1><p>{blacksmithLine(mastery.level,g.forgeAttempts??0,totalMaterials)}</p><div className="forge-hero-actions"><button className="primary" onClick={()=>setActiveWorkbench('craft')}><Sparkles size={16}/>Receitas</button><button onClick={()=>setActiveWorkbench('upgrade')}><ArrowUpDown size={16}/>Aprimorar</button></div></div><aside className="forge-blacksmith-card"><img src={assetUrl(BLACKSMITH.retrato)} alt={BLACKSMITH.nome}/><div><small>{BLACKSMITH.titulo}</small><strong>{BLACKSMITH.nome}</strong><span>Ouro disponível: {g.gold}</span></div></aside></section>}<nav className="forge-workbench-nav" aria-label="Bancadas da Forja">{workbenches.map(bench=><button key={bench.id} className={activeWorkbench===bench.id?'active':''} onClick={()=>setActiveWorkbench(bench.id)}>{bench.icon}<span><strong>{bench.label}</strong><small>{bench.description}</small></span><b>{bench.count}</b></button>)}</nav><div className="forge-workspace-v2"><main className="forge-workbench-shell"><div className="forge-active-head"><span>{active.icon}</span><div><small>BANCADA ATIVA</small><h2>{active.label}</h2><p>{active.description}</p></div></div><AnimatePresence mode="wait" initial={false}><motion.div key={activeWorkbench} className="forge-workbench-content" initial={{opacity:0,y:10}} animate={{opacity:1,y:0}} exit={{opacity:0,y:-8}} transition={{duration:.18}}>{activeWorkbench==='craft'?<RecipeCatalog/>:activeWorkbench==='upgrade'?<ForgeUpgradePanel/>:activeWorkbench==='workshop'?<ForgeSalvagePanel/>:<ForgeTutorialPanel/>}</motion.div></AnimatePresence></main><aside className="forge-side-panel"><ForgeMasteryPanel mastery={mastery} rate={rate} attempts={g.forgeAttempts??0} successes={g.forgeSuccesses??0}/><ForgeMaterialsPanel materials={materials} totalMaterials={totalMaterials} onSelect={setMaterialInfo}/><ForgeTutorialPanel showAttunement={false}/></aside></div>{materialInfo&&<MaterialSourceDialog material={materialInfo} onClose={()=>setMaterialInfo(undefined)}/>}</div>
}
function Panel({title,children,className=''}:{title?:string;children:React.ReactNode;className?:string}){const content=className.split(' ').includes('summary'),guildHead=className.split(' ').includes('guild-head');const nodes=React.Children.toArray(children);return <section className={'panel '+className}>{title&&<h2 className="panel-title">{title}</h2>}{content?<>{nodes.slice(0,-4)}<hr/><MapGuildMissions/></>:children}{guildHead&&<button className="guild-reset-rank" onClick={resetGuildRanking}>Reiniciar ranking</button>}</section>}
// Ferramenta de teste da Guilda (zera ranking e contratos); o modo Moderno a mostra na Visão geral.
function resetGuildRanking(){if(window.confirm('Zerar ranking, contratos aceitos e progresso da Guilda? Seus demais itens e avanços serão preservados.'))useGame.setState({guildAccepted:[],guildProgress:{},guildClaimed:[],guildNotice:'Ranking da Guilda reiniciado para teste.'})}
...
function CharacterScreen(){const g=useGame();const h=HEROES.find(x=>x.id===g.heroId)!;const li=levelInfo(g.xp),permanentLife=g.permanentLife??0,champion=championStats(g),championCard=championCardForActiveHero(g),profile=heroStatProfile(g.heroId);const ability=heroAbilityParts(h.habilidade);const subChoiceId=g.specializations?.['30'];const subChoice=g.heroId?HERO_SUBCLASSES[g.heroId]?.find((s:any)=>s.id===subChoiceId):undefined;const totalAllocated=PRIMARY_ATTRIBUTES.reduce((sum,key)=>sum+(g.allocatedAttr[key]??0),0)
...
function ChronicleScreen(){const g=useGame(),modernChronicle=useUiMode()==='modern',identity=CLASS_IDENTITIES[g.heroId as keyof typeof CLASS_IDENTITIES],activeDifficulty=DIFFICULTIES[g.difficultyMode as keyof typeof DIFFICULTIES]??DIFFICULTIES.veterano
 const difficultyPicker=<><div className="difficulty-row" role="group" aria-label="Dificuldade da campanha">{Object.entries(DIFFICULTIES).map(([id,d])=><button className={g.difficultyMode===id?'active':''} aria-pressed={g.difficultyMode===id} title={d.descricao} onClick={()=>g.setDifficulty(id as DifficultyMode)} key={id}><strong>{d.nome}</strong><small>Inimigos ×{d.enemy} • recompensas ×{d.reward}</small></button>)}</div><p className="difficulty-description">{activeDifficulty.descricao}</p></>
 // Modo Moderno: o capítulo atual vira o cabeçalho (cinemática, objetivo real e a ação de seguir viagem).
 const story=campChapter(g),questCount=Object.keys(g.activeStoryQuests??{}).length
 return <div className="chronicle-page">{modernChronicle?<ScreenMasthead className="chronicle-masthead" assetUrl={assetUrl} art={chapterScene(storyChapterArt(story.chapter))} eyebrow={`Crônicas • Ato ${story.act} • ${story.region}`} title={story.title} lead={story.complete?<>Objetivo cumprido: escolha o rumo da história na aba <b>História</b>.</>:<>Objetivo atual: <b>{story.objective}</b>. Classe: {identity?.nome}.</>} stats={[{label:'Objetivo',value:`${story.current}/${story.required}`,hot:story.complete},{label:'Missões ativas',value:questCount},{label:'Dificuldade',value:activeDifficulty.nome}]} action={{label:g.subregionId?'Voltar à exploração':'Continuar expedição',detail:g.territory,icon:<Swords size={20}/>,onClick:()=>g.setScreen(continueTarget(g))}}>{difficultyPicker}</ScreenMasthead>:<Panel className="chronicle-hero"><span className="eyebrow">CRÔNICAS DA CAMPANHA</span><h1>Crônicas de Havendown</h1><p>{identity?.nome}: {identity?.texto}</p>{difficultyPicker}</Panel>}{g.difficultyMode==='aventura'&&<div className="exploration-note"><Sparkles/>Modo Aventura ativo: combate mais leve pra você aproveitar a história em paz. Troque a qualquer momento aqui mesmo.</div>}{modernChronicle?<ChronicleTabsModern/>:<div className="moved-systems"><StoryQuestsJournalPanel/><StoryCampaignPanel/></div>}</div>}
// Crônicas no modo Moderno: História e Missões da campanha, em abas. Até a v0.9.6 a tela também guardava Bestiário,
// Conquistas, Masmorras e uma aba "Regras" (que na verdade era o resumo da build): cada um foi para a tela do seu assunto
// (Conquistas, Masmorras, Equipamento).
function ChronicleTabsModern(){
 ...
 const tabs=[{id:'story',label:'História'},{id:'quests',label:'Missões',count:questCount||undefined}]
 return <div className="chronicle-tabs"><SectionTabs tabs={tabs} active={tab} onChange={setTab} label="Seções das Crônicas" idPrefix="chronicle"/><SectionPanel idPrefix="chronicle" id="story" active={tab}><StoryCampaignPanel/></SectionPanel><SectionPanel idPrefix="chronicle" id="quests" active={tab}><StoryQuestsJournalPanel/></SectionPanel></div>
}
// Masmorras: antes uma aba das Crônicas; é uma atividade de combate, então virou tela própria dentro de Expedição.
function DungeonScreen(){const g=useGame(),modern=useUiMode()==='modern',lvl=levelInfo(g.xp).lvl
 return <div className="dungeon-page">{modern&&<ScreenMasthead className="dungeon-masthead" assetUrl={assetUrl} art={SCENE_ART.journey} eyebrow="Expedição • Masmorras" title="Masmorras" lead={g.dungeonActive?<>Expedição em andamento na profundidade <b>{g.dungeonDepth}</b>. Trocar de região encerra o progresso atual.</>:<>Salas em sequência, chefe a cada cinco andares e recompensas que crescem com a profundidade. Escolha a região que serve de base.</>} stats={[{label:'Profundidade',value:g.dungeonDepth,hot:g.dungeonActive},{label:'Seu nível',value:lvl},{label:'Expedição',value:g.dungeonActive?'Em andamento':'Nenhuma'}]} action={{label:'Escolher a base',detail:'Regiões e níveis logo abaixo',icon:<Castle size={20}/>,onClick:()=>focusSection('.dungeon-picker')}} secondary={[{label:'Voltar ao mapa',icon:<ArrowLeft size={16}/>,onClick:()=>g.setScreen('map')}]}/>}<DungeonPanel/></div>}
// Conquistas: títulos, histórico de progresso e bestiário estavam em abas das Crônicas, enquanto o grupo "Conquistas" do
// topo só abria a Coleção de cartas. Agora ficam juntos aqui; a Coleção é a tela ao lado, no mesmo grupo.
function AchievementsScreen(){const g=useGame(),modern=useUiMode()==='modern',[tab,setTab]=React.useState('titles')
 const unlocked=unlockedAchievements(g).length,known=Object.keys(g.bestiary??{}).length
 const sections=modern?<><SectionTabs tabs={[{id:'titles',label:'Conquistas',count:unlocked||undefined},{id:'bestiary',label:'Bestiário',count:known||undefined}]} active={tab} onChange={setTab} label="Seções de Conquistas" idPrefix="achievements"/><SectionPanel idPrefix="achievements" id="titles" active={tab}><AchievementsPanel/><ProgressHistoryPanel/></SectionPanel><SectionPanel idPrefix="achievements" id="bestiary" active={tab}><BestiaryPanel/></SectionPanel></>:<div className="moved-systems"><AchievementsPanel/><ProgressHistoryPanel/><BestiaryPanel/></div>
 return <div className="achievements-page">{modern&&<ScreenMasthead className="achievements-masthead" assetUrl={assetUrl} art={SCENE_ART.party} eyebrow="Conquistas" title="Conquistas e bestiário" lead={<>Títulos conquistados, marcos da campanha e as criaturas que você já enfrentou. As cartas descobertas ficam na <b>Coleção</b>.</>} stats={[{label:'Conquistas',value:`${unlocked}/${ACHIEVEMENTS.length}`,hot:unlocked>0},{label:'Criaturas no bestiário',value:known}]} action={{label:'Abrir a Coleção',detail:'Cartas de heróis, itens e criaturas',icon:<Images size={20}/>,onClick:()=>g.setScreen('gallery')}}/>}{sections}</div>}
...
function InventoryScreen(){
  ...
  // Modo Moderno: cabeçalho da mochila com totais reais e o caminho para repor suprimentos.
  const modern=useUiMode()==='modern',supplies=consumableTotals(g.inventory),questTotal=questItemEntries.reduce((sum,[,n])=>sum+n,0)
  return <>{modern&&<ScreenMasthead className="inventory-masthead" assetUrl={assetUrl} eyebrow="Inventário • Mochila" title="Mochila" lead={supplies.total?<>{supplies.total} {supplies.total===1?'consumível':'consumíveis'} de {supplies.kinds} {supplies.kinds===1?'tipo':'tipos'} prontos para a viagem. Poções de efeito ativo ficam marcadas até o fim do efeito.</>:<>Nenhum consumível na mochila. Poções e elixires ficam na Loja; alguns inimigos e baús também deixam suprimentos.</>} stats={[{label:'Consumíveis',value:supplies.total},{label:'Itens de missão',value:questTotal},{label:'Equip. guardados',value:`${g.equipmentBag.length}/${capacity}`},{label:'Ouro',value:g.gold}]} action={{label:'Comprar suprimentos',detail:supplies.total?`Ouro disponível: ${g.gold}`:'A mochila está sem consumíveis',icon:<ShoppingBag size={20}/>,onClick:()=>g.setScreen('shop')}} secondary={[{label:'Equipamentos',icon:<Shield size={16}/>,onClick:()=>g.setScreen('equipment')}]}/>}<div className="two-col">
...
function EquipmentScreen(){const g=useGame(),modernEquipment=useUiMode()==='modern';const capacity=equipmentBagCapacity(g);const dualWielding=equipmentWeaponClass(equipmentByRef(g.equipped.mao_direita))==='facas'
 ...
 // Modo Moderno: o cabeçalho aponta a melhoria que já está na mochila (mesma comparação do saque) ou manda à Forja.
 const [pdSlot,setPdSlot]=React.useState<Slot>('mao_direita'),upgrades=modernEquipment?bagUpgrades(g):[],equippedCount=SLOT_ORDER.filter(slot=>g.equipped[slot]).length
 const equipMasthead=modernEquipment&&<ScreenMasthead className="equipment-masthead" assetUrl={assetUrl} eyebrow="Herói • Equipamento" title="Equipamento" lead={<>{equippedCount} de {SLOT_ORDER.length} espaços ocupados e {g.equipmentBag.length}/{capacity} peças na mochila. Escolha um espaço para comparar as peças que servem nele.</>} stats={[{label:'Poder de ataque',value:attackValue(g)},{label:'Armadura',value:armorValue(g)},{label:'Vida máxima',value:maxHp(g)},{label:'Melhorias na mochila',value:upgrades.length,hot:upgrades.length>0}]} action={upgrades.length?{label:'Ver melhoria na mochila',detail:`${upgrades.length} ${upgrades.length===1?'peça melhor':'peças melhores'} que a vestida`,icon:<Sparkles size={20}/>,onClick:()=>{setPdSlot(upgrades[0].slot);window.setTimeout(()=>focusSection('.pd-candidates'),0)}}:{label:'Aprimorar na Forja',detail:'Reforce as peças vestidas',icon:<Wand2 size={20}/>,onClick:()=>g.setScreen('forge')}} secondary={[{label:'Ficha do herói',icon:<ScrollText size={16}/>,onClick:()=>g.setScreen('character')}]}/>
 return <>{equipMasthead}<div className="equipment-layout">{modernEquipment?<PaperdollModern assetUrl={assetUrl} art={item=>cardArt(item)} slotNames={slotNames} selectedSlot={pdSlot} onSelectSlot={setPdSlot}/>:<Panel title="Slots equipados"><div className="slot-grid">{SLOT_ORDER.map((slot)=>{const id=g.equipped[slot],e=equipmentByRef(id);if(!e)return <button key={slot} className={'slot '+(slot==='botas'?'boots':'')}><span>{slotNames[slot]}</span><div className="slot-empty">{slot==='mao_esquerda'&&dualWielding?'Ocupada pelas facas':'Vazio'}</div></button>;const p=equipmentStatParts(e,id,g);const affinity=compatibilityLabel(e,g.heroId);const bagSlot=slot==='bolsa';const stats=bagSlot?`Capacidade ${e.capacidade??8} espaços`:p.text;return <button key={slot} className={'slot '+(slot==='botas'?'boots':'')+(bagSlot?' backpack-slot':'')} onClick={bagSlot?undefined:()=>g.unequip(slot)}><span>{slotNames[slot]}</span><ArtPreview className="slot-art-preview" image={cardArt(e)} name={e.nome} text={`${e.habilidade} • ${affinity}${elementalNote(g,id!)}${gemNote(id,g)}${craftedEffectNote(id,g)}`} stats={stats} instanceRef={id}/><div className="slot-info"><strong>{e.nome}</strong><div className="slot-stats">{bagSlot?`CAPACIDADE ${e.capacidade??8}`:formatItemStatsShort(p.total)}</div><small className="slot-effect">{affinity}{elementalNote(g,id!)}{gemNote(id,g)}{craftedEffectNote(id,g)}</small></div><small className="slot-remove">{bagSlot?'equipe outra bolsa para trocar':'clique para retirar'}</small></button>})}</div></Panel>}<div className="equipment-side-col"><Panel title="Atributos e bônus" className="equipment-attr-panel"><Stat label="Vida máxima" value={`(${maxHp(g)-itemBonus.life}+${itemBonus.life})=${maxHp(g)}`}/><Stat label="Poder de ataque" value={`(${attackValue(g)-itemBonus.atk}+${itemBonus.atk})=${attackValue(g)}`}/><Stat label="Armadura" value={`${armorValue(g)} (mitiga ${defenseValue(g)} de dano físico)`}/><Stat label="Esquiva" value={percentText(heroDodgeChance(g))}/><Stat label="Dos equipamentos" value={formatItemStats(itemBonus.items)}/><hr/><div className="equipment-elemental-list">{elementalEntries.length?elementalEntries.map(entry=><div key={entry.id} className="equipment-elemental-row"><span>{entry.name}</span><b className={entry.kind==='Elemento'?'elemental-attack':'elemental-resist'}>{entry.kind}: {entry.value}</b><small>{entry.detail}</small></div>):<p className="muted">Nenhum bônus elemental equipado — forje um item com sucesso ou derrote um chefe para conseguir um.</p>}</div><hr/><h3 className="equipment-subtitle">Bônus especiais de equipamento</h3><div className="equipment-elemental-list">{specialBonusEntries.length?specialBonusEntries.map(entry=><div key={entry.effect} className="equipment-elemental-row"><span>{entry.label}</span><b className="elemental-attack">Ativo</b></div>):<p className="muted">Nenhum bônus especial (crítico, esquiva, cura...) equipado — refine uma peça na Forja para conseguir um.</p>}</div><p className="muted">A sintonia cresce com o nível do herói e com o nível efetivo da peça. Resistência soma redução por item equipado; armas elementais aumentam a chance de aplicar a condição no crítico.</p></Panel><Panel title={`Equipamentos guardados ${g.equipmentBag.length}/${capacity}`}><div className="equipment-bag-toolbar"><div className="inventory-search-bar"><Search size={14}/><input type="text" placeholder="Buscar na mochila..." value={bagSearch} onChange={e=>setBagSearch(e.target.value)}/>{bagSearch&&<button className="clear-search" onClick={()=>setBagSearch('')}>×</button>}</div><button className="sort-bag-btn" onClick={()=>g.sortEquipmentBag()} title="Auto-organizar mochila por raridade e nível"><ArrowUpDown size={14}/> Organizar</button></div><div className="item-grid compact">{filteredBag.map((id,idx)=>{const e=equipmentByRef(id);if(!e)return null;const p=equipmentStatParts(e,id,g);const allowed=equipmentClassAllowed(e,g.heroId);const levelAllowed=equipmentLevelAllowed(e,g.xp);const required=equipmentRequiredLevel(e);const fits=e.slot!=='bolsa'||g.equipmentBag.length<=(e.capacidade??8);const dualLocked=e.slot==='mao_esquerda'&&equipmentWeaponClass(equipmentByRef(g.equipped.mao_direita))==='facas';const equipLabel=!allowed?'Impossível equipar':!levelAllowed?`Requer nível ${required}`:!fits?`Reduza para ${e.capacidade} itens`:dualLocked?'Facas ocupam as duas mãos':'Equipar';const stats=e.slot==='bolsa'?`Capacidade ${e.capacidade??8} espaços`:p.text;const targetSlot=e.slot==='anel_1'&&g.equipped.anel_1?'anel_2':e.slot;const equippedRef=g.equipped[targetSlot];const equippedItem=equippedRef?equipmentByRef(equippedRef):undefined;const pseudoState={heroId:g.heroId,equipmentUpgrades:g.equipmentUpgrades,equipmentGems:g.equipmentGems} as any;const candBreakdown=equipmentInstanceBreakdown(e,id,pseudoState);const eqBreakdown=equippedItem?equipmentInstanceBreakdown(equippedItem,equippedRef,pseudoState):undefined;const statDelta=Object.fromEntries(ITEM_STAT_KEYS.map(key=>[key,candBreakdown.total[key]-(eqBreakdown?.total[key]??0)])) as unknown as ItemStats;const isLocked=Boolean(g.lockedEquipment?.[id]);return <ItemCard key={id+idx} image={cardArt(e)} rarity={cardRarity(e,'Equipamento')} name={e.nome} subtitle={e.slot==='bolsa'?`${e.capacidade} espaços`:slotNames[e.slot]} footer={`${e.habilidade} • ${compatibilityLabel(e,g.heroId)}${elementalNote(g,id!)}${gemNote(id,g)}${craftedEffectNote(id,g)}`} previewStats={stats} instanceRef={id} locked={isLocked} onToggleLock={()=>g.toggleLockEquipment(id)} statDiff={e.slot!=='bolsa'?statDelta:undefined}><button className={!allowed||!fits||dualLocked?'equip-impossible':!levelAllowed?'equip-level-locked':''} disabled={!allowed||!levelAllowed||!fits||dualLocked} title={!allowed?compatibilityLabel(e,g.heroId):!levelAllowed?`Disponível no nível ${required}`:!fits?'Há equipamentos demais para esta bolsa':dualLocked?'Combate com facas exige as duas mãos livres':undefined} onClick={()=>g.equip(id)}>{equipLabel}</button></ItemCard>})}</div></Panel></div></div><EquipmentRulesPanel/></>}
function ItemCard({image,name,subtitle,footer,previewStats,previewAllowEquip=false,rarity='comum',instanceRef,badge,children,locked,onToggleLock,statDiff}:{image:string;name:string;subtitle?:string;footer?:string;previewStats?:string;previewAllowEquip?:boolean;rarity?:Rarity;instanceRef?:string;badge?:string;children?:React.ReactNode;locked?:boolean;onToggleLock?:()=>void;statDiff?:Partial<ItemStats>}){const equipment=EQUIPMENT.find(item=>item.nome===name);const emblem=equipment?cardEmblem(equipment,'Equipamento'):undefined;const owner=equipment?.classeExclusiva??(equipment?equipmentAffinity(equipment):undefined);const emblemLabel=classOwnerLabel(owner)
 const tiltRef=useCardTilt(true)
  return <motion.article {...rarityMotionProps(rarity)} whileHover={rarityHoverLift(rarity)} whileTap={effectsReduced()?undefined:{scale:.992}} className={`item-card item-rarity-${rarity}${equipment?'':' item-card-contain'}`} tabIndex={0} role="button" aria-label={`Abrir detalhes de ${name}`} onClick={event=>{const target=event.target as HTMLElement;if(target.closest('button,a,input,select,textarea,label'))return;const trigger=(event.currentTarget as HTMLElement).querySelector('.art-preview-trigger') as HTMLElement|null;trigger?.click()}} onKeyDown={event=>{if(event.key!=='Enter'&&event.key!==' ')return;const target=event.target as HTMLElement;if(target.closest('button,a,input,select,textarea,label'))return;event.preventDefault();const trigger=(event.currentTarget as HTMLElement).querySelector('.art-preview-trigger') as HTMLElement|null;trigger?.click()}}><div ref={tiltRef as React.RefObject<HTMLDivElement>} data-rarity-tier={rarityMotionTier(rarity)} className={`item-art-wrap tilt-card holo-rainbow holo-textured rarity-${rarity}`}><ArtPreview image={image} name={name} text={footer} stats={previewStats??subtitle} compareEquipment={Boolean(equipment)} allowEquip={previewAllowEquip} instanceRef={instanceRef}/>{emblem&&<img className="item-class-emblem" src={assetUrl(emblem)} alt={emblemLabel} title={`Classe: ${emblemLabel}`}/>}{badge&&<span className="item-new-badge"><Sparkles size={11}/>{badge}</span>}</div><div className="item-copy"><div className="item-title-row"><strong>{name}</strong><div className="item-title-actions">{onToggleLock&&<button type="button" className={`item-lock-btn${locked?' locked':''}`} title={locked?'Item protegido contra venda e desmonte':'Travar item (proteger contra venda/desmonte)'} onClick={e=>{e.stopPropagation();onToggleLock()}}>{locked?<Lock size={12}/>:<Unlock size={12}/>}</button>}<span className={`mini-rarity rarity-${rarity}`}>{rarityLabel[rarity]}</span></div></div>{statDiff&&ITEM_STAT_KEYS.some(key=>statDiff[key])&&<div className="item-stat-diff">{ITEM_STAT_KEYS.filter(key=>statDiff[key]).map(key=>{const delta=statDiff[key]!;return <span key={key} className={delta>0?'stat-better':'stat-worse'}>{ITEM_STAT_SHORT[key]} {delta>0?'+':''}{delta}{key==='esquiva'?'%':''}</span>})}</div>}{subtitle&&<span>{subtitle}</span>}{footer&&<small>{footer}</small>}{children}</div></motion.article>}
...
function ShopRow({image,name,rarity='comum',emblem,emblemLabel,meta,description,price,afford=true,actionLabel,actionDisabled=false,actionTitle,onAction,previewStats,compareEquipment=false,quantity=0,statChips,verdict}:{image:string;name:string;rarity?:Rarity;emblem?:string;emblemLabel?:string;meta:string;description:string;price:number;afford?:boolean;actionLabel:string;actionDisabled?:boolean;actionTitle?:string;onAction:()=>void;previewStats?:string;compareEquipment?:boolean;quantity?:number;statChips?:ShopStatChip[];verdict?:ShopVerdict}){
 return <motion.article {...rarityMotionProps(rarity)} whileHover={rarityHoverLift(rarity)} whileTap={effectsReduced()?undefined:{scale:.992}} className={`shop-row item-rarity-${rarity}${afford?'':' unaffordable'}${statChips?.length?' has-stats':''}`} tabIndex={0} role="button" aria-label={`Abrir detalhes de ${name}`} onClick={event=>{const target=event.target as HTMLElement;if(target.closest('button,a,input,select,textarea,label'))return;const trigger=(event.currentTarget as HTMLElement).querySelector('.art-preview-trigger') as HTMLElement|null;trigger?.click()}} onKeyDown={event=>{if(event.key!=='Enter'&&event.key!==' ')return;const target=event.target as HTMLElement;if(target.closest('button,a,input,select,textarea,label'))return;event.preventDefault();const trigger=(event.currentTarget as HTMLElement).querySelector('.art-preview-trigger') as HTMLElement|null;trigger?.click()}}>
  ...
  <div className="shop-row-info">
   ...
   {statChips&&statChips.length>0&&<ul className="shop-row-stats" aria-label="Atributos da peça">{statChips.map(chip=><li key={chip.key} className={`${chip.tone??''}${chip.dim?' dim':''}`} title={chip.title} aria-label={chip.title}><small>{chip.label}</small><b>{chip.value}</b>{chip.delta&&<em>{chip.delta}</em>}</li>)}</ul>}
   {verdict&&<span className={`shop-row-verdict ${verdict.tone}`} title={verdict.title}>{verdict.text}</span>}
...
function LootEquipmentPanel(){
 ...
 return <section className="loot-equipment">
  ...
  <div className="item-grid compact">{g.equipmentBag.map((id,idx)=>{
   ...
   const stats=e.slot==='bolsa'?`Capacidade ${e.capacidade??8} espaços`:p.text
...
function GuildScreen(){
 ...
 const guildOverview=<>{modernGuild&&import.meta.env.DEV&&<button className="guild-reset-rank guild-reset-rank-inline" onClick={resetGuildRanking}>Reiniciar ranking (teste)</button>}<section className="guild-rank-panel"><div className="guild-current-rank" style={{'--rank-color':rank.cor} as React.CSSProperties}><Shield/><span><small>RANK DE AVENTUREIRO</small><strong>{rank.nome}</strong></span></div><div className="guild-rank-progress"><div><span>{reputation} pontos de reputação</span><strong>{nextRank?`Próximo: ${nextRank.nome} (${nextRank.minimo})`:'Rank máximo alcançado'}</strong></div><div className="xp-track"><div style={{width:nextRank?`${Math.min(100,(reputation-rank.minimo)/(nextRank.minimo-rank.minimo)*100)}%`:'100%'}}/></div></div><div className="guild-rank-road">{GUILD_RANKS.map(r=><span className={reputation>=r.minimo?'reached':''} style={{'--rank-color':r.cor} as React.CSSProperties} key={r.id} title={`${r.nome}: ${r.minimo} pontos`}><i/>{r.nome}</span>)}</div></section><ChallengesPanel/><DailyProvisionPanel/></>
 const guildBagSummary=<div className="guild-summary"><span><ScrollText/><small>CONTRATOS ATIVOS</small><strong>{active}</strong></span><span><Trophy/><small>PRONTOS PARA ENTREGAR</small><strong>{completed}</strong></span><span><Package/><small>ESPAÇO NA BOLSA</small><strong>{g.equipmentBag.length}/{equipmentBagCapacity(g)}</strong></span></div>
 ...
 return <div className="guild-page">{modernGuild?<ScreenMasthead className="guild-masthead" assetUrl={assetUrl} art={SCENE_ART.allies} eyebrow="Salão dos aventureiros" title="Guilda de Havendown" lead={<>Rank <b>{rank.nome}</b> com {reputation} pontos de reputação{nextRank?<>; faltam {nextRank.minimo-reputation} para {nextRank.nome}</>:null}. Contratos entregues liberam missões mais valiosas.</>} stats={[{label:'Contratos ativos',value:active},{label:'Prontos',value:completed,hot:completed>0},{label:'Rank',value:rank.nome}]} action={{...guildNextStep(active,completed),icon:<ScrollText size={20}/>,onClick:()=>{setGuildTab('contracts');window.setTimeout(()=>focusSection('#guild-panel-contracts'),0)}}} secondary={[{label:'Voltar ao mapa',icon:<ArrowLeft size={16}/>,onClick:()=>g.setScreen('map')}]} aside={<div className="masthead-npc"><img src={assetUrl(GUILD_LEADER.retrato)} alt=""/><div><small>{GUILD_LEADER.titulo}</small><strong>{GUILD_LEADER.nome}</strong><p>“{guildLeaderLine(g,active,completed,reputation)}”</p></div></div>}/>:<><Panel className="guild-head"><button onClick={()=>g.setScreen('map')}><ArrowLeft/>Voltar ao mapa</button><div><span className="eyebrow">SALÃO DOS AVENTUREIROS</span><h1>Guilda de Havendown</h1><p>Aceite contratos, aumente sua reputação e conquiste acesso às missões mais valiosas.</p></div><Shield className="guild-crest"/></Panel><div className="guild-summary"><span><ScrollText/><small>CONTRATOS ATIVOS</small><strong>{active}</strong></span><span><Trophy/><small>PRONTOS</small><strong>{completed}</strong></span><span><Shield/><small>RANK</small><strong>{rank.nome}</strong></span></div>
