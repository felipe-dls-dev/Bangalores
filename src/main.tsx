...
function CombatDiceRoll({roll}:{roll:{attacker:'hero'|'enemy';attackRoll:number;defenseRoll:number;attackBase:number;defenseBase:number;attackTotal:number;defenseTotal:number;damage:number;shieldBlocked?:number}}){return <motion.aside className={`combat-dice-roll ${roll.attacker}`} initial={{opacity:0,y:-18,scale:.9}} animate={{opacity:1,y:0,scale:1}} exit={{opacity:0,y:-12}} aria-live="assertive"><small>{roll.attacker==='hero'?'SEU TESTE DE ATAQUE':'ATAQUE DO INIMIGO'}</small><div className="combat-dice-pair"><div><span>ATAQUE</span><motion.b className="combat-die attack-die" animate={{rotate:[0,110,250,370,360],scale:[.75,1.18,.88,1]}} transition={{duration:.55}}>{roll.attackRoll}</motion.b><em>{roll.attackBase} + {roll.attackRoll} = <strong>{roll.attackTotal}</strong></em></div><i>VS</i><div><span>DEFESA</span><motion.b className="combat-die defense-die" animate={{rotate:[0,-120,-260,-370,-360],scale:[.75,1.18,.88,1]}} transition={{duration:.55}}>{roll.defenseRoll}</motion.b><em>{roll.defenseBase} + {roll.defenseRoll} = <strong>{roll.defenseTotal}</strong></em></div></div><p>DANO <strong>{roll.damage}</strong>{roll.shieldBlocked?` • ESCUDO BLOQUEOU ${roll.shieldBlocked}`:''}</p></motion.aside>}
